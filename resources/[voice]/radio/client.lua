...
local PlayerData = Core.Functions.GetPlayerData() -- Just for resource restart (same as event handler)
...
local function connecttoradio(channel)
    ...
    if SplitStr(tostring(channel), ".")[2] ~= nil and SplitStr(tostring(channel), ".")[2] ~= "" then
        Core.Functions.Notify(Config.messages['joined_to_radio'] ..channel.. ' MHz', 'success')
    else
        Core.Functions.Notify(Config.messages['joined_to_radio'] ..channel.. '.00 MHz', 'success')
    end
end
...
local function leaveradio()
    ...
    Core.Functions.Notify(Config.messages['you_leave'] , 'error')
end
            ...
            if rchannel ~= RadioChannel then
                if Config.RestrictedChannels[rchannel] ~= nil then
                    if Config.RestrictedChannels[rchannel][PlayerData.job.name] and PlayerData.job.onduty then
                        ...
                        Core.Functions.Notify(Config.messages['restricted_channel_error'], 'error')
                    end
                else
                    connecttoradio(rchannel)
                end
            else
                Core.Functions.Notify(Config.messages['you_on_radio'] , 'error')
            end
        else
            Core.Functions.Notify(Config.messages['invalid_radio'] , 'error')
        end
    else
        Core.Functions.Notify(Config.messages['invalid_radio'] , 'error')
    end
...
RegisterNUICallback('leaveRadio', function(_, cb)
    if RadioChannel == 0 then
        Core.Functions.Notify(Config.messages['not_on_radio'], 'error')
    else
        leaveradio()
    end
    cb("ok")
...
RegisterNUICallback("volumeUp", function(_, cb)
	if RadioVolume <= 95 then
		...
		Core.Functions.Notify(Config.messages["volume_radio"] .. RadioVolume, "success")
		exports["pma-voice"]:setRadioVolume(RadioVolume)
	else
		Core.Functions.Notify(Config.messages["decrease_radio_volume"], "error")
	end
    cb('ok')
end)
...
RegisterNUICallback("volumeDown", function(_, cb)
	if RadioVolume >= 10 then
		...
		Core.Functions.Notify(Config.messages["increase_radio_volume"], "error")
	end
    cb('ok')
end)
...
RegisterNUICallback("increaseradiochannel", function(_, cb)
    ...
    Core.Functions.Notify(Config.messages["increase_decrease_radio_channel"] .. newChannel, "success")
    cb("ok")
end)
