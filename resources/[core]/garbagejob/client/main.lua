...
local function DeliverAnim()
    ...
    if Config.UseTarget and hasBag then
        ...
        exports['target']:RemoveTargetEntity(garbageVehicle)
        if (amountOfBags - 1) <= 0 then
            Core.Functions.TriggerCallback('garbagejob:server:NextStop', function(hasMoreStops, nextStop, newBagAmount)
                if hasMoreStops and nextStop ~= 0 then
                    ...
                    SetGarbageRoute()
                    Core.Functions.Notify(Lang:t("info.all_bags"))
                    SetVehicleDoorShut(garbageVehicle, 5, false)
                else
                    if hasMoreStops and nextStop == currentStop then
                        Core.Functions.Notify(Lang:t("info.depot_issue"))
                        amountOfBags = 0
                    else
                        -- You are done with work here.
                        Core.Functions.Notify(Lang:t("info.done_working"))
                        SetVehicleDoorShut(garbageVehicle, 5, false)
                        RemoveBlip(deliveryBlip)
                        SetRouteBack()
                        amountOfBags = 0
            ...
            if amountOfBags > 1 then
                Core.Functions.Notify(Lang:t("info.bags_left", { value = amountOfBags }))
            else
                Core.Functions.Notify(Lang:t("info.bags_still", { value = amountOfBags }))
            end
            exports['target']:AddCircleZone('garbagebin', vector3(CL.coords.x, CL.coords.y, CL.coords.z), 2.0,{
...
function TakeAnim()
    local ped = PlayerPedId()
    Core.Functions.Progressbar("bag_pickup", Lang:t("info.picking_bag"), math.random(3000, 5000), false, true, {
        ...
        if Config.UseTarget and not hasBag then
            hasBag = true
            exports['target']:RemoveZone("garbagebin")
            exports['target']:AddTargetEntity(garbageVehicle, {
            options = {
        ...
        StopAnimTask(PlayerPedId(), "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
        Core.Functions.Notify(Lang:t("error.cancled"), "error")
    end)
end
                ...
                if not hasBag and canTakeBag then
                    if Distance < 1.5 then
                        if not GarbText then
                            GarbText = true
                            exports['core']:DrawText(Lang:t("info.grab_garbage"), 'left')
                        end
                    ...
                    if DoesEntityExist(garbageVehicle) then
                        ...
                        if TruckDist < 2 then
                            if not TrucText then
                                TrucText = true
                                exports['core']:DrawText(Lang:t("info.dispose_garbage"), 'left')
                            end
                            if IsControlJustPressed(0, 51) and hasBag then
                                StopAnimTask(PlayerPedId(), 'missfbi4prepp1', '_bag_walk_garbage_man', 1.0)
                                DeliverAnim()
                                Core.Functions.Progressbar("deliverbag", Lang:t("info.progressbar"), 2000, false, true, {
                                        disableMovement = true,
                                        disableCarMovement = true,
                                        disableMouse = false,
                                        disableCombat = true,
                        ...
                        Core.Functions.Notify(Lang:t("error.no_truck"), "error")
                        hasBag = false
                    end
                end
            end
    ...
    CreateThread(function()
        while ControlListen do
            if IsControlJustReleased(0, 38) then
                TriggerEvent("garbagejob:client:MainMenu")
            end
            Wait(1)
        end
    end)
    ...
    for i = 1, #Config.Peds do
        ...
        if Config.UseTarget then
            exports['target']:AddTargetEntity(ped, {
                options = {{type = "client", event = "garbagejob:client:MainMenu", label = Lang:t("target.talk"), icon = 'fa-solid fa-recycle', job = "garbage",}},
                distance = 2.0
            })
        else
            ...
            if options then
                ...
                zone:onPlayerInOut(function(inside)
                    if LocalPlayer.state.isLoggedIn then
                        if inside then
                            exports['core']:DrawText(Lang:t("info.talk"), 'left')
                            Listen4Control()
                        else
                            ControlListen = false
                            exports['core']:HideText()
...
RegisterNetEvent('garbagejob:client:RequestRoute', function()
    if garbageVehicle then continueworking = true TriggerServerEvent('garbagejob:server:PayShift', continueworking) end
    Core.Functions.TriggerCallback('garbagejob:server:NewShift', function(shouldContinue, firstStop, totalBags)
        if shouldContinue then
            if not garbageVehicle then
                ...
                for _,v in pairs(Config.Locations["vehicle"].coords) do
                    if not IsAnyVehicleNearPoint(vector3(v.x,v.y,v.z), 2.5) then
                        Core.Functions.TriggerCallback('Core:Server:SpawnVehicle', function(netId)
                            ...
                            garbageVehicle = veh
                            SetVehicleNumberPlateText(veh, "CORE-" .. tostring(math.random(1000, 9999)))
                            ...
                            Core.Functions.Notify(Lang:t("info.deposit_paid", { value = Config.TruckPrice }))
                            Core.Functions.Notify(Lang:t("info.started"))
                            TriggerServerEvent("garbagejob:server:payDeposit")
                ...
                if occupied then
                    Core.Functions.Notify(Lang:t("error.all_occupied"))
                end
            end
            currentStop = firstStop
            currentStopNum = 1
            ...
            Core.Functions.Notify(Lang:t("info.not_enough", { value = Config.TruckPrice }))
        end
    end, continueworking)
end)

RegisterNetEvent('garbagejob:client:RequestPaycheck', function()
    if garbageVehicle then
        BringBackCar()
        Core.Functions.Notify(Lang:t("info.truck_returned"))
    end
    TriggerServerEvent('garbagejob:server:PayShift')
end)

RegisterNetEvent('garbagejob:client:MainMenu', function()
    local MainMenu = {}
    MainMenu[#MainMenu+1] = {isMenuHeader = true,header = Lang:t("menu.header")}
    MainMenu[#MainMenu+1] = { header = Lang:t("menu.collect"),txt = Lang:t("menu.return_collect"),params = { event = 'garbagejob:client:RequestPaycheck',}}
    if not garbageVehicle or finished then
        MainMenu[#MainMenu+1] = { header = Lang:t("menu.route"), txt = Lang:t("menu.request_route"), params = { event = 'garbagejob:client:RequestRoute',}}
    end
