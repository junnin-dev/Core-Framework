...
Core.Functions.CreateCallback("garbagejob:server:NewShift", function(source, cb, continue)
    ...
    if CanPay(Player) or continue then
        ...
        TriggerClientEvent('Core:Notify', source, Lang:t("error.not_enough", {value = Config.TruckPrice}), "error")
    end
    cb(shouldContinue, nextStop, bagNum, totalNumberOfStops)
end)

RegisterNetEvent("garbagejob:server:payDeposit", function()
    local Player = Core.Functions.GetPlayer(source)
...
Core.Functions.CreateCallback("garbagejob:server:NextStop", function(source, cb, currentStop, currentStopNum, currLocation)
    local Player = Core.Functions.GetPlayer(source)
    local CitizenId = Player.PlayerData.citizenid
    ...
    if(math.random(100) >= Config.CryptoStickChance) and Config.GiveCryptoStick then
        ...
        TriggerClientEvent('inventory:client:ItemBox', source, Core.Shared.Items["cryptostick"], 'add')
        TriggerClientEvent('Core:Notify', source, Lang:t("info.found_crypto"))
    ...
    if distance <= 20 then
        ...
        TriggerClientEvent('Core:Notify', source, Lang:t("error.too_far"), "error")
    end
    cb(shouldContinue,newStop,newBagAmount)
end)

Core.Functions.CreateCallback('garbagejob:server:EndShift', function(source, cb)
    local Player = Core.Functions.GetPlayer(source)
    local CitizenId = Player.PlayerData.citizenid
    local status = false
...
RegisterNetEvent('garbagejob:server:PayShift', function(continue)
    ...
    if Routes[CitizenId] ~= nil then
        ...
        if tonumber(Routes[CitizenId].stopsCompleted) < tonumber(Routes[CitizenId].totalNumberOfStops) then
            depositPay = 0
            TriggerClientEvent('Core:Notify', src, Lang:t("error.early_finish", {completed = Routes[CitizenId].stopsCompleted, total = Routes[CitizenId].totalNumberOfStops}), "error")
        end
        if continue then
            depositPay = 0
        end
        ...
        TriggerClientEvent('Core:Notify', src, Lang:t("success.pay_slip", {total = totalToPay, deposit = payoutDeposit}), "success")
        Routes[CitizenId] = nil
    else
        TriggerClientEvent('Core:Notify', source, Lang:t("error.never_clocked_on"), "error")
    end
end)
...
Core.Commands.Add("cleargarbroutes", "Remove as rotas Garbo para usuário (somente admin)", {{name="id", help="Player ID (may be empty)"}}, false, function(source, args)
    ...
    TriggerClientEvent('Core:Notify', source, Lang:t("success.clear_routes", {value = count}), "success")
    Routes[CitizenId] = nil
end, "admin")
