...
Core.Functions.CreateCallback('doorlock:server:setupDoors', function(_, cb)
	cb(Config.DoorList)
end)

Core.Functions.CreateCallback('doorlock:server:checkItems', function(source, cb, items, needsAll)
...
RegisterNetEvent('doorlock:server:updateState', function(doorID, locked, src, usedLockpick, unlockAnyway, enableSounds, enableAnimation, sentSource)
	...
	TriggerClientEvent('doorlock:client:setState', -1, playerId, doorID, locked, src or false, enableSounds, enableAnimation)
	...
	SetTimeout(Config.DoorList[doorID].autoLock, function()
		...
		if Config.DoorStates[doorID] == nil then Config.DoorStates[doorID] = locked elseif Config.DoorStates[doorID] ~= locked then Config.DoorStates[doorID] = nil end
		TriggerClientEvent('doorlock:client:setState', -1, playerId, doorID, true, src or false, enableSounds, enableAnimation)
	end)
end)
...
Core.Commands.Add('newdoor', Lang:t("general.newdoor_command_description"), {}, false, function(source)
