...
local function lockpickFinish(success)
	if success then
		Core.Functions.Notify(Lang:t("success.lockpick_success"), 'success', 2500)
		...
		Wait(1800)
		TriggerServerEvent('doorlock:server:updateState', closestDoor.id, false, false, true, false) -- Broadcast new state of the door to everyone
	else
		Core.Functions.Notify(Lang:t("error.lockpick_fail"), 'error', 2500)
...
local function isAuthorized(door)
	...
	if door.items then
		local p = promise.new()
		Core.Functions.TriggerCallback('doorlock:server:checkItems', function(result)
			p:resolve(result)
...
function SetupDoors()
	local p = promise.new()
	Core.Functions.TriggerCallback('doorlock:server:setupDoors', function(result)
		p:resolve(result)
	end)
...
RegisterNetEvent('doorlock:client:setState', function(serverId, doorID, state, src, enableAnimation, enableSounds)
...
RegisterNetEvent('doorlock:client:addNewDoor', function()
	...
	if Config.DoorList[identifier] then
		Core.Functions.Notify((Lang:t("error.door_identifier_exists")):format(identifier), 'error')
		canContinue = true
		return
	end
	...
	if doorData.doortype == 'door' or doorData.doortype == 'sliding' or doorData.doortype == 'garage' then
		...
		if not model or model == 0 then Core.Functions.Notify(Lang:t("error.door_not_found"), 'error') canContinue = true return end
		result = DoorSystemFindExistingDoor(coords.x, coords.y, coords.z, model)
		if result then Core.Functions.Notify(Lang:t("error.door_registered"), 'error') canContinue = true return end
		...
		if not model[1] or model[1] == 0 or not model[2] or model[2] == 0 then Core.Functions.Notify(Lang:t("error.door_not_found"), 'error') return end
		if entity[1] == entity[2] then Core.Functions.Notify(Lang:t("error.same_entity"), 'error') canContinue = true return end
		doorData.doorHash = {}
		for i = 1, 2 do
...
RegisterCommand('remotetriggerdoor', function()
	...
	TriggerServerEvent('doorlock:server:updateState', nearestDoor.id, not nearestDoor.data.locked, NetworkGetNetworkIdFromEntity(playerPed), false, false, true, true) -- Broadcast new state of the door to everyone
end, false)
