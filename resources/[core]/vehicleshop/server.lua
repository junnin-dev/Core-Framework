...
RegisterNetEvent('vehicleshop:server:addPlayer', function(citizenid)
    financetimer[citizenid] = os.time()
end)

-- Deduct stored game time from player on logout
RegisterNetEvent('vehicleshop:server:removePlayer', function(citizenid)
...
local function GeneratePlate()
    local plate = Core.Shared.RandomInt(1) .. Core.Shared.RandomStr(2) .. Core.Shared.RandomInt(3) .. Core.Shared.RandomStr(2)
...
-- Callbacks
Core.Functions.CreateCallback('vehicleshop:server:getVehicles', function(source, cb)
    local src = source
...
RegisterNetEvent('vehicleshop:server:deleteVehicle', function (netId)
    local vehicle = NetworkGetEntityFromNetworkId(netId)
    DeleteEntity(vehicle)
end)

-- Sync vehicle for other players
RegisterNetEvent('vehicleshop:server:swapVehicle', function(data)
    local src = source
    TriggerClientEvent('vehicleshop:client:swapVehicle', -1, data)
    Wait(1500)-- let new car spawn
    TriggerClientEvent('vehicleshop:client:homeMenu', src)-- reopen main menu
end)

-- Send customer for test drive
RegisterNetEvent('vehicleshop:server:customTestDrive', function(vehicle, playerid)
    local src = source
    local target = tonumber(playerid)
    if not Core.Functions.GetPlayer(target) then
        TriggerClientEvent('Core:Notify', src, Lang:t('error.Invalid_ID'), 'error')
        return
    end
    if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(target))) < 3 then
        TriggerClientEvent('vehicleshop:client:customTestDrive', target, vehicle)
    else
        TriggerClientEvent('Core:Notify', src, Lang:t('error.playertoofar'), 'error')
    end
end)

-- Make a finance payment
RegisterNetEvent('vehicleshop:server:financePayment', function(paymentAmount, vehData)
    local src = source
    local player = Core.Functions.GetPlayer(src)
    ...
    if newBalance > 0 then
        if player and paymentAmount >= minPayment then
            ...
            elseif bank >= paymentAmount then
                ...
                TriggerClientEvent('Core:Notify', src, Lang:t('error.notenoughmoney'), 'error')
            end
        else
            TriggerClientEvent('Core:Notify', src, Lang:t('error.minimumallowed') .. comma_value(minPayment), 'error')
        end
    else
        TriggerClientEvent('Core:Notify', src, Lang:t('error.overpaid'), 'error')
    end
end)


-- Pay off vehice in full
RegisterNetEvent('vehicleshop:server:financePaymentFull', function(data)
    local src = source
    local player = Core.Functions.GetPlayer(src)
    local cash = player.PlayerData.money['cash']
    local bank = player.PlayerData.money['bank']
    ...
    if player and vehBalance ~= 0 then
        ...
        TriggerClientEvent('Core:Notify', src, Lang:t('error.alreadypaid'), 'error')
    end
end)

-- Buy public vehicle outright
RegisterNetEvent('vehicleshop:server:buyShowroomVehicle', function(vehicle)
    local src = source
    ...
    local vehiclePrice = Core.Shared.Vehicles[vehicle]['price']
    ...
    if cash > tonumber(vehiclePrice) then
        ...
        TriggerClientEvent('Core:Notify', src, Lang:t('success.purchased'), 'success')
        TriggerClientEvent('vehicleshop:client:buyShowroomVehicle', src, vehicle, plate)
...
RegisterNetEvent('vehicleshop:server:financeVehicle', function(downPayment, paymentAmount, vehicle)
    local src = source
    ...
    if downPayment > vehiclePrice then return TriggerClientEvent('Core:Notify', src, Lang:t('error.notworth'), 'error') end
    if downPayment < minDown then return TriggerClientEvent('Core:Notify', src, Lang:t('error.downtoosmall'), 'error') end
    if paymentAmount > Config.MaximumPayments then return TriggerClientEvent('Core:Notify', src, Lang:t('error.exceededmax'), 'error') end
    local plate = GeneratePlate()
...
RegisterNetEvent('vehicleshop:server:sellShowroomVehicle', function(data, playerid)
    local src = source
    local player = Core.Functions.GetPlayer(src)
    local target = Core.Functions.GetPlayer(tonumber(playerid))
    ...
    if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(target.PlayerData.source))) < 3 then
        ...
        if cash >= tonumber(vehiclePrice) then
            ...
            TriggerClientEvent('vehicleshop:client:buyShowroomVehicle', target.PlayerData.source, vehicle, plate)
            target.Functions.RemoveMoney('cash', vehiclePrice, 'vehicle-bought-in-showroom')
            player.Functions.AddMoney('bank', commission)
            TriggerClientEvent('Core:Notify', src, Lang:t('success.earned_commission', {amount = comma_value(commission)}), 'success')
            exports['management']:AddMoney(player.PlayerData.job.name, vehiclePrice)
            TriggerClientEvent('Core:Notify', target.PlayerData.source, Lang:t('success.purchased'), 'success')
        elseif bank >= tonumber(vehiclePrice) then
            MySQL.insert('INSERT INTO player_vehicles (license, citizenid, vehicle, hash, mods, plate, garage, state) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', {
                target.PlayerData.license,
                cid,
...
RegisterNetEvent('vehicleshop:server:sellfinanceVehicle', function(downPayment, paymentAmount, vehicle, playerid)
    local src = source
    local player = Core.Functions.GetPlayer(src)
    local target = Core.Functions.GetPlayer(tonumber(playerid))
...
RegisterNetEvent('vehicleshop:server:checkFinance', function()
    local src = source
    local player = Core.Functions.GetPlayer(src)
    ...
    if result[1] then
        TriggerClientEvent('Core:Notify', src, Lang:t('general.paymentduein', {time = Config.PaymentWarning}))
        ...
        for _, v in pairs(vehicles) do
            ...
            TriggerClientEvent('Core:Notify', src, Lang:t('error.repossessed', {plate = plate}), 'error')
        end
    end
end)

-- Transfer vehicle to player in passenger seat
Core.Commands.Add('transfervehicle', Lang:t('general.command_transfervehicle'), {{name = 'ID', help = Lang:t('general.command_transfervehicle_help')}, {name = 'amount', help = Lang:t('general.command_transfervehicle_amount')}}, false, function(source, args)
    local src = source
    local buyerId = tonumber(args[1])
    local sellAmount = tonumber(args[2])
    if buyerId == 0 then return TriggerClientEvent('Core:Notify', src, Lang:t('error.Invalid_ID'), 'error') end
    local ped = GetPlayerPed(src)
    local targetPed = GetPlayerPed(buyerId)
    if targetPed == 0 then return TriggerClientEvent('Core:Notify', src, Lang:t('error.buyerinfo'), 'error') end
    local vehicle = GetVehiclePedIsIn(ped, false)
    if vehicle == 0 then return TriggerClientEvent('Core:Notify', src, Lang:t('error.notinveh'), 'error') end
    local plate = Core.Shared.Trim(GetVehicleNumberPlateText(vehicle))
    if not plate then return TriggerClientEvent('Core:Notify', src, Lang:t('error.vehinfo'), 'error') end
    local player = Core.Functions.GetPlayer(src)
    local target = Core.Functions.GetPlayer(buyerId)
    local row = MySQL.single.await('SELECT * FROM player_vehicles WHERE plate = ?', {plate})
    if Config.PreventFinanceSelling then
        if row.balance > 0 then return TriggerClientEvent('Core:Notify', src, Lang:t('error.financed'), 'error') end
    end
    if row.citizenid ~= player.PlayerData.citizenid then return TriggerClientEvent('Core:Notify', src, Lang:t('error.notown'), 'error') end
    if #(GetEntityCoords(ped) - GetEntityCoords(targetPed)) > 5.0 then return TriggerClientEvent('Core:Notify', src, Lang:t('error.playertoofar'), 'error') end
    local targetcid = target.PlayerData.citizenid
    local targetlicense = Core.Functions.GetIdentifier(target.PlayerData.source, 'license')
    if not target then return TriggerClientEvent('Core:Notify', src, Lang:t('error.buyerinfo'), 'error') end
    if not sellAmount then
        ...
        TriggerClientEvent('Core:Notify', src, Lang:t('success.gifted'), 'success')
        TriggerClientEvent('vehiclekeys:client:SetOwner', buyerId, plate)
        TriggerClientEvent('Core:Notify', buyerId, Lang:t('success.received_gift'), 'success')
        return
    end
    if target.Functions.GetMoney('cash') > sellAmount then
        ...
        TriggerClientEvent('Core:Notify', src, Lang:t('success.soldfor') .. comma_value(sellAmount), 'success')
        TriggerClientEvent('vehiclekeys:client:SetOwner', buyerId, plate)
        TriggerClientEvent('Core:Notify', buyerId, Lang:t('success.boughtfor') .. comma_value(sellAmount), 'success')
    elseif target.Functions.GetMoney('bank') > sellAmount then
        ...
        TriggerClientEvent('Core:Notify', src, Lang:t('error.buyertoopoor'), 'error')
    end
end)
