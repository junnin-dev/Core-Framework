...
AddEventHandler('Core:Client:OnPlayerLoaded', function()
    ...
    TriggerServerEvent('vehicleshop:server:addPlayer', citizenid)
    TriggerServerEvent('vehicleshop:server:checkFinance')
...
RegisterNetEvent('Core:Client:OnPlayerUnload', function()
    local citizenid = PlayerData.citizenid
    TriggerServerEvent('vehicleshop:server:removePlayer', citizenid)
    PlayerData = {}
end)
...
local vehHeaderMenu = {
    {
        ...
        params = {
            event = 'vehicleshop:client:showVehOptions'
        }
    }
}
...
local financeMenu = {
    {
        ...
        params = {
            event = 'vehicleshop:client:getVehicles'
        }
    }
}
...
local returnTestDrive = {
    {
        ...
        params = {
            event = 'vehicleshop:client:TestDriveReturn'
        }
    }
}
...
local function getVehName()
    return Core.Shared.Vehicles[Config.Shops[insideShop]["ShowroomVehicles"][ClosestVehicle].chosenVehicle]["name"]
end

local function getVehPrice()
    return comma_value(Core.Shared.Vehicles[Config.Shops[insideShop]["ShowroomVehicles"][ClosestVehicle].chosenVehicle]["price"])
end

local function getVehBrand()
    return Core.Shared.Vehicles[Config.Shops[insideShop]["ShowroomVehicles"][ClosestVehicle].chosenVehicle]['brand']
end
...
local function createTestDriveReturn()
    ...
    testDriveZone:onPlayerInOut(function(isPointInside)
        if isPointInside and IsPedInAnyVehicle(PlayerPedId()) then
            ...
            exports['menu']:openMenu(returnTestDrive)
        else
        ...
        while inTestDrive do
            if GetGameTimer() < gameTimer + tonumber(1000 * testDriveTime) then
                ...
                if secondsLeft >= tonumber(1000 * testDriveTime) - 20 then
                    TriggerServerEvent('vehicleshop:server:deleteVehicle', testDriveVeh)
                    testDriveVeh = 0
                    inTestDrive = false
                    SetEntityCoords(PlayerPedId(), prevCoords)
                    Core.Functions.Notify(Lang:t('general.testdrive_complete'))
                end
        ...
        combo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if PlayerData and PlayerData.job and (PlayerData.job.name == Config.Shops[insideShop]['Job'] or Config.Shops[insideShop]['Job'] == 'none') then
                    exports['menu']:showHeader(vehHeaderMenu)
                end
            else
                exports['menu']:closeMenu()
            end
                ...
                while insideShop do
                    ...
                    vehicleMenu = {
                        ...
                        },
                        {
                            ...
                            params = {
                                event = 'vehicleshop:client:TestDrive',
                            }
                        },
                        {
                            ...
                            params = {
                                isServer = true,
                                event = 'vehicleshop:server:buyShowroomVehicle',
                                args = {
                        ...
                        },
                        {
                            ...
                            params = {
                                event = 'vehicleshop:client:openFinance',
                                args = {
                                    price = getVehPrice(),
                                    buyVehicle = Config.Shops[insideShop]["ShowroomVehicles"][ClosestVehicle].chosenVehicle
                                }
                        ...
                        },
                        {
                            ...
                            params = {
                                event = 'vehicleshop:client:vehCategories',
                            }
                        },
                    }
                    Wait(1000)
                ...
                while insideShop and PlayerData.job and PlayerData.job.name == Config.Shops[name]['Job'] do
                    ...
                    vehicleMenu = {
                        ...
                        },
                        {
                            ...
                            params = {
                                event = 'vehicleshop:client:openIdMenu',
                                args = {
                        ...
                        },
                        {
                            ...
                            icon = "fa-solid fa-coins",
                            params = {
                                event = 'vehicleshop:client:openCustomFinance',
                                args = {
                                    price = getVehPrice(),
                                    vehicle = Config.Shops[insideShop]["ShowroomVehicles"][ClosestVehicle].chosenVehicle
                                }
...
function Init()
    ...
    CreateThread(function()
        ...
        financeZone:onPlayerInOut(function(isPointInside)
            if isPointInside then
                exports['menu']:showHeader(financeMenu)
            else
                exports['menu']:closeMenu()
            end
        end)
...
-- Events
RegisterNetEvent('vehicleshop:client:homeMenu', function()
    exports['menu']:openMenu(vehicleMenu)
end)

RegisterNetEvent('vehicleshop:client:showVehOptions', function()
    exports['menu']:openMenu(vehicleMenu)
end)

RegisterNetEvent('vehicleshop:client:TestDrive', function()
    if not inTestDrive and ClosestVehicle ~= 0 then
        ...
        Core.Functions.TriggerCallback('Core:Server:SpawnVehicle', function(netId)
            ...
            testDriveVeh = netId
            Core.Functions.Notify(Lang:t('general.testdrive_timenoti', {testdrivetime = Config.Shops[tempShop]["TestDriveTimeLimit"]}))
        ...
        Core.Functions.Notify(Lang:t('error.testdrive_alreadyin'), 'error')
    end
end)

RegisterNetEvent('vehicleshop:client:customTestDrive', function(data)
...
RegisterNetEvent('vehicleshop:client:TestDriveReturn', function()
    ...
    if veh == entity then
        ...
        Core.Functions.Notify(Lang:t('error.testdrive_return'), 'error')
    end
end)

RegisterNetEvent('vehicleshop:client:vehCategories', function()
	local catmenu = {}
    local categoryMenu = {
        {
            ...
            params = {
                event = 'vehicleshop:client:homeMenu'
            }
        }
    }
	for k, v in pairs(Core.Shared.Vehicles) do
        if type(Core.Shared.Vehicles[k]["shop"]) == 'table' then
            for _, shop in pairs(Core.Shared.Vehicles[k]["shop"]) do
                if shop == insideShop then
                    catmenu[v.category] = v.category
                end
            end
        elseif Core.Shared.Vehicles[k]["shop"] == insideShop then
                catmenu[v.category] = v.category
        end
    end
    for k, v in pairs(catmenu) do
        categoryMenu[#categoryMenu + 1] = {
            ...
            params = {
                event = 'vehicleshop:client:openVehCats',
                args = {
                    catName = k
                }
            }
    ...
    exports['menu']:openMenu(categoryMenu)
end)

RegisterNetEvent('vehicleshop:client:openVehCats', function(data)
    local vehMenu = {
        {
            header = Lang:t('menus.goback_header'),
            icon = "fa-solid fa-angle-left",
    ...
    for k, v in pairs(Core.Shared.Vehicles) do
        if Core.Shared.Vehicles[k]["category"] == data.catName then
            if type(Core.Shared.Vehicles[k]["shop"]) == 'table' then
                for _, shop in pairs(Core.Shared.Vehicles[k]["shop"]) do
                    if shop == insideShop then
                        vehMenu[#vehMenu + 1] = {
                            ...
                            params = {
                                isServer = true,
                                event = 'vehicleshop:server:swapVehicle',
                                args = {
                                    toVehicle = v.model,
    ...
    exports['menu']:openMenu(vehMenu)
end)

RegisterNetEvent('vehicleshop:client:openFinance', function(data)
    ...
    if dialog then
        ...
        TriggerServerEvent('vehicleshop:server:financeVehicle', dialog.downPayment, dialog.paymentAmount, data.buyVehicle)
    end
end)

RegisterNetEvent('vehicleshop:client:openCustomFinance', function(data)
    ...
    if dialog then
        ...
        TriggerServerEvent('vehicleshop:server:sellfinanceVehicle', dialog.downPayment, dialog.paymentAmount, data.vehicle, dialog.playerid)
    end
end)

RegisterNetEvent('vehicleshop:client:swapVehicle', function(data)
    local shopName = data.ClosestShop
    if Config.Shops[shopName]["ShowroomVehicles"][data.ClosestVehicle].chosenVehicle ~= data.toVehicle then
        local closestVehicle, closestDistance = Core.Functions.GetClosestVehicle(vector3(Config.Shops[shopName]["ShowroomVehicles"][data.ClosestVehicle].coords.x, Config.Shops[shopName]["ShowroomVehicles"][data.ClosestVehicle].coords.y, Config.Shops[shopName]["ShowroomVehicles"][data.ClosestVehicle].coords.z))
...
RegisterNetEvent('vehicleshop:client:buyShowroomVehicle', function(vehicle, plate)
    tempShop = insideShop -- temp hacky way of setting the shop because it changes after the callback has returned since you are outside the zone
    Core.Functions.TriggerCallback('Core:Server:SpawnVehicle', function(netId)
        local veh = NetToVeh(netId)
        exports['LegacyFuel']:SetFuel(veh, 100)
        ...
        TriggerEvent("vehiclekeys:client:SetOwner", Core.Functions.GetPlate(veh))
        TriggerServerEvent("occasion:server:SaveVehicleProps", Core.Functions.GetVehicleProperties(veh))
    end, vehicle, Config.Shops[tempShop]["VehicleSpawn"], true)
end)

RegisterNetEvent('vehicleshop:client:getVehicles', function()
    Core.Functions.TriggerCallback('vehicleshop:server:getVehicles', function(vehicles)
        local ownedVehicles = {}
        for _, v in pairs(vehicles) do
            if v.balance ~= 0 then
                local name = Core.Shared.Vehicles[v.vehicle]["name"]
                ...
                ownedVehicles[#ownedVehicles + 1] = {
                    ...
                    icon = "fa-solid fa-car-side",
                    params = {
                        event = 'vehicleshop:client:getVehicleFinance',
                        args = {
                            vehiclePlate = plate,
                            balance = v.balance,
        ...
        if #ownedVehicles > 0 then
            exports['menu']:openMenu(ownedVehicles)
        else
            Core.Functions.Notify(Lang:t('error.nofinanced'), 'error', 7500)
        end
    end)
end)

RegisterNetEvent('vehicleshop:client:getVehicleFinance', function(data)
    local vehFinance = {
        {
            header = Lang:t('menus.goback_header'),
            params = {
        ...
        },
        {
            ...
            icon = "fa-solid fa-hand-holding-dollar",
            params = {
                event = 'vehicleshop:client:financePayment',
                args = {
                    vehData = data,
        ...
        },
        {
            ...
            params = {
                isServer = true,
                event = 'vehicleshop:server:financePaymentFull',
                args = {
    ...
    exports['menu']:openMenu(vehFinance)
end)

RegisterNetEvent('vehicleshop:client:financePayment', function(data)
    local dialog = exports['input']:ShowInput({
    ...
    if dialog then
        ...
        TriggerServerEvent('vehicleshop:server:financePayment', dialog.paymentAmount, data.vehData)
    end
end)

RegisterNetEvent('vehicleshop:client:openIdMenu', function(data)
    local dialog = exports['input']:ShowInput({
        header = Core.Shared.Vehicles[data.vehicle]["name"],
        submitText = Lang:t('menus.submit_text'),
        inputs = {
            {
                text = Lang:t('menus.submit_ID'),
    ...
    if dialog then
        ...
        if data.type == 'testDrive' then
            TriggerServerEvent('vehicleshop:server:customTestDrive', data.vehicle, dialog.playerid)
        elseif data.type == 'sellVehicle' then
            TriggerServerEvent('vehicleshop:server:sellShowroomVehicle', data.vehicle, dialog.playerid)
        end
    end
end)
