            ...
            CreateThread(function()
                repeat
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        Core.Functions.Notify(Lang:t('success.dropped_off'), 'success')
                        if NpcData.DeliveryBlip ~= nil then
                            RemoveBlip(NpcData.DeliveryBlip)
                        end
                        ...
                        resetNpcTask()
                        nextStop()
                        TriggerEvent('busjob:client:DoBusNpc')
...
local function busGarage()
    ...
    for _, v in pairs(Config.AllowedVehicles) do
        vehicleMenu[#vehicleMenu + 1] = {
            header = v.label,
            params = {
                event = "busjob:client:TakeVehicle",
                args = {
                    model = v.model
                }
            }
...
RegisterNetEvent("busjob:client:TakeVehicle", function(data)
    local coords = Config.Location
    if (BusData.Active) then
        Core.Functions.Notify(Lang:t('error.one_bus_active'), 'error')
        return
    else
...
RegisterNetEvent('busjob:client:DoBusNpc', function()
    if whitelistedVehicle() then
        if not NpcData.Active then
            ...
            Core.Functions.Notify(Lang:t('info.goto_busstop'), 'primary')
                    ...
                    CreateThread(function()
                        repeat
                            ...
                            if IsControlJustPressed(0, 38) then
                                ...
                                TriggerServerEvent('busjob:server:NpcPay')
                                exports["core"]:HideText()
                                PolyZone:destroy()
                                break
                            end
            ...
            Core.Functions.Notify(Lang:t('error.already_driving_bus'), 'error')
        end
    else
        Core.Functions.Notify(Lang:t('error.not_in_bus'), 'error')
    end
end)
                        ...
                        if not inVeh then
                            ...
                            if IsControlJustReleased(0, 38) then
                                if (not NpcData.Active or NpcData.Active and NpcData.NpcTaken == false) then
                                    ...
                                    Core.Functions.Notify(Lang:t('error.drop_off_passengers'), 'error')
                                end
                            end
                        end
                    until not inRange
