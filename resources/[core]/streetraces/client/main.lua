                    ...
                    if Races[k] ~= nil then
                        if #(pos - vector3(Races[k].startx, Races[k].starty, Races[k].startz)) < 15.0 and not Races[k].started then
                            ...
                            if IsControlJustReleased(0, 74) then
                                TriggerServerEvent("streetraces:JoinRace", k)
                            end
                        end
                    end
            ...
            if RaceId ~= 0 and InRace then
                if #(pos - vector3(Races[RaceId].endx, Races[RaceId].endy, pos.z)) < 250.0 and Races[RaceId].started then
                    ...
                    if #(pos - vector3(Races[RaceId].endx, Races[RaceId].endy, pos.z)) < 15.0 then
                        TriggerServerEvent("streetraces:RaceWon", RaceId)
                        InRace = false
                    end
                end
            end
...
RegisterNetEvent('streetraces:StartRace', function(race)
...
RegisterNetEvent('streetraces:RaceDone', function(race, winner)
    if RaceId ~= 0 and RaceId == race then
        RaceId = 0
        InRace = false
...
RegisterNetEvent('streetraces:StopRace', function()
    RaceId = 0
    InRace = false
end)

RegisterNetEvent('streetraces:CreateRace', function(amount)
    local pos = GetEntityCoords(PlayerPedId(), true)
...
RegisterNetEvent('streetraces:SetRace', function(RaceTable)
    Races = RaceTable
end)

RegisterNetEvent('streetraces:SetRaceId', function(race)
    RaceId = race
    SetNewWaypoint(Races[RaceId].endx, Races[RaceId].endy)
end)
...
function RaceCountDown()
    ...
    while RaceCount ~= 0 do
        ...
        Core.Functions.Notify(RaceCount, 'primary', 800)
        Wait(1000)
