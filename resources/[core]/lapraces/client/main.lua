...
local function DeleteCheckpoint()
    ...
    if RaceData.ClosestCheckpoint ~= 0 then
        if CreatorData.Checkpoints[RaceData.ClosestCheckpoint] ~= nil then
            ...
            Core.Functions.Notify('Você não pode ir para rápido', 'error')
        end
    else
        Core.Functions.Notify('Você não pode ir muito rápido', 'error')
    end
end
...
local function SaveRace()
    ...
    TriggerServerEvent('lapraces:server:SaveRace', CreatorData)

    Core.Functions.Notify('Race: '..CreatorData.RaceName..' is saved!', 'success')
        ...
        while RaceData.InCreator do
            ...
            if PlayerVeh ~= 0 then
                ...
                if IsControlJustPressed(0, 162) or IsDisabledControlJustPressed(0, 162) then
                    if CreatorData.Checkpoints ~= nil and next(CreatorData.Checkpoints) ~= nil then
                        ...
                        Core.Functions.Notify('Você ainda não colocou nenhum posto de controle..', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, 311) or IsDisabledControlJustPressed(0, 311) then
                    if CreatorData.Checkpoints ~= nil and #CreatorData.Checkpoints >= 2 then
                        ...
                        Core.Functions.Notify('Você deve ter pelo menos 10 pontos de verificação', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, 40) or IsDisabledControlJustPressed(0, 40) then
                    if CreatorData.TireDistance + 1.0 ~= 16.0 then
                        ...
                        Core.Functions.Notify('Você não pode ir mais alto que 15')
                    end
                end
                ...
                if IsControlJustPressed(0, 39) or IsDisabledControlJustPressed(0, 39) then
                    if CreatorData.TireDistance - 1.0 ~= 1.0 then
                        ...
                        Core.Functions.Notify('Você não pode ir mais baixo de 2')
                    end
                end
            else
                local coords = GetEntityCoords(PlayerPedId())
                DrawText3Ds(coords.x, coords.y, coords.z, 'Você deve estar em um veículo')
            end
            ...
            if IsControlJustPressed(0, 163) or IsDisabledControlJustPressed(0, 163) then
                if not CreatorData.ConfirmDelete then
                    ...
                    Core.Functions.Notify('Press [9] novamente para confirmar', 'error', 5000)
                else
                    ...
                    CreatorData.RaceName = nil
                    CreatorData.Checkpoints = {}
                    Core.Functions.Notify('Race-Editor cancelado!', 'error')
...
local function FinishRace()
    TriggerServerEvent('lapraces:server:FinishPlayer', CurrentRaceData, CurrentRaceData.TotalTime, CurrentRaceData.TotalLaps, CurrentRaceData.BestLap)
    if CurrentRaceData.BestLap ~= 0 then
        Core.Functions.Notify('Race finished in '..SecondsToClock(CurrentRaceData.TotalTime)..', with the best lap: '..SecondsToClock(CurrentRaceData.BestLap))
    else
        Core.Functions.Notify('Race finished in '..SecondsToClock(CurrentRaceData.TotalTime))
    end
    for k, _ in pairs(CurrentRaceData.Checkpoints) do
...
RegisterNetEvent('lapraces:client:StartRaceEditor', function(RaceName)
    if not RaceData.InCreator then
        ...
        Core.Functions.Notify('Você já está fazendo uma corrida.', 'error')
    end
end)

RegisterNetEvent('lapraces:client:UpdateRaceRacerData', function(RaceId, aRaceData)
    if (CurrentRaceData.RaceId ~= nil) and CurrentRaceData.RaceId == RaceId then
        CurrentRaceData.Racers = aRaceData.Racers
    end
end)

RegisterNetEvent('lapraces:client:JoinRace', function(Data, Laps)
    if not RaceData.InRace then
        RaceData.InRace = true
        SetupRace(Data, Laps)
        TriggerServerEvent('lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, false, true)
    else
        Core.Functions.Notify('Você já está em uma corrida ..', 'error')
    end
end)

RegisterNetEvent('lapraces:client:LeaveRace', function(_)
    Core.Functions.Notify('Você completou a corrida!')
    for k, _ in pairs(CurrentRaceData.Checkpoints) do
        if CurrentRaceData.Checkpoints[k].blip ~= nil then
            RemoveBlip(CurrentRaceData.Checkpoints[k].blip)
            CurrentRaceData.Checkpoints[k].blip = nil
...
RegisterNetEvent('lapraces:client:RaceCountdown', function()
    TriggerServerEvent('lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, true, false)
    if CurrentRaceData.RaceId ~= nil then
        while Countdown ~= 0 do
            if CurrentRaceData.RaceName ~= nil then
                if Countdown == 10 then
                    Core.Functions.Notify('A corrida começará em 10 segundos', 'error', 2500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                elseif Countdown <= 5 then
                    Core.Functions.Notify(Countdown, 'error', 500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                end
        ...
        if CurrentRaceData.RaceName ~= nil then
            ...
            Core.Functions.Notify('VAI!', 'success', 1000)
        ...
        Core.Functions.Notify('Você não está atualmente em uma corrida ..', 'error')
    end
end)

RegisterNetEvent('lapraces:client:PlayerFinishs', function(RaceId, Place, FinisherData)
    if CurrentRaceData.RaceId ~= nil then
        if CurrentRaceData.RaceId == RaceId then
            Core.Functions.Notify(FinisherData.PlayerData.charinfo.firstname..' is finished on spot: '..Place, 'error', 3500)
        end
    end
end)

RegisterNetEvent('lapraces:client:WaitingDistanceCheck', function()
    Wait(1000)
    CreateThread(function()
        while true do
                ...
                if CurrentRaceData.Checkpoints[1] ~= nil then
                    ...
                    if dist > 115.0 then
                        if ToFarCountdown ~= 0 then
                            ...
                            Core.Functions.Notify('Volte ao início ou você será expulso da corrida: '..ToFarCountdown..'s', 'error', 500)
                        else
                            TriggerServerEvent('lapraces:server:LeaveRace', CurrentRaceData)
                            ToFarCountdown = 10
                            break
                        end
                        Wait(1000)
                ...
                if CheckpointDistance < MaxDistance then
                    if CurrentRaceData.TotalLaps == 0 then
                        if CurrentRaceData.CurrentCheckpoint + 1 < #CurrentRaceData.Checkpoints then
                            ...
                            SetNewWaypoint(CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.x, CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.y)
                            TriggerServerEvent('lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, false)
                            DoPilePfx()
                            ...
                            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                            CurrentRaceData.CurrentCheckpoint = CurrentRaceData.CurrentCheckpoint + 1
                            TriggerServerEvent('lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, true)
                            FinishRace()
                        end
                    else
                        if CurrentRaceData.CurrentCheckpoint + 1 > #CurrentRaceData.Checkpoints then
