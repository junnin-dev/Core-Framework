...
local function IsWhitelisted(CitizenId)
    ...
    local Perms = Core.Functions.GetPermission(Player.PlayerData.source)
...
RegisterNetEvent('lapraces:server:FinishPlayer', function(RaceData, TotalTime, TotalLaps, BestLap)
    local src = source
    ...
    if Races[RaceData.RaceId].Records ~= nil and next(Races[RaceData.RaceId].Records) ~= nil then
        if BLap < Races[RaceData.RaceId].Records.Time then
            ...
            TriggerClientEvent('phoneclient:RaceNotify', src, 'You have won the WR from ' .. RaceData.RaceName ..
    ...
    TriggerClientEvent('lapraces:client:PlayerFinishs', -1, RaceData.RaceId, PlayersFinished, Player)
...
RegisterNetEvent('lapraces:server:CreateLapRace', function(RaceName)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
...
RegisterNetEvent('lapraces:server:JoinRace', function(RaceData)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('lapraces:client:JoinRace', src, Races[RaceId], AvailableRaces[AvailableKey].Laps)
    TriggerClientEvent('phoneclient:UpdateLapraces', -1)
    local creatorsource = Core.Functions.GetPlayerByCitizenId(AvailableRaces[AvailableKey].SetupCitizenId).PlayerData
                              .source
    if creatorsource ~= Player.PlayerData.source then
        TriggerClientEvent('phoneclient:RaceNotify', creatorsource,
            string.sub(Player.PlayerData.charinfo.firstname, 1, 1) .. '. ' .. Player.PlayerData.charinfo.lastname ..
                ' the race has been joined!')
    end
end)

RegisterNetEvent('lapraces:server:LeaveRace', function(RaceData)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    local RaceName
...
RegisterNetEvent('lapraces:server:SetupRace', function(RaceId, Laps)
            ...
            if not Races[RaceId].Started then
                ...
                SetTimeout(5 * 60 * 1000, function()
                    if Races[RaceId].Waiting then
                        local AvailableKey = GetOpenedRaceKey(RaceId)
                        for cid, _ in pairs(Races[RaceId].Racers) do
                            local RacerData = Core.Functions.GetPlayerByCitizenId(cid)
                            if RacerData ~= nil then
                                TriggerClientEvent('lapraces:client:LeaveRace', RacerData.PlayerData.source,
                                    Races[RaceId])
                            end
                        end
                        table.remove(AvailableRaces, AvailableKey)
...
RegisterNetEvent('lapraces:server:CancelRace', function(raceId)
    local src = source
    local Player = Core.Functions.GetPlayer(source)
    local AvailableKey = GetOpenedRaceKey(raceId)
    ...
    if AvailableKey ~= nil then
        if AvailableRaces[AvailableKey].SetupCitizenId == Player.PlayerData.citizenid then
            for cid, _ in pairs(Races[raceId].Racers) do
                local RacerData = Core.Functions.GetPlayerByCitizenId(cid)
                if RacerData ~= nil then
                    TriggerClientEvent('lapraces:client:LeaveRace', RacerData.PlayerData.source, Races[raceId])
                end
            end
...
RegisterNetEvent('lapraces:server:UpdateRaceState', function(RaceId, Started, Waiting)
    Races[RaceId].Waiting = Waiting
    Races[RaceId].Started = Started
end)

RegisterNetEvent('lapraces:server:UpdateRacerData', function(RaceId, Checkpoint, Lap, Finished)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('lapraces:client:UpdateRaceRacerData', -1, RaceId, Races[RaceId])
end)

RegisterNetEvent('lapraces:server:StartRace', function(RaceId)
    local src = source
    local MyPlayer = Core.Functions.GetPlayer(src)
    local AvailableKey = GetOpenedRaceKey(RaceId)
    ...
    if RaceId ~= nil then
        if AvailableRaces[AvailableKey].SetupCitizenId == MyPlayer.PlayerData.citizenid then
            ...
            for CitizenId, _ in pairs(Races[RaceId].Racers) do
                local Player = Core.Functions.GetPlayerByCitizenId(CitizenId)
                if Player ~= nil then
                    TriggerClientEvent('lapraces:client:RaceCountdown', Player.PlayerData.source)
                end
            end
            TriggerClientEvent('phoneclient:UpdateLapraces', -1)
        else
...
RegisterNetEvent('lapraces:server:SaveRace', function(RaceData)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
...
Core.Functions.CreateCallback('lapraces:server:GetRacingLeaderboards', function(_, cb)
    cb(Races)
end)

Core.Functions.CreateCallback('lapraces:server:GetRaces', function(_, cb)
    cb(AvailableRaces)
end)

Core.Functions.CreateCallback('lapraces:server:GetListedRaces', function(_, cb)
    cb(Races)
end)

Core.Functions.CreateCallback('lapraces:server:GetRacingData', function(_, cb, RaceId)
    cb(Races[RaceId])
end)

Core.Functions.CreateCallback('lapraces:server:HasCreatedRace', function(source, cb)
    cb(HasOpenedRace(Core.Functions.GetPlayer(source).PlayerData.citizenid))
end)

Core.Functions.CreateCallback('lapraces:server:IsAuthorizedToCreateRaces', function(source, cb, TrackName)
    cb(IsWhitelisted(Core.Functions.GetPlayer(source).PlayerData.citizenid), IsNameAvailable(TrackName))
end)

Core.Functions.CreateCallback('lapraces:server:CanRaceSetup', function(_, cb)
    cb(Config.RaceSetupAllowed)
end)

Core.Functions.CreateCallback('lapraces:server:GetTrackData', function(_, cb, RaceId)
...
Core.Commands.Add("cancelrace", "Cancel going race..", {}, false, function(source, args)
    local Player = Core.Functions.GetPlayer(source)
