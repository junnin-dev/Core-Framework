...
RegisterNetEvent('commandbinding:client:openUI', function()
    openBindingMenu()
end)
...
for k, v in pairs(availableKeys) do
    RegisterCommand(v[1], function()
        if LocalPlayer.state.isLoggedIn and not keyPressed and GetLastInputMethod(0) then
            local keyMeta = Core.Functions.GetPlayerData().metadata["commandbinds"]
            local args
            if next(keyMeta) ~= nil then
                if keyMeta[v[2]]["command"] ~= "" then
                    ...
                    TriggerServerEvent('Core:CallCommand', keyMeta[v[2]]["command"], args)
                    keyPressed = true
                    Wait(1000)
                    keyPressed = false
                else
                    Core.Functions.Notify('Ainda não há nada ['..v[2]..'] vinculada, /binds Para vincular um comando', 'primary', 4000)
                end
            else
                Core.Functions.Notify('Você não vinculou nenhum comando, /binds Para vincular um comando', 'primary', 4000)
            end
        end
    end, false)
...
RegisterNUICallback('save', function(data, cb)
    ...
    Core.Functions.Notify('As ligações de comando foram salvas!', 'success')
    TriggerServerEvent('commandbinding:server:setKeyMeta', keyData)
    cb('ok')
end)
