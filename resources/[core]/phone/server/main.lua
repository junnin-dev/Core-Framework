...
Core.Functions.CreateCallback('phone:server:GetCallState', function(_, cb, ContactData)
...
Core.Functions.CreateCallback('phone:server:DeclineInvoice', function(source, cb, _, _, invoiceId)
    local Invoices = {}
    local Ply = Core.Functions.GetPlayer(source)
    MySQL.query('DELETE FROM phone_invoices WHERE id = ?', {invoiceId})
    local invoices = MySQL.query.await('SELECT * FROM phone_invoices WHERE citizenid = ?', {Ply.PlayerData.citizenid})
...
Core.Functions.CreateCallback('phone:server:GetContactPictures', function(_, cb, Chats)
...
Core.Functions.CreateCallback('phone:server:GetContactPicture', function(_, cb, Chat)
...
Core.Functions.CreateCallback('phone:server:GetPicture', function(_, cb, number)
    local query = '%' .. number .. '%'
    local result = MySQL.query.await('SELECT * FROM players WHERE charinfo LIKE ?', {query})
    if result[1] ~= nil then
...
Core.Functions.CreateCallback('phone:server:FetchResult', function(_, cb, search)
...
Core.Functions.CreateCallback('phone:server:GetVehicleSearchResults', function(_, cb, search)
    search = escape_sqli(search)
    local searchData = {}
    local query = '%' .. search .. '%'
...
Core.Functions.CreateCallback('phone:server:GetCurrentLawyers', function(_, cb)
    local Lawyers = {}
...
Core.Functions.CreateCallback("phone:server:GetWebhook",function(_,cb)
	if WebHook ~= "" then
		cb(WebHook)
	else
...
RegisterNetEvent('phone:server:DeleteTweet', function(tweetId)
    ...
    if delete then
        ...
        TriggerClientEvent('phone:client:UpdateTweets', -1, TWData, nil, true)
    end
end)
...
RegisterNetEvent('phone:server:EditContact', function(newName, newNumber, newIban, oldName, oldNumber, _)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    MySQL.update(
...
RegisterNetEvent('phone:server:UpdateMessages', function(ChatMessages, ChatNumber, _)
    local src = source
...
RegisterNetEvent('phone:server:sendPing', function(data)
    local src = source
    if src == data then
