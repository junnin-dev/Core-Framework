...
RegisterNetEvent('spawn:client:openUI', function(value)
...
RegisterNetEvent('spawn:client:setupSpawns', function(cData, new, apps)
    if not new then
        Core.Functions.TriggerCallback('spawn:server:getOwnedHouses', function(houses)
            local myHouses = {}
            if houses ~= nil then
                for i = 1, (#houses), 1 do
                    myHouses[#myHouses+1] = {
...
RegisterNUICallback('setCam', function(data, cb)
    ...
    elseif type == "normal" then
        SetCam(JN.Spawns[location].coords)
...
RegisterNUICallback('spawnplayer', function(data, cb)
    ...
    if type == "current" then
        ...
        if insideMeta.house ~= nil then
            ...
            TriggerEvent('houses:client:LastLocationHouse', houseId)
        elseif insideMeta.apartment.apartmentType ~= nil or insideMeta.apartment.apartmentId ~= nil then
            local apartmentType = insideMeta.apartment.apartmentType
            local apartmentId = insideMeta.apartment.apartmentId
            TriggerEvent('apartments:client:LastLocationHouse', apartmentType, apartmentId)
        end
        TriggerServerEvent('Core:Server:OnPlayerLoaded')
        TriggerEvent('Core:Client:OnPlayerLoaded')
        PostSpawnPlayer()
    elseif type == "house" then
        PreSpawnPlayer()
        TriggerEvent('houses:client:enterOwnedHouse', location)
        TriggerServerEvent('Core:Server:OnPlayerLoaded')
        TriggerEvent('Core:Client:OnPlayerLoaded')
        ...
        PostSpawnPlayer()
    elseif type == "normal" then
        local pos = JN.Spawns[location].coords
        PreSpawnPlayer()
