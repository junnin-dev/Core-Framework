...
RegisterNetEvent('Core:Client:OnPlayerLoaded', function()
    Core.Functions.TriggerCallback('bankrobbery:server:GetConfig', function(config1, config2, config3)
...
function openLocker(bankId, lockerId) -- Globally Used
    ...
    if bankId == "paleto" then
        ...
        if hasItem then
            ...
            IsDrilling = true
            Core.Functions.Progressbar("open_locker_drill", Lang:t("general.breaking_open_safe"), math.random(18000, 30000), false, true, {
                ...
                TriggerServerEvent('bankrobbery:server:recieveItem', 'paleto', bankId, lockerId)
                Core.Functions.Notify(Lang:t("success.success_message"), "success")
                ...
                DetachEntity(DrillObject, true, true)
                DeleteObject(DrillObject)
                Core.Functions.Notify(Lang:t("error.cancel_message"), "error")
                SetTimeout(500, function()
                    IsDrilling = false
                end)
            end)
        else
            Core.Functions.Notify(Lang:t("error.safe_too_strong"), "error")
            TriggerServerEvent('bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
        end
    elseif bankId == "pacific" then
        ...
        if hasItem then
            ...
            Core.Functions.Progressbar("open_locker_drill", Lang:t("general.breaking_open_safe"), math.random(18000, 30000), false, true, {
                ...
                TriggerServerEvent('bankrobbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
                TriggerServerEvent('bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                TriggerServerEvent('bankrobbery:server:recieveItem', 'pacific', bankId, lockerId)
                Core.Functions.Notify(Lang:t("success.success_message"), "success")
                SetTimeout(500, function()
                    IsDrilling = false
                end)
        ...
        Core.Functions.Progressbar("open_locker", Lang:t("general.breaking_open_safe"), math.random(27000, 37000), false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
            ...
            TriggerServerEvent('bankrobbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
            TriggerServerEvent('bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
            TriggerServerEvent('bankrobbery:server:recieveItem', 'small', bankId, lockerId)
            Core.Functions.Notify(Lang:t("success.success_message"), "success")
            SetTimeout(500, function()
                IsDrilling = false
            end)
...
RegisterNetEvent('electronickit:UseElectronickit', function()
    ...
    Core.Functions.TriggerCallback('bankrobbery:server:isRobberyActive', function(isBusy)
        if not isBusy then
                ...
                if not Config.SmallBanks[closestBank]["isOpened"] then
                    ...
                    if hasItem then
                        Config.ShowRequiredItems({
                            [1] = {name = Core.Shared.Items["electronickit"]["name"], image = Core.Shared.Items["electronickit"]["image"]},
                            [2] = {name = Core.Shared.Items["trojan_usb"]["name"], image = Core.Shared.Items["trojan_usb"]["image"]}
                        }, false)
                        loadAnimDict("anim@gangops@facility@servers@")
                        TaskPlayAnim(ped, 'anim@gangops@facility@servers@', 'hotwire', 3.0, 3.0, -1, 1, 0, false, false, false)
                        Core.Functions.Progressbar("hack_gate", Lang:t("general.connecting_hacking_device"), math.random(5000, 10000), false, true, {
                            disableMovement = true,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true,
                            ...
                            StopAnimTask(ped, "anim@gangops@facility@servers@", "hotwire", 1.0)
                            TriggerServerEvent('bankrobbery:server:removeElectronicKit')
                            ...
                            TriggerServerEvent("bankrobbery:server:callCops", "small", closestBank, pos)
                            copsCalled = true
                        ...
                        Core.Functions.Notify(Lang:t("error.missing_item"), "error")
                    end
                else
                    Core.Functions.Notify(Lang:t("error.bank_already_open"), "error")
                end
            else
                Core.Functions.Notify(Lang:t("error.minimum_police_required", {police = Config.MinimumFleecaPolice}), "error")
            end
        else
            Core.Functions.Notify(Lang:t("error.security_lock_active"), "error", 5500)
        end
    end)
end)

RegisterNetEvent('bankrobbery:client:setBankState', function(bankId)
    if bankId == "paleto" then
...
RegisterNetEvent('bankrobbery:client:enableAllBankSecurity', function()
    for k in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = true
    end
end)

RegisterNetEvent('bankrobbery:client:disableAllBankSecurity', function()
    for k in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = false
    end
end)

RegisterNetEvent('bankrobbery:client:BankSecurity', function(key, status)
    ...
    elseif type(key) == 'number' then
        ...
        error(Lang:t("error.wrong_type", {receiver = 'bankrobbery:client:BankSecurity', argument = "key", receivedType = type(key), receivedValue = key, expected = "table/array"}))
    end
end)

RegisterNetEvent('bankrobbery:client:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == "paleto" then
...
RegisterNetEvent('bankrobbery:client:ResetFleecaLockers', function(BankId)
...
RegisterNetEvent('bankrobbery:client:robberyCall', function(type, coords)
    ...
    for i = 1, #Config.SmallBanks do
        ...
        for k in pairs(Config.SmallBanks[i]["lockers"]) do
            if Config.UseTarget then
                exports['target']:AddBoxZone('fleeca_'..i..'_coords_locker_'..k, Config.SmallBanks[i]["lockers"][k]["coords"], 1.0, 1.0, {
                ...
                lockerZone:onPlayerInOut(function(inside)
                    if inside and closestBank ~= 0 and not IsDrilling and Config.SmallBanks[i]["isOpened"] and not Config.SmallBanks[i]["lockers"][k]["isOpened"] and not Config.SmallBanks[i]["lockers"][k]["isBusy"] then
                        exports['core']:DrawText(Lang:t("general.break_safe_open_option_drawtext"), 'right')
                        currentLocker = k
                    else
