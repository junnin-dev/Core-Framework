...
function Config.OnRobberyStart(bankId)
    ...
    TriggerEvent('scoreboard:server:SetActivityBusy', bankName, true)
    if bankName ~= "bankrobbery" then return end
    TriggerEvent('banking:server:SetBankClosed', bankId, true)
end
...
function Config.OnRobberyTimeoutEnd(bankId)
    local bankName = type(bankId) == "number" and "bankrobbery" or bankId
    TriggerEvent('scoreboard:server:SetActivityBusy', bankName, false)
    if bankName ~= "bankrobbery" then return end
    TriggerEvent('banking:server:SetBankClosed', bankId, false)
end
