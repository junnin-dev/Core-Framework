...
local function CheckStationHits()
    ...
    if table.type(bankHits) == 'array' then TriggerClientEvent("bankrobbery:client:BankSecurity", -1, bankHits, false) end
end
...
RegisterNetEvent('bankrobbery:server:setBankState', function(bankId)
    ...
    if bankId == "paleto" then
        if Config.BigBanks["paleto"]["isOpened"] or #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks["paleto"]["coords"]) > 2.5 then
            return error(Lang:t("error.event_trigger_wrong", {event = "bankrobbery:server:setBankState", extraInfo = " (paleto) ", source = source}))
        end
        Config.BigBanks["paleto"]["isOpened"] = true
        TriggerEvent('bankrobbery:server:setTimeout')
    elseif bankId == "pacific" then
        if Config.BigBanks["pacific"]["isOpened"] or #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks["pacific"]["coords"][2]) > 2.5 then
            return error(Lang:t("error.event_trigger_wrong", {event = "bankrobbery:server:setBankState", extraInfo = " (pacific) ", source = source}))
        end
        ...
        if Config.SmallBanks[bankId]["isOpened"] or #(GetEntityCoords(GetPlayerPed(source)) - Config.SmallBanks[bankId]["coords"]) > 2.5 then
            return error(Lang:t("error.event_trigger_wrong", {event = "bankrobbery:server:setBankState", extraInfo = " (smallbank "..bankId..") ", source = source}))
        end
        Config.SmallBanks[bankId]["isOpened"] = true
        TriggerEvent('bankrobbery:server:SetSmallBankTimeout', bankId)
    end
    TriggerClientEvent('bankrobbery:client:setBankState', -1, bankId)
    robberyBusy = true
    Config.OnRobberyStart(bankId)
end)

RegisterNetEvent('bankrobbery:server:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == "paleto" or bankId == "pacific" then
        if #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks[bankId]["lockers"][lockerId]["coords"]) > 2.5 then
            return error(Lang:t("error.event_trigger_wrong", {event = "bankrobbery:server:setLockerState", extraInfo = " ("..bankId..") ", source = source}))
        end
        ...
        if #(GetEntityCoords(GetPlayerPed(source)) - Config.SmallBanks[bankId]["lockers"][lockerId]["coords"]) > 2.5 then
            return error(Lang:t("error.event_trigger_wrong", {event = "bankrobbery:server:setLockerState", extraInfo = " (smallbank "..bankId..") ", source = source}))
        end
        Config.SmallBanks[bankId]["lockers"][lockerId][state] = bool
    end
    TriggerClientEvent('bankrobbery:client:setLockerState', -1, bankId, lockerId, state, bool)
end)

RegisterNetEvent('bankrobbery:server:recieveItem', function(type, bankId, lockerId)
    local src = source
    ...
    if type == "small" then
        if #(GetEntityCoords(GetPlayerPed(src)) - Config.SmallBanks[bankId]["lockers"][lockerId]["coords"]) > 2.5 then
            return error(Lang:t("error.event_trigger_wrong", {event = "bankrobbery:server:receiveItem", extraInfo = " (smallbank "..bankId..") ", source = source}))
        end
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                 if Config.RewardTypes[itemType].type == "item" then
                    ...
                    TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items[item.item], "add")
                 elseif Config.RewardTypes[itemType].type == "money" then
                    local info = {
                        worth = math.random(2300, 3200)
                    }
                    ply.Functions.AddItem('markedbills', math.random(2,3), false, info)
                    TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items['markedbills'], "add")
                end
            else
                ply.Functions.AddItem('security_card_01', 1)
                TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items['security_card_01'], "add")
            end
        else
            ply.Functions.AddItem('weapon_stungun', 1)
            TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items['weapon_stungun'], "add")
        end
    elseif type == "paleto" then
        if #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks["paleto"]["lockers"][lockerId]["coords"]) > 2.5 then
            return error(Lang:t("error.event_trigger_wrong", {event = "bankrobbery:server:receiveItem", extraInfo = " (paleto) ", source = source}))
        end
        local itemType = math.random(#Config.RewardTypes)
        local tierChance = math.random(1, 100)
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items['security_card_02'], "add")
            end
        else
            ply.Functions.AddItem('weapon_vintagepistol', 1)
            TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items['weapon_vintagepistol'], "add")
        end
    elseif type == "pacific" then
        if #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks["pacific"]["lockers"][lockerId]["coords"]) > 2.5 then
            return error(Lang:t("error.event_trigger_wrong", {event = "bankrobbery:server:receiveItem", extraInfo = " (pacific) ", source = source}))
        end
        local itemType = math.random(#Config.RewardTypes)
        ...
        if WeaponChance ~= odd1 or WeaponChance ~= odd2 then
            if tier ~= 4 then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items['cryptostick'], "add")
            end
        else
            ...
            if chance == odd then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items['weapon_microsmg'], "add")
            else
                ply.Functions.AddItem('weapon_minismg', 1)
                TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items['weapon_minismg'], "add")
            end
        end
    end
end)

AddEventHandler('bankrobbery:server:setTimeout', function()
    ...
    CreateThread(function()
        SetTimeout(60000 * 90, function()
            ...
            TriggerClientEvent('bankrobbery:client:ClearTimeoutDoors', -1)
...
AddEventHandler('bankrobbery:server:SetSmallBankTimeout', function(bankId)
    if robberyBusy or timeOut then return end
    timeOut = true
    CreateThread(function()
        SetTimeout(60000 * 30, function()
            ...
            TriggerClientEvent('bankrobbery:client:ResetFleecaLockers', -1, bankId)
            timeOut = false
            robberyBusy = false
...
RegisterNetEvent('bankrobbery:server:callCops', function(type, bank, coords)
    if type == "small" then
        if not Config.SmallBanks[bank]["alarm"] then
            return error(Lang:t("error.event_trigger_wrong", {event = "bankrobbery:server:callCops", extraInfo = " (smallbank "..bank..") ", source = source}))
        end
    elseif type == "paleto" then
        if not Config.BigBanks["paleto"]["alarm"] then
            return error(Lang:t("error.event_trigger_wrong", {event = "bankrobbery:server:callCops", extraInfo = " (paleto) ", source = source}))
        end
    elseif type == "pacific" then
        if not Config.BigBanks["pacific"]["alarm"] then
            return error(Lang:t("error.event_trigger_wrong", {event = "bankrobbery:server:callCops", extraInfo = " (pacific) ", source = source}))
        end
    end
    TriggerClientEvent("bankrobbery:client:robberyCall", -1, type, coords)
end)

RegisterNetEvent('bankrobbery:server:SetStationStatus', function(key, isHit)
    Config.PowerStations[key].hit = isHit
    TriggerClientEvent("bankrobbery:client:SetStationStatus", -1, key, isHit)
    if AllStationsHit() then
        exports["weathersync"]:setBlackout(true)
        TriggerClientEvent("bankrobbery:client:disableAllBankSecurity", -1)
        Config.OnBlackout(true)
        CreateThread(function()
            SetTimeout(60000 * Config.BlackoutTimer, function()
                exports["weathersync"]:setBlackout(false)
                TriggerClientEvent("bankrobbery:client:enableAllBankSecurity", -1)
...
RegisterNetEvent('bankrobbery:server:removeElectronicKit', function()
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if not Player then return end
    Player.Functions.RemoveItem('electronickit', 1)
    TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items["electronickit"], "remove")
    Player.Functions.RemoveItem('trojan_usb', 1)
    TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items["trojan_usb"], "remove")
end)

RegisterNetEvent('bankrobbery:server:removeBankCard', function(number)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if not Player then return end
    Player.Functions.RemoveItem('security_card_'..number, 1)
    TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items['security_card_'..number], "remove")
end)
...
Core.Functions.CreateCallback('bankrobbery:server:isRobberyActive', function(_, cb)
    cb(robberyBusy)
end)

Core.Functions.CreateCallback('bankrobbery:server:GetConfig', function(_, cb)
    cb(Config.PowerStations, Config.BigBanks, Config.SmallBanks)
end)

Core.Functions.CreateCallback("thermite:server:check", function(source, cb)
    local Player = Core.Functions.GetPlayer(source)
    if not Player then return cb(false) end
    if Player.Functions.RemoveItem("thermite", 1) then
        TriggerClientEvent('inventory:client:ItemBox', source, Core.Shared.Items["thermite"], "remove")
        cb(true)
    else
        cb(false)
    end
...
Core.Functions.CreateUseableItem("thermite", function(source)
    local Player = Core.Functions.GetPlayer(source)
	...
	if Player.Functions.GetItemByName('lighter') then
        ...
        TriggerClientEvent('Core:Notify', source, Lang:t("error.missing_ignition_source"), "error")
    end
end)

Core.Functions.CreateUseableItem("security_card_01", function(source)
    local Player = Core.Functions.GetPlayer(source)
	if not Player or not Player.Functions.GetItemByName('security_card_01') then return end
    TriggerClientEvent("bankrobbery:UseBankcardA", source)
end)

Core.Functions.CreateUseableItem("security_card_02", function(source)
    local Player = Core.Functions.GetPlayer(source)
	if not Player or not Player.Functions.GetItemByName('security_card_02') then return end
    TriggerClientEvent("bankrobbery:UseBankcardB", source)
end)

Core.Functions.CreateUseableItem("electronickit", function(source)
    local Player = Core.Functions.GetPlayer(source)
