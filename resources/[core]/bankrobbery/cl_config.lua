...
Config.UseTarget = GetConvar('UseTarget', 'false') == 'true' -- Use target interactions (don't change this, go to your server.cfg and add `setr UseTarget true` to use this and just that from true to false or the other way around)

-- This is the handler for the cop count, you can change this to anything you want as this is by default the policejob event
...
function Config.DoorlockAction(doorId, setLocked)
    TriggerServerEvent('doorlock:server:updateState', doorId, setLocked, false, false, true, false, false)
end
...
function Config.OnHackDone(success, bank)
    ...
    TriggerServerEvent('bankrobbery:server:setBankState', bank)
end
...
function Config.OnEvidence(pos, chance)
    if math.random(1, 100) > chance or Core.Functions.IsWearingGloves() then return end
