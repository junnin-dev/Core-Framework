...
local function showEntranceHeaderMenu()
    ...
    if Core.Functions.GetPlayerData().job and Core.Functions.GetPlayerData().job.name == 'realestate' then
        isOwned = true
    end
    ...
    if not isOwned then
        ...
        if isOwned and HasHouseKey then
            headerMenu[#headerMenu+1] = {
                ...
                params = {
                    event = "houses:client:EnterHouse",
                    args = {}
                }
            }
            headerMenu[#headerMenu+1] = {
        ...
        elseif isOwned and not HasHouseKey then
            headerMenu[#headerMenu+1] = {
                ...
                params = {
                    event = "houses:client:RequestRing",
                    args = {}
                }
            }
            headerMenu[#headerMenu+1] = {
            ...
            if Core.Functions.GetPlayerData().job and Core.Functions.GetPlayerData().job.name == 'police' then
                headerMenu[#headerMenu+1] = {
...
local function showExitHeaderMenu()
    ...
    headerMenu[#headerMenu+1] = {
        ...
        params = {
            event = "houses:client:ExitOwnedHouse",
            args = {}
        }
    }
    if isOwned then
        headerMenu[#headerMenu+1] = {
            ...
            params = {
                event = "houses:client:FrontDoorCam",
                args = {}
            }
        }
        headerMenu[#headerMenu+1] = {
            ...
            params = {
                event = "houses:client:AnswerDoorbell",
                args = {}
            }
        }
    end
...
local function RegisterStashTarget()
    ...
    stashTargetBox:onPlayerInOut(function (isPointInside)
        if isPointInside and not entering and isOwned then
            exports['core']:DrawText(Lang:t("target.open_stash"), 'left')
        else
...
local function RegisterOutfitsTarget()
    ...
    outfitsTargetBox:onPlayerInOut(function (isPointInside)
        if isPointInside and not entering and isOwned then
            exports['core']:DrawText(Lang:t("target.outfits"), 'left')
        else
            exports['core']:HideText()
        end
...
local function RegisterCharactersTarget()
    ...
    charactersTargetBox:onPlayerInOut(function (isPointInside)
        if isPointInside and not entering and isOwned then
            exports['core']:DrawText(Lang:t("target.change_character"), 'left')
        else
            exports['core']:HideText()
        end
...
RegisterNetEvent('houses:client:setHouseConfig', function(houseConfig)
...
local function GetClosestPlayer()
    local closestPlayers = Core.Functions.GetPlayersFromCoords()
...
local function SetClosestHouse()
    ...
    if not IsInside then
        ...
        if ClosestHouse ~= nil and tonumber(dist) < 30 then
            Core.Functions.TriggerCallback('houses:server:ProximityKO', function(key, owned)
                HasHouseKey = key
                isOwned = owned
            end, ClosestHouse)
        end
    end
    TriggerEvent('garages:client:setHouseGarage', ClosestHouse, HasHouseKey)
end

local function setHouseLocations()
    if ClosestHouse ~= nil then
        Core.Functions.TriggerCallback('houses:server:getHouseLocations', function(result)
            if result ~= nil then
...
local function LoadDecorations(house)
	if Config.Houses[house].decorations == nil or next(Config.Houses[house].decorations) == nil then
		Core.Functions.TriggerCallback('houses:server:getHouseDecorations', function(result)
...
local function RemoveHouseKey(citizenData)
    TriggerServerEvent('houses:server:removeHouseKey', ClosestHouse, citizenData)
    CloseMenuFull()
end
...
local function getKeyHolders()
    ...
    local p = promise.new()
    Core.Functions.TriggerCallback('houses:server:getHouseKeyHolders', function(holders)
        p:resolve(holders)
    end,ClosestHouse)
...
function HouseKeysMenu()
    ...
    if holders == nil or next(holders) == nil then
        Core.Functions.Notify(Lang:t("error.no_key_holders"), "error", 3500)
        CloseMenuFull()
    else
        keyholderMenu = {}
        ...
        for k, _ in pairs(holders) do
            keyholderMenu[#keyholderMenu+1] = {
                ...
                params = {
                    event = "houses:client:OpenClientOptions",
                    args = {
...
local function getDataForHouseTier(house, coords)
    if Config.Houses[house].tier == 1 then
        return exports['interior']:CreateApartmentShell(coords)
    elseif Config.Houses[house].tier == 2 then
        return exports['interior']:CreateTier1House(coords)
    elseif Config.Houses[house].tier == 3 then
        return exports['interior']:CreateTrevorsShell(coords)
    elseif Config.Houses[house].tier == 4 then
        return exports['interior']:CreateCaravanShell(coords)
    elseif Config.Houses[house].tier == 5 then
        return exports['interior']:CreateLesterShell(coords)
    elseif Config.Houses[house].tier == 6 then
        return exports['interior']:CreateRanchShell(coords)
    elseif Config.Houses[house].tier == 7 then
        return exports['interior']:CreateContainer(coords)
    elseif Config.Houses[house].tier == 8 then
        return exports['interior']:CreateFurniMid(coords)
    elseif Config.Houses[house].tier == 9 then
        return exports['interior']:CreateFurniMotelModern(coords)
    elseif Config.Houses[house].tier == 10 then
        return exports['interior']:CreateFranklinAunt(coords)
    elseif Config.Houses[house].tier == 11 then
        return exports['interior']:CreateGarageMed(coords)
    elseif Config.Houses[house].tier == 12 then
        return exports['interior']:CreateMichael(coords)
    elseif Config.Houses[house].tier == 13 then
        return exports['interior']:CreateOffice1(coords)
    elseif Config.Houses[house].tier == 14 then
        return exports['interior']:CreateStore1(coords)
    elseif Config.Houses[house].tier == 15 then
        return exports['interior']:CreateWarehouse1(coords)
    elseif Config.Houses[house].tier == 16 then
        return exports['interior']:CreateFurniMotelStandard(coords) -- End of free shells
    elseif Config.Houses[house].tier == 17 then
        return exports['interior']:CreateMedium2(coords)
    elseif Config.Houses[house].tier == 18 then
        return exports['interior']:CreateMedium3(coords)
    elseif Config.Houses[house].tier == 19 then
        return exports['interior']:CreateBanham(coords)
    elseif Config.Houses[house].tier == 20 then
        return exports['interior']:CreateWestons(coords)
    elseif Config.Houses[house].tier == 21 then
        return exports['interior']:CreateWestons2(coords)
    elseif Config.Houses[house].tier == 22 then
        return exports['interior']:CreateClassicHouse(coords)
    elseif Config.Houses[house].tier == 23 then
        return exports['interior']:CreateClassicHouse2(coords)
    elseif Config.Houses[house].tier == 24 then
        return exports['interior']:CreateClassicHouse3(coords)
    elseif Config.Houses[house].tier == 25 then
        return exports['interior']:CreateHighend1(coords)
    elseif Config.Houses[house].tier == 26 then
        return exports['interior']:CreateHighend2(coords)
    elseif Config.Houses[house].tier == 27 then
        return exports['interior']:CreateHighend3(coords)
    elseif Config.Houses[house].tier == 28 then
        return exports['interior']:CreateHighend(coords)
    elseif Config.Houses[house].tier == 29 then
        return exports['interior']:CreateHighendV2(coords)
    elseif Config.Houses[house].tier == 30 then
        return exports['interior']:CreateStashHouse(coords)
    elseif Config.Houses[house].tier == 31 then
        return exports['interior']:CreateStashHouse2(coords)
    elseif Config.Houses[house].tier == 32 then
        return exports['interior']:CreateGarageLow(coords)
    elseif Config.Houses[house].tier == 33 then
        return exports['interior']:CreateGarageHigh(coords)
    elseif Config.Houses[house].tier == 34 then
        return exports['interior']:CreateOffice2(coords)
    elseif Config.Houses[house].tier == 35 then
        return exports['interior']:CreateOfficeBig(coords)
    elseif Config.Houses[house].tier == 36 then
        return exports['interior']:CreateBarber(coords)
    elseif Config.Houses[house].tier == 37 then
        return exports['interior']:CreateGunstore(coords)
    elseif Config.Houses[house].tier == 38 then
        return exports['interior']:CreateStore2(coords)
    elseif Config.Houses[house].tier == 39 then
        return exports['interior']:CreateStore3(coords)
    elseif Config.Houses[house].tier == 40 then
        return exports['interior']:CreateWarehouse2(coords)
    elseif Config.Houses[house].tier == 41 then
        return exports['interior']:CreateWarehouse3(coords)
    elseif Config.Houses[house].tier == 42 then
        return exports['interior']:CreateK4Coke(coords)
    elseif Config.Houses[house].tier == 43 then
        return exports['interior']:CreateK4Meth(coords)
    elseif Config.Houses[house].tier == 44 then
        return exports['interior']:CreateK4Weed(coords)
    elseif Config.Houses[house].tier == 45 then
        return exports['interior']:CreateContainer2(coords)
    elseif Config.Houses[house].tier == 46 then
        return exports['interior']:CreateFurniStash1(coords)
    elseif Config.Houses[house].tier == 47 then
        return exports['interior']:CreateFurniStash3(coords)
    elseif Config.Houses[house].tier == 48 then
        return exports['interior']:CreateFurniLow(coords)
    elseif Config.Houses[house].tier == 49 then
        return exports['interior']:CreateFurniMotel(coords)
    elseif Config.Houses[house].tier == 50 then
        return exports['interior']:CreateFurniMotelClassic(coords)
    elseif Config.Houses[house].tier == 51 then
        return exports['interior']:CreateFurniMotelHigh(coords)
    elseif Config.Houses[house].tier == 52 then
        return exports['interior']:CreateFurniMotelModern2(coords)
    elseif Config.Houses[house].tier == 53 then
        return exports['interior']:CreateFurniMotelModern3(coords)
    elseif Config.Houses[house].tier == 54 then
        return exports['interior']:CreateCoke(coords)
    elseif Config.Houses[house].tier == 55 then
        return exports['interior']:CreateCoke2(coords)
    elseif Config.Houses[house].tier == 56 then
        return exports['interior']:CreateMeth(coords)
    elseif Config.Houses[house].tier == 57 then
        return exports['interior']:CreateWeed(coords)
    elseif Config.Houses[house].tier == 58 then
        return exports['interior']:CreateWeed2(coords)
    elseif Config.Houses[house].tier == 59 then
        return exports['interior']:CreateMansion(coords)
    elseif Config.Houses[house].tier == 60 then
        return exports['interior']:CreateMansion2(coords)
    elseif Config.Houses[house].tier == 61 then
        return exports['interior']:CreateMansion3(coords)
    elseif Config.Houses[house].tier == 62 then
        return exports['interior']:CreateHotel1(coords)
    elseif Config.Houses[house].tier == 63 then
        return exports['interior']:CreateHotel2(coords)
    elseif Config.Houses[house].tier == 64 then
        return exports['interior']:CreateHotel3(coords)
    elseif Config.Houses[house].tier == 65 then
        return exports['interior']:CreateMotel1(coords)
    elseif Config.Houses[house].tier == 66 then
        return exports['interior']:CreateMotel2(coords)
    elseif Config.Houses[house].tier == 67 then
        return exports['interior']:CreateMotel3(coords)
    elseif Config.Houses[house].tier == 68 then
        return exports['interior']:CreateV2Default1(coords)
    elseif Config.Houses[house].tier == 69 then
        return exports['interior']:CreateV2Default2(coords)
    elseif Config.Houses[house].tier == 70 then
        return exports['interior']:CreateV2Default3(coords)
    elseif Config.Houses[house].tier == 71 then
        return exports['interior']:CreateV2Default4(coords)
    elseif Config.Houses[house].tier == 72 then
        return exports['interior']:CreateV2Default5(coords)
    elseif Config.Houses[house].tier == 73 then
        return exports['interior']:CreateV2Default6(coords)
    elseif Config.Houses[house].tier == 74 then
        return exports['interior']:CreateV2Deluxe1(coords)
    elseif Config.Houses[house].tier == 75 then
        return exports['interior']:CreateV2Deluxe2(coords)
    elseif Config.Houses[house].tier == 76 then
        return exports['interior']:CreateV2Deluxe3(coords)
    elseif Config.Houses[house].tier == 77 then
        return exports['interior']:CreateV2HighEnd1(coords)
    elseif Config.Houses[house].tier == 78 then
        return exports['interior']:CreateV2HighEnd2(coords)
    elseif Config.Houses[house].tier == 79 then
        return exports['interior']:CreateV2HighEnd3(coords)
    elseif Config.Houses[house].tier == 80 then
        return exports['interior']:CreateV2Medium1(coords)
    elseif Config.Houses[house].tier == 81 then
        return exports['interior']:CreateV2Medium2(coords)
    elseif Config.Houses[house].tier == 82 then
        return exports['interior']:CreateV2Medium3(coords)
    elseif Config.Houses[house].tier == 83 then
        return exports['interior']:CreateV2Modern1(coords)
    elseif Config.Houses[house].tier == 84 then
        return exports['interior']:CreateV2Modern2(coords)
    elseif Config.Houses[house].tier == 85 then
        return exports['interior']:CreateV2Modern3(coords)
    else
        Core.Functions.Notify(Lang:t("error.invalid_tier"), 'error')
    end
end
...
local function enterOwnedHouse(house)
    ...
    entering = true
    Wait(500)
    TriggerServerEvent('houses:server:SetInsideMeta', house, true)
...
local function LeaveHouse(house)
    if not FrontCam then
        ...
        exports['interior']:DespawnInterior(houseObj, function()
            ...
            TriggerServerEvent('houses:server:SetInsideMeta', house, false)
            CurrentHouse = nil
...
RegisterNetEvent('houses:server:sethousedecorations', function(house, decorations)
...
RegisterNetEvent('houses:client:sellHouse', function()
    if ClosestHouse and HasHouseKey then
        TriggerServerEvent('houses:server:viewHouse', ClosestHouse)
    end
end)

RegisterNetEvent('houses:client:EnterHouse', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
...
RegisterNetEvent('houses:client:RequestRing', function()
    if ClosestHouse ~= nil then
        TriggerServerEvent('houses:server:RingDoor', ClosestHouse)
    end
end)

AddEventHandler('Core:Client:OnPlayerLoaded', function()
    TriggerServerEvent('houses:server:setHouses')
    SetClosestHouse()
    TriggerEvent('houses:client:setupHouseBlips')
    if Config.UnownedBlips then TriggerEvent('houses:client:setupHouseBlips2') end
    Wait(100)
    TriggerEvent('garages:client:setHouseGarage', ClosestHouse, HasHouseKey)
    TriggerServerEvent("houses:server:setHouses")
end)
...
RegisterNetEvent('houses:client:lockHouse', function(bool, house)
    Config.Houses[house].locked = bool
end)

RegisterNetEvent('houses:client:createHouses', function(price, tier)
    ...
    street = street:gsub("%-", " ")
    TriggerServerEvent('houses:server:addNewHouse', street, coords, price, tier)
    if Config.UnownedBlips then TriggerServerEvent('houses:server:createBlip') end
end)

RegisterNetEvent('houses:client:addGarage', function()
    if ClosestHouse ~= nil then
        local pos = GetEntityCoords(PlayerPedId())
        local heading = GetEntityHeading(PlayerPedId())
        local coords = {
        ...
        TriggerServerEvent('houses:server:addGarage', ClosestHouse, coords)
    else
        Core.Functions.Notify(Lang:t("error.no_house"), "error")
    end
end)

RegisterNetEvent('houses:client:toggleDoorlock', function()
    local pos = GetEntityCoords(PlayerPedId())
    local dist = #(pos - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
    if dist <= 1.5 then
        if HasHouseKey then
            if Config.Houses[ClosestHouse].locked then
                TriggerServerEvent('houses:server:lockHouse', false, ClosestHouse)
                Core.Functions.Notify(Lang:t("success.unlocked"), "success", 2500)
            else
                TriggerServerEvent('houses:server:lockHouse', true, ClosestHouse)
                Core.Functions.Notify(Lang:t("error.locked"), "error", 2500)
            end
        else
            Core.Functions.Notify(Lang:t("error.no_keys"), "error", 3500)
        end
    else
        Core.Functions.Notify(Lang:t("error.no_door"), "error", 3500)
    end
end)

RegisterNetEvent('houses:client:RingDoor', function(player, house)
    if ClosestHouse == house and IsInside then
        CurrentDoorBell = player
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "doorbell", 0.1)
        Core.Functions.Notify(Lang:t("info.door_ringing"))
    end
end)

RegisterNetEvent('houses:client:giveHouseKey', function()
    ...
    if player ~= -1 and distance < 2.5 and ClosestHouse ~= nil then
        ...
        if housedist < 10 then
            TriggerServerEvent('houses:server:giveHouseKey', playerId, ClosestHouse)
        else
            Core.Functions.Notify(Lang:t("error.no_door"), "error")
        end
    elseif ClosestHouse == nil then
        Core.Functions.Notify(Lang:t("error.no_house"), "error")
    else
        Core.Functions.Notify(Lang:t("error.no_one_near"), "error")
    end
end)

RegisterNetEvent('houses:client:removeHouseKey', function()
    if ClosestHouse ~= nil then
        local pedpos = GetEntityCoords(PlayerPedId())
        local housedist = #(pedpos - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
        if housedist <= 5 then
            Core.Functions.TriggerCallback('houses:server:getHouseOwner', function(result)
                if Core.Functions.GetPlayerData().citizenid == result then
                    HouseKeysMenu()
                else
                    Core.Functions.Notify(Lang:t("error.not_owner"), "error")
                end
            end, ClosestHouse)
        else
            Core.Functions.Notify(Lang:t("error.no_door"), "error")
...
RegisterNetEvent('houses:client:RevokeKey', function(cData)
    RemoveHouseKey(cData.citizenData)
end)

RegisterNetEvent('houses:client:refreshHouse', function()
    Wait(100)
    SetClosestHouse()
end)

RegisterNetEvent('houses:client:SpawnInApartment', function(house)
    local pos = GetEntityCoords(PlayerPedId())
...
RegisterNetEvent('houses:client:enterOwnedHouse', function(house)
...
RegisterNetEvent('houses:client:LastLocationHouse', function(houseId)
    Core.Functions.GetPlayerData(function(PlayerData)
		if PlayerData.metadata["injail"] == 0 then
...
RegisterNetEvent('houses:client:setupHouseBlips', function() -- Setup owned on load
    CreateThread(function()
        Wait(2000)
        if LocalPlayer.state['isLoggedIn'] then
            Core.Functions.TriggerCallback('houses:server:getOwnedHouses', function(ownedHouses)
                if ownedHouses then
...
RegisterNetEvent('houses:client:setupHouseBlips2', function() -- Setup unowned on load
...
RegisterNetEvent('houses:client:createBlip', function(coords) -- Create unowned on command
...
RegisterNetEvent('houses:client:refreshBlips', function() -- Refresh unowned on buy
    for _, v in pairs(UnownedHouseBlips) do RemoveBlip(v) end
    Wait(250)
    TriggerEvent('houses:client:setupHouseBlips2')
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterNetEvent('houses:client:SetClosestHouse', function()
    SetClosestHouse()
end)

RegisterNetEvent('houses:client:viewHouse', function(houseprice, brokerfee, bankfee, taxes, firstname, lastname)
...
RegisterNetEvent('houses:client:setLocation', function(cData)
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    ...
    if IsInside then
        if HasHouseKey then
            if cData.id == 'setstash' then
                TriggerServerEvent('houses:server:setLocation', coords, ClosestHouse, 1)
            elseif cData.id == 'setoutift' then
                TriggerServerEvent('houses:server:setLocation', coords, ClosestHouse, 2)
            elseif cData.id == 'setlogout' then
                TriggerServerEvent('houses:server:setLocation', coords, ClosestHouse, 3)
            end
        else
            Core.Functions.Notify(Lang:t("error.not_owner"), "error")
        end
    else
        Core.Functions.Notify(Lang:t("error.not_in_house"), "error")
    end
end)

RegisterNetEvent('houses:client:refreshLocations', function(house, location, type)
...
RegisterNetEvent('houses:client:HomeInvasion', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    local Skillbar = exports['skillbar']:GetSkillbarObject()
    if ClosestHouse ~= nil then
        Core.Functions.TriggerCallback('police:server:IsPoliceForcePresent', function(IsPresent)
            if IsPresent then
                local dist = #(pos - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
                        ...
                        if not Config.Houses[ClosestHouse].IsRaming then
                            ...
                            Skillbar.Start({
                                ...
                                if RamsDone + 1 >= Config.RamsNeeded then
                                    TriggerServerEvent('houses:server:lockHouse', false, ClosestHouse)
                                    Core.Functions.Notify(Lang:t("success.home_invasion"), 'success')
                                    TriggerServerEvent('houses:server:SetHouseRammed', true, ClosestHouse)
                                    TriggerServerEvent('houses:server:SetRamState', false, ClosestHouse)
                                ...
                                RamsDone = 0
                                TriggerServerEvent('houses:server:SetRamState', false, ClosestHouse)
                                Core.Functions.Notify(Lang:t("error.failed_invasion"), 'error')
                                DoRamAnimation(false)
                            end)
                            TriggerServerEvent('houses:server:SetRamState', true, ClosestHouse)
                        else
                            Core.Functions.Notify(Lang:t("error.inprogress_invasion"), 'error')
                        end
                    else
                        Core.Functions.Notify(Lang:t("error.already_open"), 'error')
                    end
                else
                    Core.Functions.Notify(Lang:t("error.no_house"), "error")
                end
            else
                Core.Functions.Notify(Lang:t("error.no_police"), 'error')
            end
        end)
    else
        Core.Functions.Notify(Lang:t("error.no_house"), "error")
...
RegisterNetEvent('houses:client:SetRamState', function(bool, house)
    Config.Houses[house].IsRaming = bool
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterNetEvent('houses:client:SetHouseRammed', function(bool, house)
    Config.Houses[house].IsRammed = bool
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterNetEvent('houses:client:ResetHouse', function()
    if ClosestHouse ~= nil then
        if Config.Houses[ClosestHouse].IsRammed == nil then
            Config.Houses[ClosestHouse].IsRammed = false
            TriggerServerEvent('houses:server:SetHouseRammed', false, ClosestHouse)
            TriggerServerEvent('houses:server:SetRamState', false, ClosestHouse)
        end
        if Config.Houses[ClosestHouse].IsRammed then
            ...
            TriggerServerEvent('houses:server:lockHouse', true, ClosestHouse)
            RamsDone = 0
            Core.Functions.Notify(Lang:t("success.lock_invasion"), 'success')
        else
            Core.Functions.Notify(Lang:t("error.no_invasion"), 'error')
        end
    end
end)

RegisterNetEvent('houses:client:ExitOwnedHouse', function()
...
RegisterNetEvent('houses:client:FrontDoorCam', function()
    local door = vector3(Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z)
    if CheckDistance(door, 1.5) then
...
RegisterNetEvent('houses:client:AnswerDoorbell', function()
    if not CurrentDoorBell or CurrentDoorBell == 0 then
        Core.Functions.Notify(Lang:t('error.nobody_at_door'))
        return
    end
    local door = vector3(Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z)
    if CheckDistance(door, 1.5) and CurrentDoorBell ~= 0 then
        TriggerServerEvent("houses:server:OpenDoor", CurrentDoorBell, ClosestHouse)
        CurrentDoorBell = 0
    end
end)

RegisterNetEvent('houses:client:OpenStash', function()
...
RegisterNetEvent('houses:client:ChangeCharacter', function()
    ...
    if CheckDistance(stashLoc, 1.5) then
        ...
        exports['interior']:DespawnInterior(houseObj, function()
            ...
            InOwnedHouse = false
            IsInside = false
            TriggerServerEvent('houses:server:LogoutLocation')
        end)
    end
end)

RegisterNetEvent('houses:client:ChangeOutfit', function()
...
RegisterNetEvent('houses:client:ViewHouse', function()
...
RegisterNetEvent('houses:client:KeyholderOptions', function(cData)
    optionMenu(cData.citizenData)
end)

RegisterNetEvent('house:client:RefreshHouseTargets', function ()
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)
...
RegisterNUICallback('HasEnoughMoney', function(cData, cb)
    Core.Functions.TriggerCallback('houses:server:HasEnoughMoney', function(_)
        cb('ok')
...
RegisterNUICallback('buy', function(_, cb)
    ...
    if Config.UnownedBlips then TriggerEvent('houses:client:refreshBlips') end
    TriggerServerEvent('houses:server:buyHouse', ClosestHouse)
    cb("ok")
end)
        ...
        if IsInside then
            ...
            if isInsideStashTarget then
                ...
                if IsControlJustPressed(0, 38) then
                    TriggerEvent('houses:client:OpenStash')
                    exports['core']:HideText()
                end
            end
            ...
            if isInsideOutfitsTarget then
                wait = 0
                if IsControlJustPressed(0, 38) then
                    TriggerEvent('houses:client:ChangeOutfit')
                    exports['core']:HideText()
                end
            end
            ...
            if isInsiteCharactersTarget then
                wait = 0
                if IsControlJustPressed(0, 38) then
                    TriggerEvent('houses:client:ChangeCharacter')
                    exports['core']:HideText()
                end
            end
        end
