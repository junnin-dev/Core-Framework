...
local function EnableEditMode()
	...
	CreateEditCamera()
	DecoMode = true
	TriggerEvent('anticheat:client:ToggleDecorate', true)
end
...
local function SaveDecorations()
	if ClosestHouse then
		...
		TriggerServerEvent("houses:server:savedecorations", ClosestHouse, ObjectList)
	end
end
...
local function DisableEditMode()
	...
	peanut = false
	DecoMode = false
	TriggerEvent('anticheat:client:ToggleDecorate', false)
end
...
local function CheckMovementInput()
	...
	if IsControlJustReleased(0, 21) then -- Left Shift
		...
		Core.Functions.Notify(Lang:t("info.speed").. tostring(speeds[curSpeed]))
	end
...
RegisterNetEvent('houses:client:decorate', function()
	Wait(500)
	if IsInside then
		if HasHouseKey then
			...
			Core.Functions.Notify(Lang:t("error.no_keys"), "error")
		end
	else
...
RegisterNUICallback("buySelectedObject", function(data, cb)
    Core.Functions.TriggerCallback('houses:server:buyFurniture', function(isSuccess)
        if isSuccess then
            SetNuiFocus(false, false)
            cursorEnabled = false
            SaveDecorations()
	...
	while true do
		...
		if DecoMode then
			...
			if dist > 50.0 then
				DisableEditMode()
				closeDecorateUI()
				Core.Functions.Notify(Lang:t("error.out_range"), 'error')
			end
		end
	end
end)
