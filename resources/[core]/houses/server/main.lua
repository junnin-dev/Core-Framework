...
CreateThread(function()
    ...
    TriggerClientEvent("houses:client:setHouseConfig", -1, Config.Houses)
end)
...
Core.Commands.Add("decorate", Lang:t("info.decorate_interior"), {}, false, function(source)
    local src = source
    TriggerClientEvent("houses:client:decorate", src)
end)

Core.Commands.Add("createhouse", Lang:t("info.create_house"), {{name = "price", help = Lang:t("info.price_of_house")}, {name = "tier", help = Lang:t("info.tier_number")}}, true, function(source, args)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    local price = tonumber(args[1])
    local tier = tonumber(args[2])
    if Player.PlayerData.job.name == "realestate" then
        TriggerClientEvent("houses:client:createHouses", src, price, tier)
    else
        TriggerClientEvent('Core:Notify', src, Lang:t("error.realestate_only"), "error")
    end
end)

Core.Commands.Add("addgarage", Lang:t('info.add_garage'), {}, false, function(source)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "realestate" then
        TriggerClientEvent("houses:client:addGarage", src)
    else
        TriggerClientEvent('Core:Notify', src, Lang:t("error.realestate_only"), "error")
    end
end)

Core.Commands.Add("ring", Lang:t("info.ring_doorbell"), {}, false, function(source)
    local src = source
    TriggerClientEvent('houses:client:RequestRing', src)
end)

-- Item

Core.Functions.CreateUseableItem("police_stormram", function(source, _)
    local Player = Core.Functions.GetPlayer(source)
    if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
        TriggerClientEvent("houses:client:HomeInvasion", source)
    else
        TriggerClientEvent('Core:Notify', source, Lang:t("error.emergency_services"), "error")
    end
end)
...
RegisterNetEvent('houses:server:setHouses', function()
    local src = source
    TriggerClientEvent("houses:client:setHouseConfig", src, Config.Houses)
end)

RegisterNetEvent('houses:server:createBlip', function()
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    TriggerClientEvent("houses:client:createBlip", -1, coords)
end)

RegisterNetEvent('houses:server:addNewHouse', function(street, coords, price, tier)
    local src = source
    street = street:gsub("%'", "")
    ...
    TriggerClientEvent("houses:client:setHouseConfig", -1, Config.Houses)
    TriggerClientEvent('Core:Notify', src, Lang:t("info.added_house", {value = label}))
    TriggerEvent('log:server:CreateLog', 'house', Lang:t("log.house_created"), 'green', Lang:t("log.house_address", {label = label, price = price, tier = tier, agent = GetPlayerName(src)}))
end)

RegisterNetEvent('houses:server:addGarage', function(house, coords)
    local src = source
    ...
    TriggerClientEvent("garages:client:addHouseGarage", -1, house, garageInfo)
    TriggerClientEvent('Core:Notify', src, Lang:t("info.added_garage", {value = garageInfo.label}))
end)

RegisterNetEvent('houses:server:viewHouse', function(house)
    local src = source
    ...
    TriggerClientEvent('houses:client:viewHouse', src, houseprice, brokerfee, bankfee, taxes,
        pData.PlayerData.charinfo.firstname, pData.PlayerData.charinfo.lastname)
end)

RegisterNetEvent('houses:server:buyHouse', function(house)
    local src = source
    local pData = Core.Functions.GetPlayer(src)
    ...
    if isOwned then
        TriggerClientEvent('Core:Notify', src, Lang:t("error.already_owned"), "error")
        CancelEvent()
        return
    end
    ...
    if (bankBalance >= HousePrice) then
        ...
        TriggerClientEvent('houses:client:SetClosestHouse', src)
        TriggerClientEvent('house:client:RefreshHouseTargets', src)
        pData.Functions.RemoveMoney('bank', HousePrice, "bought-house") -- 21% Extra house costs
        exports['management']:AddMoney("realestate", (HousePrice / 100) * math.random(18, 25))
        TriggerEvent('log:server:CreateLog', 'house', Lang:t("log.house_purchased"), 'green', Lang:t("log.house_purchased_by", {house = house:upper(), price = HousePrice, firstname = pData.PlayerData.charinfo.firstname, lastname = pData.PlayerData.charinfo.lastname}))
        TriggerClientEvent('Core:Notify', src, Lang:t("success.house_purchased"), 'success', 5000)
    else
        TriggerClientEvent('Core:Notify', src, Lang:t("error.not_enough_money"), "error")
    end
end)

RegisterNetEvent('houses:server:lockHouse', function(bool, house)
    TriggerClientEvent('houses:client:lockHouse', -1, bool, house)
end)

RegisterNetEvent('houses:server:SetRamState', function(bool, house)
    Config.Houses[house].IsRaming = bool
    TriggerClientEvent('houses:server:SetRamState', -1, bool, house)
end)

RegisterNetEvent('houses:server:giveKey', function(house, target)
    local pData = Core.Functions.GetPlayer(target)
...
RegisterNetEvent('houses:server:removeHouseKey', function(house, citizenData)
    local src = source
    local newHolders = {}
    if housekeyholders[house] then
    ...
    TriggerClientEvent('Core:Notify', src, Lang:t("error.remove_key_from", {firstname = citizenData.firstname, lastname = citizenData.lastname}), 'error')
    MySQL.update('UPDATE player_houses SET keyholders = ? WHERE house = ?', {json.encode(housekeyholders[house]), house})
end)

RegisterNetEvent('houses:server:OpenDoor', function(target, house)
    local OtherPlayer = Core.Functions.GetPlayer(target)
    if OtherPlayer then
        TriggerClientEvent('houses:client:SpawnInApartment', OtherPlayer.PlayerData.source, house)
    end
end)

RegisterNetEvent('houses:server:RingDoor', function(house)
    local src = source
    TriggerClientEvent('houses:client:RingDoor', -1, src, house)
end)

RegisterNetEvent('houses:server:savedecorations', function(house, decorations)
    MySQL.update('UPDATE player_houses SET decorations = ? WHERE house = ?', {json.encode(decorations), house})
    TriggerClientEvent("houses:server:sethousedecorations", -1, house, decorations)
end)

RegisterNetEvent('houses:server:LogoutLocation', function()
    local src = source
    local Player = Core.Functions.GetPlayer(src)
...
RegisterNetEvent('houses:server:giveHouseKey', function(target, house)
    local src = source
    local tPlayer = Core.Functions.GetPlayer(target)
    if tPlayer then
        if housekeyholders[house] then
            for _, cid in pairs(housekeyholders[house]) do
                if cid == tPlayer.PlayerData.citizenid then
                    TriggerClientEvent('Core:Notify', src, Lang:t("error.already_keys"), 'error', 3500)
                    return
                end
            end
            housekeyholders[house][#housekeyholders[house]+1] = tPlayer.PlayerData.citizenid
            MySQL.update('UPDATE player_houses SET keyholders = ? WHERE house = ?', {json.encode(housekeyholders[house]), house})
            TriggerClientEvent('houses:client:refreshHouse', tPlayer.PlayerData.source)

            TriggerClientEvent('Core:Notify', tPlayer.PlayerData.source, Lang:t("success.recieved_key", {value = Config.Houses[house].adress}), 'success', 2500)
        else
            local sourceTarget = Core.Functions.GetPlayer(src)
            housekeyholders[house] = {
        ...
        TriggerClientEvent('Core:Notify', src, Lang:t("error.something_wrong"), 'error', 2500)
    end
end)

RegisterNetEvent('houses:server:setLocation', function(coords, house, type)
    if type == 1 then
    ...
    TriggerClientEvent('houses:client:refreshLocations', -1, house, json.encode(coords), type)
end)

RegisterNetEvent('houses:server:SetHouseRammed', function(bool, house)
    Config.Houses[house].IsRammed = bool
    TriggerClientEvent('houses:client:SetHouseRammed', -1, bool, house)
end)

RegisterNetEvent('houses:server:SetInsideMeta', function(insideId, bool)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
...
Core.Functions.CreateCallback('houses:server:buyFurniture', function(source, cb, price)
    local src = source
    local pData = Core.Functions.GetPlayer(src)
    local bankBalance = pData.PlayerData.money["bank"]
...
Core.Functions.CreateCallback('houses:server:ProximityKO', function(source, cb, house)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    local retvalK = false
    local retvalO
...
Core.Functions.CreateCallback('houses:server:hasKey', function(source, cb, house)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    local retval = false
    if Player then
...
Core.Functions.CreateCallback('houses:server:isOwned', function(source, cb, house)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
...
Core.Functions.CreateCallback('houses:server:getHouseOwner', function(_, cb, house)
    cb(houseownercid[house])
end)

Core.Functions.CreateCallback('houses:server:getHouseKeyHolders', function(source, cb, house)
    local retval = {}
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if housekeyholders[house] then
...
Core.Functions.CreateCallback('phoneserver:TransferCid', function(_, cb, NewCid, house)
...
Core.Functions.CreateCallback('houses:server:getHouseDecorations', function(_, cb, house)
    local retval = nil
...
Core.Functions.CreateCallback('houses:server:getHouseLocations', function(_, cb, house)
    local retval = nil
    local result = MySQL.query.await('SELECT * FROM player_houses WHERE house = ?', {house})
    if result[1] then
        retval = result[1]
...
Core.Functions.CreateCallback('houses:server:getOwnedHouses', function(source, cb)
    local src = source
    local pData = Core.Functions.GetPlayer(src)
    if pData then
...
Core.Functions.CreateCallback('houses:server:getSavedOutfits', function(source, cb)
    local src = source
    local pData = Core.Functions.GetPlayer(src)
...
Core.Functions.CreateCallback('phoneserver:GetPlayerHouses', function(source, cb)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    local MyHouses = {}
...
Core.Functions.CreateCallback('phoneserver:GetHouseKeys', function(source, cb)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    local MyKeys = {}
...
Core.Functions.CreateCallback('phoneserver:MeosGetPlayerHouses', function(_, cb, input)
    if input then
