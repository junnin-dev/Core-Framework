...
local function openMenu(data)
	...
	for _,v in pairs(data) do
		if v["icon"] then
			local img = "inventory/html/"
			if Core.Shared.Items[tostring(v["icon"])] then
				if not string.find(Core.Shared.Items[tostring(v["icon"])].image, "images/") then
					img = img.."images/"
				end
				v["icon"] = img..Core.Shared.Items[tostring(v["icon"])].image
			end
		end
	end
...
RegisterNetEvent('menu:client:openMenu', function(data)
    openMenu(data)
end)
    ...
    if sendData then
        ...
        if data then
            if data.params.event then
                ...
                elseif data.params.isJNCommand then
                    TriggerServerEvent('Core:CallCommand', data.params.event, data.params.args)
