...
function TakeOutVehicle(vehicleInfo)
    ...
    if coords then
		...
		if Config.VehicleSettings[vehicleInfo].livery ~= nil then
            ...
            TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
            TriggerEvent("vehiclekeys:client:SetOwner", Core.Functions.GetPlate(veh))
            TriggerServerEvent("inventory:server:addTrunkItems", Core.Functions.GetPlate(veh), Config.CarItems)
            SetVehicleEngineOn(veh, true, true)
...
local function IsArmoryWhitelist() -- being removed
    ...
    if Core.Functions.GetPlayerData().job.name == 'police' then
        retval = true
    end
    return retval
end
...
local function SetWeaponSeries()
    for k, _ in pairs(Config.Items.items) do
        if k < 6 then
            Config.Items.items[k].info.serie = tostring(Core.Shared.RandomInt(2) .. Core.Shared.RandomStr(3) .. Core.Shared.RandomInt(1) .. Core.Shared.RandomStr(2) .. Core.Shared.RandomInt(3) .. Core.Shared.RandomStr(4))
        end
    end
end
...
function MenuImpound(currentSelection)
    ...
    Core.Functions.TriggerCallback("police:GetImpoundedVehicles", function(result)
        local shouldContinue = false
        if result == nil then
            Core.Functions.Notify(Lang:t("error.no_impound"), "error", 5000)
        else
            shouldContinue = true
            for _ , v in pairs(result) do
                local enginePercent = Core.Shared.Round(v.engine / 10, 0)
        ...
        if shouldContinue then
            ...
            exports['menu']:openMenu(impoundMenu)
        end
    end)
...
RegisterNetEvent('police:client:ImpoundVehicle', function(fullImpound, price)
    ...
    if vehicle ~= 0 and vehicle then
        ...
        if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
           Core.Functions.Progressbar('impound', Lang:t('progressbar.impound'), 5000, false, true, {
                ...
                TriggerEvent('Core:Notify', Lang:t('success.impounded'), 'success')
                ClearPedTasks(ped)
            end, function() -- Play When Cancel
                ClearPedTasks(ped)
                TriggerEvent('Core:Notify', Lang:t('error.canceled'), 'error')
            end)
        end
    end
end)
    ...
    Core.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.name == "police" then
            ...
            if player ~= -1 and distance < 5.0 then
                ...
                Core.Functions.TriggerCallback('police:GetPlayerStatus', function(result)
                    if result then
                        for _, v in pairs(result) do
                            Core.Functions.Notify(''..v..'')
                        end
                    end
                end, playerId)
            else
                Core.Functions.Notify(Lang:t("error.none_nearby"), "error")
            end
        end
    end)
end)
...
RegisterNetEvent('police:client:EvidenceStashDrawer', function(data)
    ...
    if #(pos - takeLoc) <= 1.0 then
        local drawer = exports['input']:ShowInput({
...
RegisterNetEvent('policejob:ToggleDuty', function()
    onDuty = not onDuty
...
RegisterNetEvent('police:client:scanFingerPrint', function()
    local player, distance = GetClosestPlayer()
...
RegisterNetEvent('police:client:openArmoury', function()
...
RegisterNetEvent('police:client:spawnHelicopter', function(k)
...
if Config.UseTarget then
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["duty"]) do
            exports['target']:AddBoxZone("PoliceDuty_"..k, vector3(v.x, v.y, v.z), 1, 1, {
                name = "PoliceDuty_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
    ...
    dutyCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            dutylisten = true
            if not onDuty then
                exports['core']:DrawText(Lang:t('info.on_duty'),'left')
                dutylistener()
            else
                exports['core']:DrawText(Lang:t('info.off_duty'),'left')
                dutylistener()
            end
        else
            inDuty = false
...
CreateThread(function()
    ...
    stashCombo:onPlayerInOut(function(isPointInside, _, _)
        if isPointInside then
            inStash = true
            exports['core']:DrawText(Lang:t('info.stash_enter'), 'left')
            stash()
        else
            exports['core']:HideText()
            inStash = false
    ...
    trashCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inTrash = true
            if onDuty then
                exports['core']:DrawText(Lang:t('info.trash_enter'),'left')
                trash()
            end
        else
            inTrash = false
    ...
    fingerprintCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inFingerprint = true
            if onDuty then
                exports['core']:DrawText(Lang:t('info.scan_fingerprint'),'left')
                fingerprint()
            end
        else
            inFingerprint = false
    ...
    armouryCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inAmoury = true
            if onDuty then
                exports['core']:DrawText(Lang:t('info.enter_armory'),'left')
                armoury()
            end
        else
            inAmoury = false
    ...
    helicopterCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            ...
            if onDuty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['core']:HideText()
                    exports['core']:DrawText(Lang:t('info.store_heli'), 'left')
                    heli()
                else
                    exports['core']:DrawText(Lang:t('info.take_heli'), 'left')
                    heli()
                end
            end
        else
    ...
    impoundCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            ...
            if onDuty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['core']:DrawText(Lang:t('info.impound_veh'), 'left')
                    impound()
                else
    ...
    garageCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            ...
            if onDuty and PlayerJob.name == 'police' then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['core']:DrawText(Lang:t('info.store_veh'), 'left')
		    garage()
                else
                    local currentSelection = 0
    ...
    CreateThread(function()
        while true do
            ...
            if inStash and PlayerJob.name == "police" then
                ...
                if IsControlJustReleased(0, 38) then
                    TriggerServerEvent("inventory:server:OpenInventory", "stash", "policestash_"..Core.Functions.GetPlayerData().citizenid)
                    TriggerEvent("inventory:client:SetCurrentStash", "policestash_"..Core.Functions.GetPlayerData().citizenid)
                    break
                end
            else
                break
    ...
    CreateThread(function()
        while true do
            ...
            if inFingerprint and PlayerJob.name == "police" then
                if onDuty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("police:client:scanFingerPrint")
                    break
                end
            else
                break
    ...
    CreateThread(function()
        while true do
            ...
            if inAmoury and PlayerJob.name == "police" then
                if onDuty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("police:client:openArmoury")
                    break
                end
            else
                break
    ...
    CreateThread(function()
        while true do
            ...
            if inHelicopter and PlayerJob.name == "police" then
                if onDuty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("police:client:spawnHelicopter")
                    break
                end
            else
                break
