...
RegisterNetEvent('evidence:client:SetStatus', function(statusId, time)
    if time > 0 and StatusList[statusId] then
        if (CurrentStatusList == nil or CurrentStatusList[statusId] == nil) or
            ...
            Core.Functions.Notify(CurrentStatusList[statusId].text, 'error')
        end
...
RegisterNetEvent('evidence:client:ClearBlooddropsInArea', function()
    ...
    local blooddropList = {}
    Core.Functions.Progressbar('clear_blooddrops', Lang:t("progressbar.blood_clear"), 5000, false, true, {
        ...
        if Blooddrops and next(Blooddrops) then
            ...
            Core.Functions.Notify(Lang:t("success.blood_clear"), "success")
        end
    end, function() -- Cancel
        Core.Functions.Notify(Lang:t("error.blood_not_cleared"), "error")
    end)
end)
...
RegisterNetEvent('evidence:client:ClearCasingsInArea', function()
    local pos = GetEntityCoords(PlayerPedId())
    local casingList = {}
    Core.Functions.Progressbar('clear_casings', Lang:t("progressbar.bullet_casing"), 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true
        ...
        if Casings and next(Casings) then
            ...
            Core.Functions.Notify(Lang:t("success.bullet_casing_removed"), "success")
            
        end
    end, function() -- Cancel
        Core.Functions.Notify(Lang:t("error.bullet_casing_not_removed"), "error")
    end)
end)
            ...
            if #(pos -vector3(Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z)) < 1.5 then
                ...
                if IsControlJustReleased(0, 47) then
                    ...
                    local info = {
                        ...
                        ammolabel = Config.AmmoLabels[Core.Shared.Weapons[Casings[CurrentCasing].type]['ammotype']],
