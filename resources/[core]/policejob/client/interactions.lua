...
local function IsTargetDead(playerId)
    ...
    Core.Functions.TriggerCallback('police:server:isPlayerDead', function(result)
        retval = result
        hasReturned = true
    end, playerId)
...
RegisterNetEvent('police:client:SearchPlayer', function()
    local player, distance = Core.Functions.GetClosestPlayer()
...
RegisterNetEvent('police:client:RobPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if IsEntityPlayingAnim(playerPed, "missminuteman_1ig_2", "handsup_base", 3) or IsEntityPlayingAnim(playerPed, "mp_arresting", "idle", 3) or IsTargetDead(playerId) then
            Core.Functions.Progressbar("robbing_player", Lang:t("progressbar.robbing"), math.random(5000, 7000), false, true, {
...
RegisterNetEvent('police:client:JailPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if tonumber(dialog['jailtime']) > 0 then
            ...
            Core.Functions.Notify(Lang:t("error.time_higher"), "error")
        end
    else
        Core.Functions.Notify(Lang:t("error.none_nearby"), "error")
    end
...
RegisterNetEvent('police:client:BillPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if tonumber(dialog['bill']) > 0 then
            ...
            Core.Functions.Notify(Lang:t("error.amount_higher"), "error")
        end
    else
        Core.Functions.Notify(Lang:t("error.none_nearby"), "error")
    end
    ...
    if not IsPedRagdoll(PlayerPedId()) then
        ...
        if player ~= -1 and distance < 1.5 then
            ...
            if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(PlayerPedId()) then
                ...
                Core.Functions.Notify(Lang:t("error.vehicle_cuff"), "error")
            end
        else
            Core.Functions.Notify(Lang:t("error.none_nearby"), "error")
        end
        ...
        if player ~= -1 and distance < 1.5 then
            Core.Functions.TriggerCallback('Core:HasItem', function(result)
                if result then
                    ...
                    Core.Functions.Notify(Lang:t("error.no_cuff"), "error")
                end
...
RegisterNetEvent('police:client:GetKidnappedDragger', function()
    Core.Functions.GetPlayerData(function(_)
...
RegisterNetEvent('police:client:GetCuffed', function(playerId, isSoftcuff)
    ...
    if not isHandcuffed then
        ...
        if not isSoftcuff then
            ...
            Core.Functions.Notify(Lang:t("info.cuff"), 'primary')
        else
            cuffType = 49
            GetCuffedAnimation(playerId)
            Core.Functions.Notify(Lang:t("info.cuffed_walk"), 'primary')
        end
    else
        isHandcuffed = false
        isEscorted = false
        ...
        ClearPedTasksImmediately(ped)
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "Uncuff", 0.2)
        Core.Functions.Notify(Lang:t("success.uncuffed"),"success")
    end
end)
...
CreateThread(function()
    while true do
        ...
        if isHandcuffed then
            ...
            if (not IsEntityPlayingAnim(PlayerPedId(), "mp_arresting", "idle", 3) and not IsEntityPlayingAnim(PlayerPedId(), "mp_arrest_paired", "crook_p2_back_right", 3)) and not Core.Functions.GetPlayerData().metadata["isdead"] then
