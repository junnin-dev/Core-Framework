...
-- Commands
Core.Commands.Add("spikestrip", Lang:t("commands.place_spike"), {}, false, function(source)
    local src = source
...
Core.Commands.Add("grantlicense", Lang:t("commands.license_grant"), {{name = "id", help = Lang:t('info.player_id')}, {name = "license", help = Lang:t('info.license_type')}}, true, function(source, args)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.grade.level >= Config.LicenseRank then
        if args[2] == "driver" or args[2] == "weapon" then
            ...
            if licenseTable[args[2]] then
                TriggerClientEvent('Core:Notify', src, Lang:t("error.license_already"), "error")
                return
            end
            licenseTable[args[2]] = true
            SearchedPlayer.Functions.SetMetaData("licences", licenseTable)
            TriggerClientEvent('Core:Notify', SearchedPlayer.PlayerData.source, Lang:t("success.granted_license"), "success")
            TriggerClientEvent('Core:Notify', src, Lang:t("success.grant_license"), "success")
        else
            TriggerClientEvent('Core:Notify', src, Lang:t("error.error_license_type"), "error")
        end
    else
        TriggerClientEvent('Core:Notify', src, Lang:t("error.rank_license"), "error")
    end
end)

Core.Commands.Add("revokelicense", Lang:t("commands.license_revoke"), {{name = "id", help = Lang:t('info.player_id')}, {name = "license", help = Lang:t('info.license_type')}}, true, function(source, args)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.grade.level >= Config.LicenseRank then
        if args[2] == "driver" or args[2] == "weapon" then
            ...
            if not licenseTable[args[2]] then
                TriggerClientEvent('Core:Notify', src, Lang:t("error.error_license"), "error")
                return
            end
            licenseTable[args[2]] = false
            SearchedPlayer.Functions.SetMetaData("licences", licenseTable)
            TriggerClientEvent('Core:Notify', SearchedPlayer.PlayerData.source, Lang:t("error.revoked_license"), "error")
            TriggerClientEvent('Core:Notify', src, Lang:t("success.revoke_license"), "success")
        else
            TriggerClientEvent('Core:Notify', src, Lang:t("error.error_license"), "error")
        end
    else
        TriggerClientEvent('Core:Notify', src, Lang:t("error.rank_revoke"), "error")
    end
end)

Core.Commands.Add("pobject", Lang:t("commands.place_object"), {{name = "type",help = Lang:t("info.poobject_object")}}, true, function(source, args)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    ...
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        ...
        TriggerClientEvent('Core:Notify', src, Lang:t("error.on_duty_police_only"), 'error')
    end
end)

Core.Commands.Add("cuff", Lang:t("commands.cuff_player"), {}, false, function(source)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
Core.Commands.Add("escort", Lang:t("commands.escort"), {}, false, function(source)
    local src = source
    TriggerClientEvent("police:client:EscortPlayer", src)
end)

Core.Commands.Add("callsign", Lang:t("commands.callsign"), {{name = "name", help = Lang:t('info.callsign_name')}}, false, function(source, args)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    Player.Functions.SetMetaData("callsign", table.concat(args, " "))
end)

Core.Commands.Add("clearcasings", Lang:t("commands.clear_casign"), {}, false, function(source)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
Core.Commands.Add("jail", Lang:t("commands.jail_player"), {}, false, function(source)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
Core.Commands.Add("unjail", Lang:t("commands.unjail_player"), {{name = "id", help = Lang:t('info.player_id')}}, true, function(source, args)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
Core.Commands.Add("clearblood", Lang:t("commands.clearblood"), {}, false, function(source)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
Core.Commands.Add("seizecash", Lang:t("commands.seizecash"), {}, false, function(source)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
Core.Commands.Add("sc", Lang:t("commands.softcuff"), {}, false, function(source)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
Core.Commands.Add("cam", Lang:t("commands.camera"), {{name = "camid", help = Lang:t('info.camera_id')}}, false, function(source, args)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
Core.Commands.Add("flagplate", Lang:t("commands.flagplate"), {{name = "plate", help = Lang:t('info.plate_number')}, {name = "reason", help = Lang:t('info.flag_reason')}}, true, function(source, args)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        local reason = {}
        ...
        TriggerClientEvent('Core:Notify', src, Lang:t("info.vehicle_flagged", {vehicle = args[1]:upper(), reason = table.concat(reason, " ")}))
    else
        TriggerClientEvent('Core:Notify', src, Lang:t("error.on_duty_police_only"), 'error')
    end
end)

Core.Commands.Add("unflagplate", Lang:t("commands.unflagplate"), {{name = "plate", help = Lang:t('info.plate_number')}}, true, function(source, args)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        if Plates and Plates[args[1]:upper()] then
            if Plates[args[1]:upper()].isflagged then
                ...
                TriggerClientEvent('Core:Notify', src, Lang:t("info.unflag_vehicle", {vehicle = args[1]:upper()}))
            else
                TriggerClientEvent('Core:Notify', src, Lang:t("error.vehicle_not_flag"), 'error')
            end
        else
            TriggerClientEvent('Core:Notify', src, Lang:t("error.vehicle_not_flag"), 'error')
        end
...
Core.Commands.Add("plateinfo", Lang:t("commands.plateinfo"), {{name = "plate", help = Lang:t('info.plate_number')}}, true, function(source, args)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        if Plates and Plates[args[1]:upper()] then
            if Plates[args[1]:upper()].isflagged then
                TriggerClientEvent('Core:Notify', src, Lang:t('success.vehicle_flagged', {plate = args[1]:upper(), reason = Plates[args[1]:upper()].reason}), 'success')
            else
                TriggerClientEvent('Core:Notify', src, Lang:t("error.vehicle_not_flag"), 'error')
            end
        else
...
Core.Commands.Add("depot", Lang:t("commands.depot"), {{name = "price", help = Lang:t('info.impound_price')}}, false, function(source, args)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
Core.Commands.Add("impound", Lang:t("commands.impound"), {}, false, function(source)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
Core.Commands.Add("paytow", Lang:t("commands.paytow"), {{name = "id", help = Lang:t('info.player_id')}}, true, function(source, args)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        local playerId = tonumber(args[1])
        ...
        if OtherPlayer then
            if OtherPlayer.PlayerData.job.name == "tow" then
                ...
                TriggerClientEvent('Core:Notify', OtherPlayer.PlayerData.source, Lang:t("success.tow_paid"), 'success')
                TriggerClientEvent('Core:Notify', src, Lang:t("info.tow_driver_paid"))
            else
                TriggerClientEvent('Core:Notify', src, Lang:t("error.not_towdriver"), 'error')
            end
        end
    else
        TriggerClientEvent('Core:Notify', src, Lang:t("error.on_duty_police_only"), 'error')
...
Core.Commands.Add("paylawyer", Lang:t("commands.paylawyer"), {{name = "id",help = Lang:t('info.player_id')}}, true, function(source, args)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "judge" then
        ...
        if OtherPlayer.PlayerData.job.name == "lawyer" then
            ...
            TriggerClientEvent('Core:Notify', OtherPlayer.PlayerData.source, Lang:t("success.tow_paid"), 'success')
            TriggerClientEvent('Core:Notify', src, Lang:t("info.paid_lawyer"))
        else
            TriggerClientEvent('Core:Notify', src, Lang:t("error.not_lawyer"), "error")
        end
    else
        TriggerClientEvent('Core:Notify', src, Lang:t("error.on_duty_police_only"), 'error')
    end
...
Core.Commands.Add("anklet", Lang:t("commands.anklet"), {}, false, function(source)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
Core.Commands.Add("ankletlocation", Lang:t("commands.ankletlocation"), {{name = "cid", help = Lang:t('info.citizen_id')}}, true, function(source, args)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        local citizenid = args[1]
        local Target = Core.Functions.GetPlayerByCitizenId(citizenid)
        ...
        if Target.PlayerData.metadata["tracker"] then
            ...
            TriggerClientEvent('Core:Notify', src, Lang:t("error.no_anklet"), 'error')
        end
    else
        TriggerClientEvent('Core:Notify', src, Lang:t("error.on_duty_police_only"), 'error')
    end
...
Core.Commands.Add("takedrivinglicense", Lang:t("commands.drivinglicense"), {}, false, function(source)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
Core.Commands.Add("takedna", Lang:t("commands.takedna"), {{name = "id", help = Lang:t('info.player_id')}}, true, function(source, args)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    local OtherPlayer = Core.Functions.GetPlayer(tonumber(args[1]))
    ...
    if Player.Functions.RemoveItem("empty_evidence_bag", 1) then
        ...
        TriggerClientEvent("inventory:client:ItemBox", src, Core.Shared.Items["filled_evidence_bag"], "add")
    else
        TriggerClientEvent('Core:Notify', src, Lang:t("error.have_evidence_bag"), "error")
    end
end)

RegisterNetEvent('police:server:SendTrackerLocation', function(coords, requestId)
    local Target = Core.Functions.GetPlayer(source)
    ...
    TriggerClientEvent("phoneclient:addPoliceAlert", requestId, alertData)
end)

Core.Commands.Add('911p', Lang:t("commands.police_report"), {{name='message', help= Lang:t("commands.message_sent")}}, false, function(source, args)
	local src = source
    local message
    ...
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == 'police' and v.PlayerData.job.onduty then
            ...
            TriggerClientEvent("phoneclient:addPoliceAlert", v.PlayerData.source, alertData)
...
-- Items
Core.Functions.CreateUseableItem("handcuffs", function(source)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
...
Core.Functions.CreateUseableItem("moneybag", function(source, item)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
...
-- Callbacks
Core.Functions.CreateCallback('police:server:isPlayerDead', function(_, cb, playerId)
    local Player = Core.Functions.GetPlayer(playerId)
    cb(Player.PlayerData.metadata["isdead"])
end)

Core.Functions.CreateCallback('police:GetPlayerStatus', function(_, cb, playerId)
    local Player = Core.Functions.GetPlayer(playerId)
    local statList = {}
    if Player then
...
Core.Functions.CreateCallback('police:IsSilencedWeapon', function(source, cb, weapon)
    local Player = Core.Functions.GetPlayer(source)
    local itemInfo = Player.Functions.GetItemByName(Core.Shared.Weapons[weapon]["name"])
    local retval = false
    if itemInfo then
...
Core.Functions.CreateCallback('police:GetDutyPlayers', function(_, cb)
    local dutyPlayers = {}
    local players = Core.Functions.GetJNPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == "police" and v.PlayerData.job.onduty then
...
Core.Functions.CreateCallback('police:GetImpoundedVehicles', function(_, cb)
    local vehicles = {}
...
Core.Functions.CreateCallback('police:IsPlateFlagged', function(_, cb, plate)
    local retval = false
...
Core.Functions.CreateCallback('police:GetCops', function(_, cb)
    local amount = 0
    local players = Core.Functions.GetJNPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == "police" and v.PlayerData.job.onduty then
...
Core.Functions.CreateCallback('police:server:IsPoliceForcePresent', function(_, cb)
    local retval = false
    local players = Core.Functions.GetJNPlayers()
    for _, v in pairs(players) do
...
RegisterNetEvent('police:server:TakeOutImpound', function(plate, garage)
    ...
    TriggerClientEvent('Core:Notify', src, Lang:t("success.impound_vehicle_removed"), 'success')
end)
...
RegisterNetEvent('police:server:CuffPlayer', function(playerId, isSoftcuff)
    ...
    local Player = Core.Functions.GetPlayer(src)
    local CuffedPlayer = Core.Functions.GetPlayer(playerId)
...
RegisterNetEvent('police:server:EscortPlayer', function(playerId)
    ...
    local Player = Core.Functions.GetPlayer(source)
    local EscortPlayer = Core.Functions.GetPlayer(playerId)
    ...
    if (Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "ambulance") or (EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"]) then
        ...
        TriggerClientEvent('Core:Notify', src, Lang:t("error.not_cuffed_dead"), 'error')
    end
end)
...
RegisterNetEvent('police:server:SetPlayerOutVehicle', function(playerId)
    ...
    local EscortPlayer = Core.Functions.GetPlayer(playerId)
    if not Core.Functions.GetPlayer(src) or not EscortPlayer then return end
...
RegisterNetEvent('police:server:BillPlayer', function(playerId, price)
    ...
    exports['management']:AddMoney("police", price)
    TriggerClientEvent('Core:Notify', OtherPlayer.PlayerData.source, Lang:t("info.fine_received", {fine = price}))
end)
...
RegisterNetEvent('police:server:JailPlayer', function(playerId, time)
    ...
    TriggerClientEvent('Core:Notify', src, Lang:t("info.sent_jail_for", {time = time}))
end)
...
RegisterNetEvent('police:server:SearchPlayer', function(playerId)
    ...
    local SearchedPlayer = Core.Functions.GetPlayer(playerId)
    if not Core.Functions.GetPlayer(src) or not SearchedPlayer then return end

    TriggerClientEvent('Core:Notify', src, Lang:t("info.cash_found", {cash = SearchedPlayer.PlayerData.money["cash"]}))
    TriggerClientEvent('Core:Notify', SearchedPlayer.PlayerData.source, Lang:t("info.being_searched"))
end)
...
RegisterNetEvent('police:server:SeizeCash', function(playerId)
    ...
    TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items["moneybag"], "add")
    TriggerClientEvent('Core:Notify', SearchedPlayer.PlayerData.source, Lang:t("info.cash_confiscated"))
end)
...
RegisterNetEvent('police:server:SeizeDriverLicense', function(playerId)
    ...
    if driverLicense then
        ...
        TriggerClientEvent('Core:Notify', SearchedPlayer.PlayerData.source, Lang:t("info.driving_license_confiscated"))
    else
        TriggerClientEvent('Core:Notify', src, Lang:t("error.no_driver_license"), 'error')
    end
end)
...
RegisterNetEvent('police:server:RobPlayer', function(playerId)
    ...
    TriggerClientEvent('Core:Notify', SearchedPlayer.PlayerData.source, Lang:t("info.cash_robbed", {money = money}))
    TriggerClientEvent('Core:Notify', Player.PlayerData.source, Lang:t("info.stolen_money", {stolen = money}))
end)
...
RegisterNetEvent('police:server:Impound', function(plate, fullImpound, price, body, engine, fuel)
    ...
    if IsVehicleOwned(plate) then
        if not fullImpound then
            ...
            TriggerClientEvent('Core:Notify', src, Lang:t("info.vehicle_taken_depot", {price = price}))
        else
            MySQL.query(
                'UPDATE player_vehicles SET state = ?, body = ?, engine = ?, fuel = ? WHERE plate = ?',
                {2, body, engine, fuel, plate})
            TriggerClientEvent('Core:Notify', src, Lang:t("info.vehicle_seized"))
        end
    end
end)
...
RegisterNetEvent('police:server:SetTracker', function(targetId)
    ...
    if not Core.Functions.GetPlayer(src) or not Target then return end

    local TrackerMeta = Target.PlayerData.metadata["tracker"]
    if TrackerMeta then
        Target.Functions.SetMetaData("tracker", false)
        TriggerClientEvent('Core:Notify', targetId, Lang:t("success.anklet_taken_off"), 'success')
        TriggerClientEvent('Core:Notify', src, Lang:t("success.took_anklet_from", {firstname = Target.PlayerData.charinfo.firstname, lastname = Target.PlayerData.charinfo.lastname}), 'success')
        TriggerClientEvent('police:client:SetTracker', targetId, false)
    else
        Target.Functions.SetMetaData("tracker", true)
        TriggerClientEvent('Core:Notify', targetId, Lang:t("success.put_anklet"), 'success')
        TriggerClientEvent('Core:Notify', src, Lang:t("success.put_anklet_on", {firstname = Target.PlayerData.charinfo.firstname, lastname = Target.PlayerData.charinfo.lastname}), 'success')
