...
$(document).on('click', '#save', function(){
    $.post('https://tunerchip/save', JSON.stringify({
...
$(document).on('click', "#save-neon", function(){
    if (RainbowNeon) {
        $.post('https://tunerchip/saveNeon', JSON.stringify({
...
$(document).on('click', '#save-headlights', function(){
    $.post('https://tunerchip/saveHeadlights', JSON.stringify({
        value: headlightVal,
...
$(document).on('click', '#save-stancer', function(){
    ...
    $.post('https://tunerchip/SetStancer', JSON.stringify({
...
JNTuner.Open = function() {
    $.post('https://tunerchip/checkItem', JSON.stringify({item: "tunerlaptop"}), function(hasItem){
        if (hasItem) {
...
JNTuner.Close = function() {
    $('.container').fadeOut(250);
    $.post('https://tunerchip/exit');
}