...
RegisterNetEvent('Core:Client:OnPlayerLoaded', function()
    Core.Functions.TriggerCallback('nitrous:GetNosLoadedVehs', function(vehs)
        VehicleNitrous = vehs
    end)
end)
    ...
    if not NitrousActivated then
        if IsInVehicle and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
            if GetPedInVehicleSeat(veh, -1) == ped then
                Core.Functions.Progressbar("use_nos", Lang:t("text.connecting_nos"), 1000, false, true, {
                    disableMovement = false,
                    disableCarMovement = false,
                    disableMouse = false,
                    disableCombat = true,
                }, {}, {}, {}, function() -- Done
                    TriggerEvent("inventory:client:ItemBox", Core.Shared.Items['nitrous'], "remove")
                ...
                Core.Functions.Notify(Lang:t("error.you_cannot_do_that_from_this_seat"), "error")
            end
        else
            Core.Functions.Notify(Lang:t("error.you_are_not_in_a_vehicle"), 'error')
        end
    else
        Core.Functions.Notify(Lang:t("error.you_already_have_nos_active"), 'error')
    end
end)
