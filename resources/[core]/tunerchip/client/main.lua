...
RegisterNUICallback('save', function(data, cb)
    Core.Functions.TriggerCallback('tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            local ped = PlayerPedId()
            local veh = GetVehiclePedIsUsing(ped)
            setVehData(veh, data)
            Core.Functions.Notify(Lang:t("error.tunerchip_vehicle_tuned"), 'error')

            TriggerServerEvent('tunerchip:server:TuneStatus', Core.Functions.GetPlate(veh), true)
        end
        cb('ok')
    end)
end)

RegisterNetEvent('tunerchip:client:TuneStatus', function()
    local ped = PlayerPedId()
    ...
    if vehModel ~= 0 then
        Core.Functions.TriggerCallback('tunerchip:server:GetStatus', function(status)
            if status then
                Core.Functions.Notify(Lang:t("success.this_vehicle_has_been_tuned"), 'success')
            else
                Core.Functions.Notify(Lang:t("error.this_vehicle_has_not_been_tuned"), 'error')
            end
        end, plate)
    else
        Core.Functions.Notify(Lang:t("error.no_vehicle_nearby"), 'error')
    end
end)
...
RegisterNUICallback('reset', function(_, cb)
    ...
    local veh = GetVehiclePedIsUsing(ped)
    resetVeh(veh)
    Core.Functions.Notify(Lang:t("error.tunerchip_vehicle_has_been_reset"), 'error')
    cb("ok")
end)

RegisterNetEvent('tunerchip:client:openChip', function()
    local ped = PlayerPedId()
    local inVehicle = IsPedInAnyVehicle(ped)

    if inVehicle then
        Core.Functions.Progressbar("connect_laptop", Lang:t("error.tunerchip_vehicle_has_been_reset"), 2000, false, true, {
        ...
        Core.Functions.Notify(Lang:t("error.you_are_not_in_a_vehicle"), "error")
    end
end)
