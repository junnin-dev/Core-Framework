...
local function saveVehicle()
    ...
    local myCar = Core.Functions.GetVehicleProperties(veh)
    TriggerServerEvent('customs:server:updateVehicle', myCar)
end
...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("customs:server:attemptPurchase", type, upgradeLevel, CustomsData.location)
...
function EnterLocation(override)
    ...
    Core.Functions.TriggerCallback('vehicletuning:server:IsMechanicAvailable', function(currentMechanics)
        if currentMechanics >= Config.MinOnlineMechanics and not override and PlayerData.job.name ~= 'mechanic' then
            ...
            Core.Functions.Notify('Algumas compras estão atualmente indisponíveis.Por favor, encontre um mecânico.')
        end
        ...
        if not canEnter then
            Core.Functions.Notify('Você não pode fazer nada aqui!')
            ExitBennys()
            return
        end

        if Config.UseRadial and radialMenuItemId then
            exports['radialmenu']:RemoveOption(radialMenuItemId)
...
function GetLocations()
    Core.Functions.TriggerCallback("customs:server:GetLocations", function(locations)
...
function CheckForGhostVehicle()
    ...
    local closestVehicle, closestDistance = Core.Functions.GetClosestVehicle(GetEntityCoords(PlayerPedId()))
        ...
        if PlayerData.job.name ~= 'mechanic' and Config.DisableWhenMechanicsOnline and currentMechanics >= Config.MinOnlineMechanics then
            ...
            if Config.UseRadial then
                if not radialMenuItemId then
                    radialMenuItemId = exports['radialmenu']:AddOption({
                        id = 'customs',
                        title = 'Enter Customs',
                        icon = 'wrench',
                        type = 'client',
                        event = 'customs:client:EnterCustoms',
                        shouldClose = true
                    })
                end
            else
...
RegisterNetEvent('Core:Client:OnJobUpdate', function(job)
    PlayerData.job = job
end)

RegisterNetEvent('customs:client:UpdateLocation', function(location, type, key, value)
...
RegisterNetEvent("customs:client:purchaseSuccessful", function()
    ...
    Core.Functions.Notify("Comprou com sucesso")
end)
