...
local function DestroyPickupTarget()
  ...
  if Config.UseTarget then
    exports['target']:RemoveZone(pickupTargetID)
    pickupZone = nil
  else
    pickupZone:destroy()
    pickupZone = nil
...
local function RegisterEntranceTarget()
  ...
  if Config.UseTarget then
    entranceZone = exports['target']:AddBoxZone(entranceTargetID, coords, 1, 4, {
      ...
      options = {
        {
          type = 'client',
          event = 'recyclejob:client:target:enterLocation',
    ...
    entranceZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['core']:DrawText(Lang:t("text.point_enter_warehouse"), 'left')
      else
...
local function RegisterExitTarget()
  ...
  if Config.UseTarget then
    exitZone = exports['target']:AddBoxZone(exitTargetID, coords, 1, 4, {
      name = exitTargetID,
      heading = 270,
      ...
      options = {
        {
          type = 'client',
          event = 'recyclejob:client:target:exitLocation',
    ...
    exitZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['core']:DrawText(Lang:t("text.point_exit_warehouse"), 'left')
      else
        exports['core']:HideText()
      end
...
local function DestroyExitTarget()
  ...
  if Config.UseTarget then
    exports['target']:RemoveZone(exitTargetID)
    exitZone = nil
  else
    exitZone:destroy()
    exitZone = nil
...
local function RegisterDutyTarget()
  ...
  if Config.UseTarget then
    dutyZone = exports['target']:AddBoxZone(dutyTargetID, coords, 1, 1, {
      name = dutyTargetID,
      heading = 270,
      ...
      options = {
        {
          type = 'client',
          event = 'recyclejob:client:target:toggleDuty',
    ...
    dutyZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['core']:DrawText(GetDutyTargetText(), 'left')
      else
        exports['core']:HideText()
      end
...
local function DestroyDutyTarget()
  ...
  if Config.UseTarget then
    exports['target']:RemoveZone(dutyTargetID)
    dutyZone = nil
  else
    dutyZone:destroy()
    dutyZone = nil
...
local function RegisterDeliveyTarget()
  ...
  if Config.UseTarget then
    deliveryZone = exports['target']:AddBoxZone(deliveryTargetID, coords, 1, 1, {
      ...
      options = {
        {
          type = 'client',
          event = 'recyclejob:client:target:dropPackage',
    ...
    deliveryZone:onPlayerInOut(function(isPointInside)
      if isPointInside and carryPackage then
        exports['core']:DrawText(Lang:t("text.point_hand_in_package"), 'left')
      else
        exports['core']:HideText()
      end
...
local function DestroyDeliveryTarget()
  ...
  if Config.UseTarget then
    exports['target']:RemoveZone(deliveryTargetID)
    deliveryZone = nil
  else
...
function RegisterPickupTarget(coords)
  ...
  if Config.UseTarget then
    pickupZone = exports['target']:AddBoxZone(pickupTargetID, targetCoords, 4, 1.5, {
      name = pickupTargetID,
      heading = coords.h,
      minZ = coords.z - 1.0,
      maxZ = coords.z + 2.0,
      ...
      options = {
        {
          type = 'client',
          event = 'recyclejob:client:target:pickupPackage',
    ...
    pickupZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['core']:DrawText( Lang:t("text.point_get_package"), 'left')
      else
        exports['core']:HideText()
      end
...
RegisterNetEvent('recyclejob:client:target:enterLocation', function()
  EnterLocation()
end)

RegisterNetEvent('recyclejob:client:target:exitLocation', function()
  ExitLocation()
end)

RegisterNetEvent('recyclejob:client:target:toggleDuty', function()
  onDuty = not onDuty
  if onDuty then
    Core.Functions.Notify(Lang:t("success.you_have_been_clocked_in"), 'success')
    GetRandomPackage()
  else
    Core.Functions.Notify(Lang:t("error.you_have_clocked_out"), 'error')
    DestroyPickupTarget()
  end
...
RegisterNetEvent('recyclejob:client:target:pickupPackage', function()
  ...
  Core.Functions.Progressbar('pickup_reycle_package', Lang:t("text.picking_up_the_package"), Config.PickupActionDuration, false, true, {
...
RegisterNetEvent('recyclejob:client:target:dropPackage', function()
  ...
  ScrapAnim()
  DestroyDeliveryTarget()
  Core.Functions.Progressbar('deliver_reycle_package',  Lang:t("text.unpacking_the_package"), Config.DeliveryActionDuration, false, true, {
      disableMovement = true,
      disableCarMovement = true,
      disableMouse = false,
      disableCombat = true
    ...
    while true do
      ...
      if isInsideEntranceZone then
        ...
        if IsControlJustReleased(0, 38) then
          ...
          Wait(500)
          TriggerEvent('recyclejob:client:target:enterLocation')
          exports['core']:HideText()
        end
      end
      ...
      if isInsideExitZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['core']:KeyPressed()
          Wait(500)
          TriggerEvent('recyclejob:client:target:exitLocation')
          exports['core']:HideText()
        end
      end
      ...
      if isInsideDutyZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['core']:KeyPressed()
          Wait(500)
          TriggerEvent('recyclejob:client:target:toggleDuty')
          exports['core']:HideText()
        end
      end
      ...
      if onDuty then
        if isInsidePickupZone and not carryPackage then
          ...
          if IsControlJustReleased(0, 38) then
            exports['core']:KeyPressed()
            Wait(500)
            TriggerEvent('recyclejob:client:target:pickupPackage')
            exports['core']:HideText()
          end
        ...
        if isInsideDeliveryZone and carryPackage then
          ...
          if IsControlJustReleased(0, 38) then
            exports['core']:KeyPressed()
            Wait(500)
            TriggerEvent('recyclejob:client:target:dropPackage')
            exports['core']:HideText()
          end
        end
      end
