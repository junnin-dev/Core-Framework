...
RegisterNetEvent('recycle:server:getItem', function()
  local src = source
  ...
  if chance < 7 then
    ...
    TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items[Config.ChanceItem], 'add')
  end
  ...
  if luck == odd then
    ...
    TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items[Config.LuckyItem], 'add')
  end
end)
