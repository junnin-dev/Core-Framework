...
RegisterNetEvent('weathersync:server:setWeather', function(weather)
    ...
    if isAllowedToChange(src) then
        ...
        if src > 0 then
            if (success) then TriggerClientEvent('Core:Notify', src, Lang:t('weather.updated'))
            else TriggerClientEvent('Core:Notify', src, Lang:t('weather.invalid'))
            end
        end
    end
end)
...
RegisterNetEvent('weathersync:server:setTime', function(hour, minute)
    ...
    if isAllowedToChange(src) then
        ...
        if src > 0 then
            if (success) then TriggerClientEvent('Core:Notify', src, Lang:t('time.change', {value = hour, value2 = minute or "00"}))
            else TriggerClientEvent('Core:Notify', src, Lang:t('time.invalid'))
            end
        end
    end
end)
...
RegisterNetEvent('weathersync:server:toggleBlackout', function(state)
    ...
    if isAllowedToChange(src) then
        ...
        if src > 0 then
            if (newstate) then TriggerClientEvent('Core:Notify', src, Lang:t('blackout.enabled'))
            else TriggerClientEvent('Core:Notify', src, Lang:t('blackout.disabled'))
            end
        end
    end
end)
...
RegisterNetEvent('weathersync:server:toggleFreezeTime', function(state)
    ...
    if isAllowedToChange(src) then
        ...
        if src > 0 then
            if (newstate) then TriggerClientEvent('Core:Notify', src, Lang:t('time.now_frozen'))
            else TriggerClientEvent('Core:Notify', src, Lang:t('time.now_unfrozen'))
            end
        end
    end
end)
...
RegisterNetEvent('weathersync:server:toggleDynamicWeather', function(state)
    ...
    if isAllowedToChange(src) then
        ...
        if src > 0 then
            if (newstate) then TriggerClientEvent('Core:Notify', src, Lang:t('weather.now_unfrozen'))
            else TriggerClientEvent('Core:Notify', src, Lang:t('weather.now_frozen'))
            end
        end
    end
end)
...
-- COMMANDS
Core.Commands.Add('freezetime', Lang:t('help.freezecommand'), {}, false, function(source)
    local newstate = setTimeFreeze()
    if source > 0 then
        if (newstate) then return TriggerClientEvent('Core:Notify', source, Lang:t('time.frozenc')) end
        return TriggerClientEvent('Core:Notify', source, Lang:t('time.unfrozenc'))
    end
    if (newstate) then return print(Lang:t('time.now_frozen')) end
    return print(Lang:t('time.now_unfrozen'))
end, 'admin')

Core.Commands.Add('freezeweather', Lang:t('help.freezeweathercommand'), {}, false, function(source)
    local newstate = setDynamicWeather()
    if source > 0 then
        if (newstate) then return TriggerClientEvent('Core:Notify', source, Lang:t('dynamic_weather.enabled')) end
        return TriggerClientEvent('Core:Notify', source, Lang:t('dynamic_weather.disabled'))
    end
    if (newstate) then return print(Lang:t('weather.now_unfrozen')) end
    return print(Lang:t('weather.now_frozen'))
end, 'admin')

Core.Commands.Add('weather', Lang:t('help.weathercommand'), {{name = Lang:t('help.weathertype'), help = Lang:t('help.availableweather')}}, true, function(source, args)
    local success = setWeather(args[1])
    if source > 0 then
        if (success) then return TriggerClientEvent('Core:Notify', source, Lang:t('weather.willchangeto', {value = string.lower(args[1])})) end
        return TriggerClientEvent('Core:Notify', source, Lang:t('weather.invalidc'), 'error')
    end
    if (success) then return print(Lang:t('weather.updated')) end
    return print(Lang:t('weather.invalid'))
end, 'admin')

Core.Commands.Add('blackout', Lang:t('help.blackoutcommand'), {}, false, function(source)
    local newstate = setBlackout()
    if source > 0 then
        if (newstate) then return TriggerClientEvent('Core:Notify', source, Lang:t('blackout.enabledc')) end
        return TriggerClientEvent('Core:Notify', source, Lang:t('blackout.disabledc'))
    end
    if (newstate) then return print(Lang:t('blackout.enabled')) end
    return print(Lang:t('blackout.disabled'))
end, 'admin')

Core.Commands.Add('morning', Lang:t('help.morningcommand'), {}, false, function(source)
    setTime(9, 0)
    if source > 0 then return TriggerClientEvent('Core:Notify', source, Lang:t('time.morning')) end
end, 'admin')

Core.Commands.Add('noon', Lang:t('help.nooncommand'), {}, false, function(source)
    setTime(12, 0)
    if source > 0 then return TriggerClientEvent('Core:Notify', source, Lang:t('time.noon')) end
end, 'admin')

Core.Commands.Add('evening', Lang:t('help.eveningcommand'), {}, false, function(source)
    setTime(18, 0)
    if source > 0 then return TriggerClientEvent('Core:Notify', source, Lang:t('time.evening')) end
end, 'admin')

Core.Commands.Add('night', Lang:t('help.nightcommand'), {}, false, function(source)
    setTime(23, 0)
    if source > 0 then return TriggerClientEvent('Core:Notify', source, Lang:t('time.night')) end
end, 'admin')

Core.Commands.Add('time', Lang:t('help.timecommand'), {{ name=Lang:t('help.timehname'), help=Lang:t('help.timeh') }, { name=Lang:t('help.timemname'), help=Lang:t('help.timem') }}, true, function(source, args)
    local success = setTime(args[1], args[2])
    if source > 0 then
        if (success) then return TriggerClientEvent('Core:Notify', source, Lang:t('time.changec', {value = args[1] .. ':' .. (args[2] or "00")})) end
        return TriggerClientEvent('Core:Notify', source, Lang:t('time.invalidc'), 'error')
    end
