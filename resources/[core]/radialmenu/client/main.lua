...
PlayerData = Core.Functions.GetPlayerData() -- Setting this for when you restart the resource in game
...
local function controlToggle(bool)
    for i = 1, #controlsToToggle,1 do
        if bool then
            exports['smallresources']:addDisableControls(controlsToToggle[i])
        else
            exports['smallresources']:removeDisableControls(controlsToToggle[i])
        end
    end
end
...
local function setRadialState(bool, sendMessage, delay)
    ...
    if Config.UseWhilstWalking then
        ...
        if bool then
            TriggerEvent('radialmenu:client:onRadialmenuOpen')
            SetupRadialMenu()
        else
            TriggerEvent('radialmenu:client:onRadialmenuClose')
        end
        SetNuiFocus(bool, bool)
    end
...
RegisterNetEvent('radialmenu:client:noPlayers', function()
    Core.Functions.Notify(Lang:t("error.no_people_nearby"), 'error', 2500)
end)

RegisterNetEvent('radialmenu:client:openDoor', function(data)
    ...
    if closestVehicle ~= 0 then
        if closestVehicle ~= GetVehiclePedIsIn(ped) then
            local plate = Core.Functions.GetPlate(closestVehicle)
        ...
        Core.Functions.Notify(Lang:t("error.no_vehicle_found"), 'error', 2500)
    end
end)

RegisterNetEvent('radialmenu:client:setExtra', function(data)
    local string = data.id
        ...
        if GetPedInVehicleSeat(veh, -1) == ped then
            ...
            if DoesExtraExist(veh, extra) then
                if IsVehicleExtraTurnedOn(veh, extra) then
                    ...
                    Core.Functions.Notify(Lang:t("error.extra_deactivated", {extra = extra}), 'error', 2500)
                else
                    SetVehicleExtra(veh, extra, 0)
                    Core.Functions.Notify(Lang:t("success.extra_activated", {extra = extra}), 'success', 2500)
                end
            else
                Core.Functions.Notify(Lang:t("error.extra_not_present", {extra = extra}), 'error', 2500)
            end
        else
            Core.Functions.Notify(Lang:t("error.not_driver"), 'error', 2500)
        end
    end
end)
...
RegisterNetEvent('radialmenu:trunk:client:Door', function(plate, door, open)
    ...
    if veh ~= 0 then
        local pl = Core.Functions.GetPlate(veh)
        if pl == plate then
            if open then
...
RegisterNetEvent('radialmenu:client:ChangeSeat', function(data)
    local Veh = GetVehiclePedIsIn(PlayerPedId())
    local IsSeatFree = IsVehicleSeatFree(Veh, data.id)
    local speed = GetEntitySpeed(Veh)
    local HasHarnass = exports['smallresources']:HasHarness()
    if not HasHarnass then
        local kmh = speed * 3.6
        if IsSeatFree then
            if kmh <= 100.0 then
                SetPedIntoVehicle(PlayerPedId(), Veh, data.id)
                Core.Functions.Notify(Lang:t("info.switched_seats", {seat = data.title}))
            else
                Core.Functions.Notify(Lang:t("error.vehicle_driving_fast"), 'error')
            end
        else
            Core.Functions.Notify(Lang:t("error.seat_occupied"), 'error')
        end
    else
        Core.Functions.Notify(Lang:t("error.race_harness_on"), 'error')
    end
end)

RegisterNetEvent('radialmenu:flipVehicle', function()
    TriggerEvent('animations:client:EmoteCommandStart', {"mechanic"})
    Core.Functions.Progressbar("pick_grape", Lang:t("progress.flipping_car"), Config.Fliptime, false, true, {
        ...
        Core.Functions.Notify(Lang:t("task.cancel_task"), "error")
        TriggerEvent('animations:client:EmoteCommandStart', {"c"})
    end)
end)
...
RegisterNUICallback('selectItem', function(inData, cb)
    ...
    if data and found then
        ...
        elseif data.type == 'jncommand' then
            TriggerServerEvent('Core:CallCommand', data.event, data)
        end
    end
    cb('ok')
end)
