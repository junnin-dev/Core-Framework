...
RegisterNetEvent('radialmenu:client:TakeStretcher', function()
    ...
    if vehicle ~= 0 then
        ...
        if obj ~= 0 then
            ...
            Core.Functions.Notify(Lang:t("error.obj_not_found"), 'error')
        end
    else
        Core.Functions.Notify(Lang:t("error.not_near_ambulance"), 'error')
    end
end)
...
RegisterNetEvent('radialmenu:client:RemoveStretcher', function()
    ...
    if stretcherObject then
        ...
        if #(coords - bCoords) < 3.0 then
            ...
            Core.Functions.Notify(Lang:t("error.far_away"), 'error')
        end
    end
end)
...
RegisterNetEvent('radialmenu:client:Result', function(isBusy, type)
    ...
    if type == "lay" then
        if not isBusy then
            ...
            Core.Functions.Notify(Lang:t("error.stretcher_in_use"), "error")
            isLayingOnBed = false
        end
    else
        if not isBusy then
