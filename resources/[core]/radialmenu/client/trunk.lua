...
RegisterNetEvent('kidnapping:client:SetKidnapping', function(bool)
    isKidnapping = bool
end)

RegisterNetEvent('trunk:client:KidnapTrunk', function()
    local closestPlayer, distance = Core.Functions.GetClosestPlayer()
    if distance ~= -1 and distance < 2 then
        if isKidnapping then
            ...
            if closestVehicle ~= 0 then
                ...
                Wait(50)
                TriggerServerEvent("trunk:server:KidnapTrunk", GetPlayerServerId(closestPlayer), closestVehicle)
            end
        else
            Core.Functions.Notify(Lang:t("error.not_kidnapped"), 'error')
        end
    end
end)

RegisterNetEvent('trunk:client:KidnapGetIn', function(veh)
    ...
    if Config.TrunkClasses[vehClass].allowed then
        Core.Functions.TriggerCallback('trunk:server:getTrunkBusy', function(isBusy)
                    ...
                    if not isBusy then
                        if not isKidnapped then
                            if GetVehicleDoorAngleRatio(closestVehicle, 5) > 0 then
                                ...
                                TriggerServerEvent('trunk:server:setTrunkBusy', plate, true)
                                inTrunk = true
                                Wait(500)
                                SetVehicleDoorShut(closestVehicle, 5, false)
                                Core.Functions.Notify(Lang:t("success.entered_trunk"), 'success', 4000)
                                TrunkCam(true)
                                isKidnapped = true
                            else
                                Core.Functions.Notify(Lang:t("error.trunk_closed"), 'error', 2500)
                            end
                        else
                            ...
                            if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                                ...
                                ClearPedTasks(ped)
                                inTrunk = false
                                TriggerServerEvent('smallresources:trunk:server:setTrunkBusy', plate, nil)
                        ...
                        Core.Functions.Notify(Lang:t("error.someone_in_trunk"), 'error', 2500)
                    end
                else
                    Core.Functions.Notify(Lang:t("error.already_in_trunk"), 'error', 2500)
                end
            else
                Core.Functions.Notify(Lang:t("error.cant_enter_trunk"), 'error', 2500)
            end
        end, plate)
    else
        Core.Functions.Notify(Lang:t("error.cant_enter_trunk"), 'error', 2500)
...
RegisterNetEvent('trunk:client:GetIn', function()
    local ped = PlayerPedId()
    local closestVehicle = getNearestVeh()
    if closestVehicle ~= 0 then
        local vehClass = GetVehicleClass(closestVehicle)
                ...
                if DoesEntityExist(vehicle) then
                    ...
                    if IsControlJustPressed(0, 38) then
                        if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                            ...
                            ClearPedTasks(ped)
                            inTrunk = false
                            TriggerServerEvent('trunk:server:setTrunkBusy', plate, false)
                            SetEntityCoords(ped, vehCoords.x, vehCoords.y, vehCoords.z)
