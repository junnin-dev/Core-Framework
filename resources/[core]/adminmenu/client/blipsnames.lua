...
RegisterNetEvent('admin:client:toggleBlips', function()
    if not ShowBlips then
        ShowBlips = true
        NetCheck1 = true
        Core.Functions.Notify(Lang:t("success.blips_activated"), "success")
    else
        ShowBlips = false
        Core.Functions.Notify(Lang:t("error.blips_deactivated"), "error")
    end
end)
...
RegisterNetEvent('admin:client:toggleNames', function()
    if not ShowNames then
        ShowNames = true
        NetCheck2 = true
        Core.Functions.Notify(Lang:t("success.names_activated"), "success")
    else
        ShowNames = false
        Core.Functions.Notify(Lang:t("error.names_deactivated"), "error")
    end
end)
