...
--[[
    Core Admin Menu button functions below.
--]]
...
menu2_admin_display_blips:On('change', function()
    TriggerEvent('admin:client:toggleBlips')
end)
...
-- Weapons list
for _,v in pairs(Core.Shared.Weapons) do
    menu15:AddButton({icon = '🎁',
        ...
        select = function(_)
            ...
            Core.Functions.Notify(Lang:t("success.spawn_weapon"))
        end
    })
end
...
local function OpenPermsMenu(permsply)
    Core.Functions.TriggerCallback('admin:server:getrank', function(rank)
        if rank then
            ...
            menu11:AddButton({
                ...
                select = function(_)
                    if selectedgroup ~= 'Unknown' then
                        ...
                        Core.Functions.Notify(Lang:t("success.changed_perm"), 'success')
                        selectedgroup = 'Unknown'
                    else
                        Core.Functions.Notify(Lang:t("error.changed_perm_failed"), 'error')
                    end
                end
            })
        else
    ...
    menu10:AddButton({
        ...
        select = function(_)
            if kickreason ~= 'Unknown' then
                ...
                Core.Functions.Notify(Lang:t("error.missing_reason"), 'error')
            end
        end
    })
end
    ...
    menu9:AddButton({
        ...
        select = function(_)
            if banreason ~= 'Unknown' and banlength ~= nil then
                ...
                Core.Functions.Notify(Lang:t("error.invalid_reason_length_ban"), 'error')
            end
        end
    })
end
...
player_management:On('select', function(_)
    menu4:ClearItems()
    Core.Functions.TriggerCallback('test:getplayers', function(players)
...
menu3_server_weather:On("select",function()
    ...
    for _, v in ipairs(elements) do
        menu8:AddButton({icon = v.icon,label = v.label,value = v,description = v.description,select = function(btn)
            ...
            TriggerServerEvent('weathersync:server:setWeather', selection.value)
            Core.Functions.Notify(Lang:t("weather.weather_changed", {value = selection.label}))
        end})
    end
end)
...
menu3_server_time:On("select", function(_, value)
    TriggerServerEvent("weathersync:server:setTime", value, value)
    Core.Functions.Notify(Lang:t("time.changed", {time = value}))
end)
...
local function OpenCarModelsMenu(category)
    ...
    for k, v in pairs(category) do
        menu13:AddButton({
            ...
            select = function(_)
                TriggerServerEvent('Core:CallCommand', "car", { k })
            end
        })
    end
end
...
menu5_vehicles_spawn:On('Select', function(_)
    ...
    for k, v in pairs(vehicles) do
        menu12:AddButton({
            label = Core.Shared.FirstToUpper(k),
            value = v,
...
menu5_vehicles_fix:On('Select', function(_)
    TriggerServerEvent('Core:CallCommand', "fix", {})
end)

menu5_vehicles_buy:On('Select', function(_)
    TriggerServerEvent('Core:CallCommand', "admincar", {})
end)

menu5_vehicles_remove:On('Select', function(_)
    TriggerServerEvent('Core:CallCommand', "dv", {})
end)

menu5_vehicles_max_upgrades:On('Select', function(_)
    TriggerServerEvent('Core:CallCommand', "maxmods", {})
end)
...
menu7_dev_info_mode:On('change', function(_, _, _)
    ...
    TriggerEvent('admin:client:ToggleDevmode')
...
local function CopyToClipboard(dataType)
    ...
    if dataType == 'coords2' then
        ...
        Core.Functions.Notify(Lang:t("success.coords_copied"), "success")
    ...
    elseif dataType == 'heading' then
        ...
        Core.Functions.Notify(Lang:t("success.heading_copied"), "success")
    elseif dataType == 'freeaimEntity' then
        ...
        if entity then
            ...
            local x = Core.Shared.Round(entityCoords.x, 2)
            local y = Core.Shared.Round(entityCoords.y, 2)
            local z = Core.Shared.Round(entityCoords.z, 2)
            local rotX = Core.Shared.Round(entityRotation.x, 2)
            local rotY = Core.Shared.Round(entityRotation.y, 2)
            local rotZ = Core.Shared.Round(entityRotation.z, 2)
            local h = Core.Shared.Round(entityHeading, 2)
            SendNUIMessage({
                string = string.format('Model Name:\t%s\nModel Hash:\t%s\n\nHeading:\t%s\nCoords:\t\tvector3(%s, %s, %s)\nRotation:\tvector3(%s, %s, %s)', entityName, entityHash, h, x, y, z, rotX, rotY, rotZ)
            })
            Core.Functions.Notify(Lang:t("success.entity_copy"), "success")
        else
            Core.Functions.Notify(Lang:t("error.failed_entity_copy"), "error")
        end
    end
end
...
local function ToggleShowCoordinates()
    ...
    CreateThread(function()
        while showCoords do
            ...
            local c = {}
            c.x = Core.Shared.Round(coords.x, 2)
            c.y = Core.Shared.Round(coords.y, 2)
            c.z = Core.Shared.Round(coords.z, 2)
            heading = Core.Shared.Round(heading, 2)
            Wait(0)
    ...
    CreateThread(function()
        while vehicleDevMode do
            ...
            if IsPedInAnyVehicle(ped, false) then
                ...
                Draw2DText(string.format(Lang:t("info.eng_health") .. '~b~%s~s~ | ' .. Lang:t("info.body_health") .. '~b~%s~s~', Core.Shared.Round(eHealth, 2), Core.Shared.Round(bHealth, 2)), 4, {255, 255, 255}, 0.4, x + 0.0, y + 0.075)
            end
        end
    end)
end
...
entity_view_distance:On("select", function(_, value)
    ...
    Core.Functions.Notify(Lang:t("info.entity_view_distance", {distance = value}))
end)
    ...
    for _, v in ipairs(elements) do
        EditDealer:AddButton({
            ...
            select = function(btn)
                local values = btn.Value
                if values == "goto" then
                    TriggerServerEvent('Core:CallCommand', "dealergoto", { dealer["name"] })
                elseif values == "remove" then
                    TriggerServerEvent('Core:CallCommand', "deletedealer", { dealer["name"] })
                    EditDealer:Close()
                    menu6:Close()
                end
            end
...
menu1_dealer_list:On('Select', function(_)
    menu6:ClearItems()
    Core.Functions.TriggerCallback('test:getdealers', function(dealers)
