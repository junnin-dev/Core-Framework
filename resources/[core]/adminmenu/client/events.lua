...
RegisterNetEvent('admin:client:inventory', function(targetPed)
...
RegisterNetEvent('admin:client:SendReport', function(name, src, msg)
    TriggerServerEvent('admin:server:SendReport', name, src, msg)
end)

RegisterNetEvent('admin:client:SendStaffChat', function(name, msg)
    TriggerServerEvent('admin:server:Staffchat:addMessage', name, msg)
end)
...
RegisterNetEvent('admin:client:SaveCar', function()
    ...
    if veh ~= nil and veh ~= 0 then
        ...
        if Core.Shared.Vehicles[vehname] ~= nil and next(Core.Shared.Vehicles[vehname]) ~= nil then
            TriggerServerEvent('admin:server:SaveCar', props, Core.Shared.Vehicles[vehname], GetHashKey(veh), plate)
        else
            Core.Functions.Notify(Lang:t("error.no_store_vehicle_garage"), 'error')
        end
    else
...
RegisterNetEvent('admin:client:SetSpeed', function(speed)
    local ped = PlayerId()
    if speed == "fast" then
...
RegisterNetEvent('weapons:client:SetWeaponAmmoManual', function(weapon, ammo)
    ...
    if weapon ~= "current" then
        ...
        Core.Functions.Notify(Lang:t("info.ammoforthe", {value = ammo, weapon = Core.Shared.Weapons[weapon]["label"]}), 'success')
    else
...
RegisterNetEvent('admin:client:GiveNuiFocus', function(focus, mouse)
