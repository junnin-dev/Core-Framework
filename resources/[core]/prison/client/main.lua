Core = exports['core']:GetCoreObject() -- Used Globally
inJail = false
jailTime = 0
currentJob = nil
CellsBlip = nil
...
local function ApplyClothes()
	...
	if DoesEntityExist(playerPed) then
		Citizen.CreateThread(function()
			...
			if gender == 0 then
				TriggerEvent('clothing:client:loadOutfit', Config.Uniforms.male)
			else
				TriggerEvent('clothing:client:loadOutfit', Config.Uniforms.female)
			end
		end)
	end
end
...
RegisterNetEvent('Core:Client:OnPlayerLoaded', function()
	...
	Core.Functions.TriggerCallback('prison:server:IsAlarmActive', function(active)
		if active then
	...
	exports['target']:AddTargetEntity(freedom_ped, {
		options = {
			{
				type = "client",
	...
	exports['target']:AddTargetEntity(canteen_ped, {
		options = {
			{
				type = "client",
...
RegisterNetEvent('prison:client:Enter', function(time)
	...
	if invokingResource and invokingResource ~= 'policejob' and invokingResource ~= 'ambulancejob' and invokingResource ~= GetCurrentResourceName() then
		-- Use Core.Debug here for a quick and easy way to print to the console to grab your attention with this message
		Core.Debug({('Player with source %s tried to execute prison:client:Enter manually or from another resource which is not authorized to call this, invokedResource: %s'):format(GetPlayerServerId(PlayerId()), invokingResource)})
		return
	end

	Core.Functions.Notify( Lang:t("error.injail", {Time = time}), "error")
	...
	Wait(2000)
	DoScreenFadeIn(1000)
	Core.Functions.Notify( Lang:t("error.do_some_work", {currentjob = Config.Jobs[currentJob] }), "error")
end)

RegisterNetEvent('prison:client:Leave', function()
	if jailTime > 0 then
		Core.Functions.Notify( Lang:t("info.timeleft", {JAILTIME = jailTime}))
	else
		jailTime = 0
		...
		RemoveBlip(ShopBlip)
		ShopBlip = nil
		Core.Functions.Notify(Lang:t("success.free_"))
		DoScreenFadeOut(500)
		while not IsScreenFadedOut() do
			Wait(10)
		end
	...
	while true do
		...
		if jailTime > 0 and inJail then
			...
			if jailTime > 0 and inJail then
				...
				if jailTime <= 0 then
					jailTime = 0
					Core.Functions.Notify(Lang:t("success.timesup"), "success", 10000)
				end
				TriggerServerEvent("prison:server:SetJailStatus", jailTime)
			end
		end
	...
	if not Config.UseTarget then
		...
		freedom:onPlayerInOut(function(isPointInside)
			...
			if isPointInside then
				...
				exports['core']:DrawText('[E] Check Time', 'left')
			else
				exports['core']:HideText()
			end
		end)
		...
		canteen:onPlayerInOut(function(isPointInside)
			...
			if isPointInside then
				...
				exports['core']:DrawText('[E] Open Canteen', 'left')
			else
				exports['core']:HideText()
			end
		end)
