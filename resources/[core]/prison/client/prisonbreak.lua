...
RegisterNetEvent('electronickit:UseElectronickit', function()
    if currentGate ~= 0 and not securityLockdown and not Gates[currentGate].hit then
        local hasItem = Core.Functions.HasItem("gatecrack")
        if hasItem then
            TriggerEvent('inventory:client:requiredItems', requiredItems, false)
            Core.Functions.Progressbar("hack_gate", Lang:t("info.connecting_device"), math.random(5000, 10000), false, true, {
            ...
            Core.Functions.Notify(Lang:t("error.item_missing"), "error")
        end
    end
end)
...
RegisterNetEvent('prison:client:PrisonBreakAlert', function()
    ...
    TriggerEvent("phoneclient:addPoliceAlert", alertData)
...
CreateThread(function()
    ...
    requiredItems = {
        ...
        [2] = {name = Core.Shared.Items["gatecrack"]["name"], image = Core.Shared.Items["gatecrack"]["image"]},
    }
    while true do
        inRange = false
        currentGate = 0
...
CreateThread(function()
    while true do
        ...
        if #(pos.xy - vec2(Config.Locations["middle"].coords.x, Config.Locations["middle"].coords.y)) > 200 and inJail then
            ...
            Core.Functions.Notify(Lang:t("error.escaped"), "error")
        end
        Wait(1000)
    end
end)
