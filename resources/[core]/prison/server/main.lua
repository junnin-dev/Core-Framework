...
RegisterNetEvent('prison:server:SetJailStatus', function(jailTime)
    ...
    if jailTime > 0 then
        if Player.PlayerData.job.name ~= "unemployed" then
            ...
            TriggerClientEvent('Core:Notify', src, Lang:t("info.lost_job"))
        end
    else
        GotItems[source] = nil
    end
...
RegisterNetEvent('prison:server:CheckChance', function()
    ...
    TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items['phone'], 'add')
    TriggerClientEvent('Core:Notify', src, Lang:t('success.found_phone'), 'success')
    GotItems[src] = true
end)

Core.Functions.CreateCallback('prison:server:IsAlarmActive', function(_, cb)
    cb(AlarmActivated)
end)
