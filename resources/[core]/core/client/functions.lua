...
function Core.Functions.DrawText(x, y, width, height, scale, r, g, b, a, text)
    SetTextFont(4)
...
function Core.Functions.DrawText3D(x, y, z, text)
...
function Core.Functions.PlayAnim(animDict, animName, upperbodyOnly, duration)
    ...
    Core.Functions.RequestAnimDict(animDict)
...
function Core.Functions.Notify(text, texttype, length)
...
function Core.Functions.TriggerClientCallback(name, cb, ...)
...
function Core.Functions.GetGroundZCoord(coords)
    ...
    if retval then
        ...
        print('Não foi possível encontrar as coordenadas do solo Z dadas as coordenadas 3D')
        print(coords)
        return coords
    end
end