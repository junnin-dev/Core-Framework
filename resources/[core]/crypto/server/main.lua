...
local function RefreshCrypto()
    ...
    if result ~= nil and result[1] ~= nil then
        ...
        if result[1].history ~= nil then
            ...
            TriggerClientEvent('crypto:client:UpdateCryptoWorth', -1, coin, result[1].worth, json.decode(result[1].history))
        else
            TriggerClientEvent('crypto:client:UpdateCryptoWorth', -1, coin, result[1].worth, nil)
        end
    end
end
    ...
    MySQL.update(
        ...
        function(affectedRows)
            if affectedRows < 1 then
                print("Cripto não encontrado, inserindo um novo recorde para " .. coin)
    ...
    if crypto ~= nil then
        if Crypto.Worth[crypto] ~= nil then
            ...
            if NewWorth ~= nil then
                ...
                TriggerClientEvent('Core:Notify', src, "Você mudou o valor de "..Crypto.Labels[crypto].." from: $"..Crypto.Worth[crypto].." to: $"..NewWorth.." ("..ChangeLabel.." "..PercentageChange.."%)")
                ...
                TriggerClientEvent('Core:Notify', src, Lang:t('text.you_have_not_given_a_new_value', {crypto = Crypto.Worth[crypto]}))
            end
        else
            TriggerClientEvent('Core:Notify', src, Lang:t('text.this_crypto_does_not_exist'))
        end
    else
        TriggerClientEvent('Core:Notify', src, Lang:t('text.you_have_not_provided_crypto_available_qbit'))
    end
end, "admin")

Core.Commands.Add("checkcryptoworth", "", {}, false, function(source)
    local src = source
    TriggerClientEvent('Core:Notify', src, Lang:t('text.the_qbit_has_a_value_of', {crypto = Crypto.Worth["qbit"]}))
end)

Core.Commands.Add("crypto", "", {}, false, function(source)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    local MyPocket = math.ceil(Player.PlayerData.money.crypto * Crypto.Worth["qbit"])

    TriggerClientEvent('Core:Notify', src, Lang:t('text.you_have_with_a_value_of',{playerPlayerDataMoneyCrypto = Player.PlayerData.money.crypto,mypocket = MyPocket}))
end)

-- Events

RegisterServerEvent('crypto:server:FetchWorth', function()
...
RegisterServerEvent('crypto:server:ExchangeFail', function()
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    ...
    if ItemData ~= nil then
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items["cryptostick"], "remove")
        TriggerClientEvent('Core:Notify', src, Lang:t('error.cryptostick_malfunctioned'), 'error')
    end
end)

RegisterServerEvent('crypto:server:Rebooting', function(state, percentage)
    Crypto.Exchange.RebootInfo.state = state
    Crypto.Exchange.RebootInfo.percentage = percentage
end)

RegisterServerEvent('crypto:server:GetRebootState', function()
    local src = source
    TriggerClientEvent('crypto:client:GetRebootState', src, Crypto.Exchange.RebootInfo)
end)

RegisterServerEvent('crypto:server:SyncReboot', function()
    TriggerClientEvent('crypto:client:SyncReboot', -1)
end)

RegisterServerEvent('crypto:server:ExchangeSuccess', function(LuckChance)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    local ItemData = Player.Functions.GetItemByName("cryptostick")
    ...
    if ItemData ~= nil then
        ...
        TriggerClientEvent('Core:Notify', src, Lang:t('success.you_have_exchanged_your_cryptostick_for',{amount = Amount}), "success", 3500)
        TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items["cryptostick"], "remove")
        TriggerClientEvent('phoneclient:AddTransaction', src, Player, {},  Lang:t('credit.there_are_amount_credited',{amount = Amount}), "Credit")
    end
end)

-- Callbacks

Core.Functions.CreateCallback('crypto:server:HasSticky', function(source, cb)
...
Core.Functions.CreateCallback('crypto:server:GetCryptoData', function(source, cb, name)
    local Player = Core.Functions.GetPlayer(source)
    local CryptoData = {
...
Core.Functions.CreateCallback('crypto:server:BuyCrypto', function(source, cb, data)
    local Player = Core.Functions.GetPlayer(source)
    ...
    if Player.PlayerData.money.bank >= total_price then
        ...
        TriggerClientEvent('phoneclient:AddTransaction', source, Player, data,  Lang:t('credit.you_have_qbit_purchased',{dataCoins = tonumber(data.Coins)}), "Credit")
...
Core.Functions.CreateCallback('crypto:server:SellCrypto', function(source, cb, data)
    local Player = Core.Functions.GetPlayer(source)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        TriggerClientEvent('phoneclient:AddTransaction', source, Player, data, Lang:t('depreciation.you_have_sold',{dataCoins = tonumber(data.Coins)}), "Depreciation")
...
Core.Functions.CreateCallback('crypto:server:TransferCrypto', function(source, cb, data)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        if result[1] ~= nil then
            ...
            Player.Functions.RemoveMoney('crypto', tonumber(data.Coins))
            TriggerClientEvent('phoneclient:AddTransaction', source, Player, data, "You have "..tonumber(data.Coins).." Qbit('s) transferred!", "Depreciation")
            local Target = Core.Functions.GetPlayerByCitizenId(result[1].citizenid)

            if Target ~= nil then
                Target.Functions.AddMoney('crypto', tonumber(data.Coins))
                TriggerClientEvent('phoneclient:AddTransaction', Target.PlayerData.source, Player, data, "There are "..tonumber(data.Coins).." Qbit('s) credited!", "Credit")
            else
