...
local requiredItems = {[1] = {name = Core.Shared.Items["cryptostick"]["name"], image = Core.Shared.Items["cryptostick"]["image"]}}
				...
				if dist < 1.5 then
					if not Crypto.Exchange.RebootInfo.state then
						...
						if IsControlJustPressed(0, 38) then
							Core.Functions.TriggerCallback('crypto:server:HasSticky', function(HasItem)
								if HasItem then
									TriggerEvent("mhacking:show")
									TriggerEvent("mhacking:start", math.random(4, 6), 45, HackingSuccess)
								else
									Core.Functions.Notify(Lang:t('error.you_dont_have_a_cryptostick'), 'error')
								end
							end)
						end
					else
...
RegisterNetEvent('crypto:client:SyncReboot', function()
...
RegisterNetEvent('crypto:client:UpdateCryptoWorth', function(crypto, amount, history)
...
RegisterNetEvent('crypto:client:GetRebootState', function(RebootInfo)
