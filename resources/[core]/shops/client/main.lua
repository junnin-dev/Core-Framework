...
local function openShop(shop, data)
    ...
    if data.type == "weapon" and Config.FirearmsLicenseCheck then
        if PlayerData.metadata['licenses'] and PlayerData.metadata["licences"].weapon and Core.Functions.HasItem("weaponlicense") then
            ShopItems.items = SetupItems(shop)
            Core.Functions.Notify(Lang:t("success.dealer_verify"), "success")
            Wait(500)
        else
            ShopItems.items = SetupItems(shop, true)
            Core.Functions.Notify(Lang:t("error.dealer_decline"), "error")
            Wait(500)
            Core.Functions.Notify(Lang:t("error.talk_cop"), "error")
            Wait(1000)
        end
    else
        ShopItems.items = SetupItems(shop)
        ...
        while listen do
            if IsControlJustPressed(0, 38) then -- E
                if inChips then
                    ...
                    TriggerServerEvent("shops:server:sellChips")
                else
                    exports["core"]:KeyPressed()
...
local function createPeds()
    ...
    for k, v in pairs(Config.Locations) do
        ...
        if Config.UseTarget then
            exports['target']:AddTargetEntity(ShopPed[k], {
                options = {
                    {
                        label = v["targetLabel"],
                        icon = v["targetIcon"],
    ...
    if Config.UseTarget then
        exports['target']:AddTargetEntity(ShopPed["casino"], {
            options = {
                {
                    label = 'Sell Chips',
...
-- Events
RegisterNetEvent("shops:client:UpdateShop", function(shop, itemData, amount)
...
RegisterNetEvent("shops:client:RestockShopItems", function(shop, amount)
    ...
    CreateThread(function()
        ...
        combo:onPlayerInOut(function(isPointInside, _, zone)
            if isPointInside then
                ...
                exports["core"]:DrawText(Lang:t("info.open_shop"))
                listenForControl()
            else
        ...
        sellChips:onPlayerInOut(function(isPointInside)
            if isPointInside then
                inChips = true
                exports["core"]:DrawText(Lang:t("info.sell_chips"))
            else
                inChips = false
                exports["core"]:HideText()
            end
