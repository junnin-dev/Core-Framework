        ...
        if Config.UseTarget then
            ...
            if current.drivingschool then
                opts = {
                    ...
                    action = function()
                        TriggerServerEvent('cityhall:server:sendDriverTest', Config.DrivingSchools[closestDrivingSchool].instructors)
                    end
                }
                    ...
                    if isLoggedIn and closestCityhall and closestDrivingSchool then
                        if inside then
                            if current.drivingschool then
                                ...
                                exports['core']:DrawText('[E] Take Driving Lessons')
                            elseif current.cityhall then
                                inRangeCityhall = true
                                exports['core']:DrawText('[E] Open Cityhall')
                            end
                        else
...
RegisterNetEvent('cityhall:client:getIds', function()
    TriggerServerEvent('cityhall:server:getIDs')
end)

RegisterNetEvent('cityhall:client:sendDriverEmail', function(charinfo)
...
RegisterNUICallback('close', function(_, cb)
    ...
    if not Config.UseTarget and inRangeCityhall then exports['core']:DrawText('[E] Open Cityhall') end -- Reopen interaction when you're still inside the zone
    cb('ok')
end)
...
RegisterNUICallback('requestId', function(id, cb)
    ...
    if inRangeCityhall and license and id.cost == license.cost then
        TriggerServerEvent('cityhall:server:requestId', id.type, closestCityhall)
        Core.Functions.Notify(('You have received your %s for $%s'):format(license.label, id.cost), 'success', 3500)
    else
        Core.Functions.Notify(Lang:t('error.not_in_range'), 'error')
    end
    cb('ok')
end)
...
RegisterNUICallback('applyJob', function(job, cb)
    if inRangeCityhall then
        TriggerServerEvent('cityhall:server:ApplyJob', job, Config.Cityhalls[closestCityhall].coords)
    else
        Core.Functions.Notify(Lang:t('error.not_in_range'), 'error')
    end
    cb('ok')
...
CreateThread(function()
    initBlips()
    spawnPeds()
    Core.Functions.TriggerCallback('cityhall:server:receiveJobs', function(result)
        SendNUIMessage({
            action = 'setJobs',
            jobs = result
        })
