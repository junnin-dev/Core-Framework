...
RegisterNetEvent('weed:client:getHousePlants', function(house)
    Core.Functions.TriggerCallback('weed:server:getBuildingPlants', function(plants)
        currentHouse = house
        ...
        if not plantSpawned then
            for k, _ in pairs(housePlants[currentHouse]) do
                local plantData = {
                    ...
                    ["plantProp"] = GetHashKey(JNWeed.Plants[housePlants[currentHouse][k].sort]["stages"][housePlants[currentHouse][k].stage]),
                }
    ...
    CreateThread(function()
        if plantSpawned then
            for k, _ in pairs(housePlants[currentHouse]) do
                ...
                for _, stage in pairs(JNWeed.Plants[housePlants[currentHouse][k].sort]["stages"]) do
                ...
                for k, _ in pairs(housePlants[currentHouse]) do
                    ...
                    local plantData = {
                        ...
                        ["plantSort"] = {
                            ...
                            ["label"] = JNWeed.Plants[housePlants[currentHouse][k].sort]["label"],
                        },
                        ["plantStats"] = {
                            ...
                            ["highestStage"] = JNWeed.Plants[housePlants[currentHouse][k].sort]["highestStage"],
                            ["gender"] = gender,
                    ...
                    if plyDistance < 0.8 then
                        ...
                        if plantData["plantStats"]["health"] > 0 then
                            if plantData["plantStage"] ~= plantData["plantStats"]["highestStage"] then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    Core.Functions.Progressbar("remove_weed_plant", Lang:t('text.harvesting_plant'), 8000, false, true, {
                        ...
                        elseif plantData["plantStats"]["health"] == 0 then
                            ...
                            if IsControlJustPressed(0, 38) then
                                Core.Functions.Progressbar("remove_weed_plant", Lang:t('text.removing_the_plant'), 8000, false, true, {
                                    disableMovement = true,
                                    disableCarMovement = true,
                                    disableMouse = false,
                                    disableCombat = true,
                                    ...
                                    ClearPedTasks(ped)
                                    Core.Functions.Notify( Lang:t('error.process_canceled'), "error")
                                end)
                            end
                        end
                    end
...
RegisterNetEvent('weed:client:placePlant', function(type, item)
    ...
    local plantData = {
        ...
        ["plantModel"] = JNWeed.Plants[type]["stages"]["stage-a"],
        ["plantLabel"] = JNWeed.Plants[type]["label"]
    }
    local ClosestPlant = 0
    for _, v in pairs(JNWeed.Props) do
        if ClosestPlant == 0 then
	...
	LocalPlayer.state:set("inv_busy", true, true)
            Core.Functions.Progressbar("plant_weed_plant", Lang:t('text.planting'), 8000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
		...
		LocalPlayer.state:set("inv_busy", false, true)
            ...
            Core.Functions.Notify(Lang:t('error.cant_place_here'), 'error', 3500)
        end
    else
        Core.Functions.Notify(Lang:t('error.not_safe_here'), 'error', 3500)
    end
end)
    ...
    if currentHouse ~= nil then
        if ClosestTarget ~= 0 then
            ...
            local plantData = {
                ...
                ["plantProp"] = GetHashKey(JNWeed.Plants[housePlants[currentHouse][ClosestTarget].sort]["stages"][housePlants[currentHouse][ClosestTarget].stage]),
                ["plantSort"] = {
                    ["name"] = housePlants[currentHouse][ClosestTarget].sort,
                    ["label"] = JNWeed.Plants[housePlants[currentHouse][ClosestTarget].sort]["label"],
                },
                ["plantStats"] = {
                    ...
                    ["highestStage"] = JNWeed.Plants[housePlants[currentHouse][ClosestTarget].sort]["highestStage"],
                    ["gender"] = gender,
            ...
            if plyDistance < 1.0 then
                if plantData["plantStats"]["food"] == 100 then
                    Core.Functions.Notify(Lang:t('error.not_need_nutrition'), 'error', 3500)
                else
		            LocalPlayer.state:set("inv_busy", true, true)
                    Core.Functions.Progressbar("plant_weed_plant", Lang:t('text.feeding_plant'), math.random(4000, 8000), false, true, {
                        disableMovement = true,
                        disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
                ...
                Core.Functions.Notify(Lang:t('error.cant_place_here'), "error")
            end
        else
            Core.Functions.Notify(Lang:t('error.cant_place_here'), "error")
        end
