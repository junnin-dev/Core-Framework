...
Core.Functions.CreateCallback('weed:server:getBuildingPlants', function(_, cb, building)
...
RegisterNetEvent('weed:server:placePlant', function(coords, sort, currentHouse)
...
RegisterNetEvent('weed:server:removeDeathPlant', function(building, plantId)
        ...
        for k, _ in pairs(housePlants) do
            if housePlants[k].health > 50 then
                ...
                elseif housePlants[k].progress + Grow >= 100 then
                    if housePlants[k].stage ~= JNWeed.Plants[housePlants[k].sort]["highestStage"] then
...
Core.Functions.CreateUseableItem("weed_white-widow_seed", function(source, item)
    TriggerClientEvent('weed:client:placePlant', source, 'white-widow', item)
end)

Core.Functions.CreateUseableItem("weed_skunk_seed", function(source, item)
    TriggerClientEvent('weed:client:placePlant', source, 'skunk', item)
end)

Core.Functions.CreateUseableItem("weed_purple-haze_seed", function(source, item)
    TriggerClientEvent('weed:client:placePlant', source, 'purple-haze', item)
end)

Core.Functions.CreateUseableItem("weed_og-kush_seed", function(source, item)
    TriggerClientEvent('weed:client:placePlant', source, 'og-kush', item)
end)

Core.Functions.CreateUseableItem("weed_amnesia_seed", function(source, item)
    TriggerClientEvent('weed:client:placePlant', source, 'amnesia', item)
end)

Core.Functions.CreateUseableItem("weed_ak47_seed", function(source, item)
    TriggerClientEvent('weed:client:placePlant', source, 'ak47', item)
end)

Core.Functions.CreateUseableItem("weed_nutrition", function(source, item)
    TriggerClientEvent('weed:client:foodPlant', source, item)
end)

RegisterServerEvent('weed:server:removeSeed', function(itemslot, seed)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    Player.Functions.RemoveItem(seed, 1, itemslot)
end)

RegisterNetEvent('weed:server:harvestPlant', function(house, amount, plantName, plantId)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
            ...
            if house ~= nil then
                ...
                if result[1] ~= nil then
                    ...
                    MySQL.query('DELETE FROM house_plants WHERE plantid = ? AND building = ?',
                        {plantId, house})
                    TriggerClientEvent('Core:Notify', src,  Lang:t('text.the_plant_has_been_harvested'), 'success', 3500)
                    TriggerClientEvent('weed:client:refreshHousePlants', -1, house)
                else
                    TriggerClientEvent('Core:Notify', src, Lang:t('error.this_plant_no_longer_exists'), 'error', 3500)
                end
            else
                TriggerClientEvent('Core:Notify', src, Lang:t('error.house_not_found'), 'error', 3500)
            end
        else
            TriggerClientEvent('Core:Notify', src, Lang:t('error.you_dont_have_enough_resealable_bags'), 'error', 3500)
        end
    else
        TriggerClientEvent('Core:Notify', src, Lang:t('error.you_Dont_have_enough_resealable_bags'), 'error', 3500)
    end
...
RegisterNetEvent('weed:server:foodPlant', function(house, amount, plantName, plantId)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
