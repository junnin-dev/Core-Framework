...
local function MenuGarage()
    ...
    for k in pairs(Config.Vehicles) do
        truckMenu[#truckMenu+1] = {
            ...
            params = {
                event = "trucker:client:TakeOutVehicle",
                args = {
                    vehicle = k
                }
            }
    ...
    exports['menu']:openMenu(truckMenu)
end
...
local function CreateZone(type, number)
    ...
    if type == "main" then
        event = "truckerjob:client:PaySlip"
        label = "Payslip"
    ...
    elseif type == "vehicle" then
        event = "truckerjob:client:Vehicle"
        label = "Vehicle"
        coords = vector3(Config.Locations[type].coords.x, Config.Locations[type].coords.y, Config.Locations[type].coords.z)
        heading = Config.Locations[type].coords.h
        boxName = Config.Locations[type].label
    ...
    if Config.UseTarget and type == "main" then
        exports['target']:AddBoxZone(boxName, coords, size, size, {
            minZ = coords.z - 5.0,
            maxZ = coords.z + 5.0,
            name = boxName,
            heading = heading,
        ...
        zoneCombo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if type == "main" then
                    TriggerEvent('truckerjob:client:PaySlip')
                elseif type == "vehicle" then
                    TriggerEvent('truckerjob:client:Vehicle')
                elseif type == "stores" then
                    markerLocation = coords
                    Core.Functions.Notify(Lang:t("mission.store_reached"))
                    ShowMarker(true)
                    SetDelivering(true)
                end
            else
...
local function getNewLocation()
    ...
    if location ~= 0 then
        ...
        Core.Functions.Notify(Lang:t("success.payslip_time"))
...
local function GetInTrunk()
    ...
    if IsPedInAnyVehicle(ped, false) then
        return Core.Functions.Notify(Lang:t("error.get_out_vehicle"), "error")
    end
    local pos = GetEntityCoords(ped, true)
    local vehicle = GetVehiclePedIsIn(ped, true)
    if not isTruckerVehicle(vehicle) or CurrentPlate ~= Core.Functions.GetPlate(vehicle) then
        return Core.Functions.Notify(Lang:t("error.vehicle_not_correct"), "error")
    end
    if not BackDoorsOpen(vehicle) then
        return Core.Functions.Notify(Lang:t("error.backdoors_not_open"), "error")
    end
    local trunkpos = GetOffsetFromEntityInWorldCoords(vehicle, 0, -2.5, 0)
    if #(pos - vector3(trunkpos.x, trunkpos.y, trunkpos.z)) > 1.5 then
        return Core.Functions.Notify(Lang:t("error.too_far_from_trunk"), "error")
    end
    if isWorking then return end
    isWorking = true
    Core.Functions.Progressbar("work_carrybox", Lang:t("mission.take_box"), 2000, false, true, {
...
local function Deliver()
    ...
    Core.Functions.Progressbar("work_dropbox", Lang:t("mission.deliver_box"), 2000, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        if currentCount == CurrentLocation.dropcount then
            ...
            TriggerServerEvent("shops:server:RestockShopItems", CurrentLocation.store)
            exports['core']:HideText()
            Delivering = false
            showMarker = false
            TriggerServerEvent('trucker:server:nano')
            if CurrentBlip ~= nil then
                RemoveBlip(CurrentBlip)
                ClearAllBlipRoutes()
                CurrentBlip = nil
            ...
            if JobsDone == Config.MaxDrops then
                Core.Functions.Notify(Lang:t("mission.return_to_station"))
                returnToStation()
            else
                Core.Functions.Notify(Lang:t("mission.goto_next_point"))
                getNewLocation()
            end
        else
            Core.Functions.Notify(Lang:t("mission.another_box"))
        end
    end, function() -- Cancel
        isWorking = false
        ClearPedTasks(PlayerPedId())
...
RegisterNetEvent('trucker:client:SpawnVehicle', function()
...
RegisterNetEvent('trucker:client:TakeOutVehicle', function(data)
    local vehicleInfo = data.vehicle
    TriggerServerEvent('trucker:server:DoBail', true, vehicleInfo)
    selectedVeh = vehicleInfo
end)

RegisterNetEvent('truckerjob:client:Vehicle', function()
    if IsPedInAnyVehicle(PlayerPedId()) and isTruckerVehicle(GetVehiclePedIsIn(PlayerPedId(), false)) then
        if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() then
            if isTruckerVehicle(GetVehiclePedIsIn(PlayerPedId(), false)) then
                ...
                TriggerServerEvent('trucker:server:DoBail', false)
                if CurrentBlip ~= nil then
                    RemoveBlip(CurrentBlip)
                    ClearAllBlipRoutes()
                    CurrentBlip = nil
                ...
                if returningToStation or CurrentLocation then
                    ...
                    Core.Functions.Notify(Lang:t("mission.job_completed"), "success")
                end
            else
                Core.Functions.Notify(Lang:t("error.vehicle_not_correct"), 'error')
            end
        else
            Core.Functions.Notify(Lang:t("error.no_driver"))
        end
    else
        MenuGarage()
    end
...
RegisterNetEvent('truckerjob:client:PaySlip', function()
    if JobsDone > 0 then
        TriggerServerEvent("trucker:server:01101110", JobsDone)
        JobsDone = 0
        ...
        Core.Functions.Notify(Lang:t("error.no_work_done"), "error")
    end
end)
            ...
            if IsControlJustReleased(0, 38) then
                if not hasBox then
                    ...
                    if #(GetEntityCoords(PlayerPedId()) - markerLocation) < 5 then
                        ...
                        Core.Functions.Notify(Lang:t("error.too_far_from_delivery"), "error")
                    end
                end
            end
            sleep = 0
