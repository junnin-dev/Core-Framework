...
AddEventHandler('Core:Client:OnPlayerLoaded', function()
    ...
    Core.Functions.TriggerCallback("weapons:server:GetConfig", function(RepairPoints)
...
RegisterNetEvent("addAttachment", function(component)
    ...
    local WeaponData = Core.Shared.Weapons[weapon]
...
RegisterNetEvent('weapons:client:AddAmmo', function(type, amount, itemData)
    ...
    if CurrentWeaponData then
        if Core.Shared.Weapons[weapon]["name"] ~= "weapon_unarmed" and Core.Shared.Weapons[weapon]["ammotype"] == type:upper() then
            ...
            if total < maxAmmo then
                Core.Functions.Progressbar("taking_bullets", Lang:t('info.loading_bullets'), Config.ReloadTime, false, true, {
                    ...
                    if Core.Shared.Weapons[weapon] then
                        ...
                        TriggerEvent('inventory:client:ItemBox', Core.Shared.Items[itemData.name], "remove")
    ...
    if weapon ~= `WEAPON_UNARMED` then
        ...
        if WeaponAttachments[WeaponData.name] then
            if WeaponAttachments[WeaponData.name][attachment]['item'] == ItemData.name then
                ...
                Core.Functions.Notify(Lang:t('error.no_support_attachment'), "error")
            end
        end
    else
            ...
            if CurrentWeaponData and next(CurrentWeaponData) then
                if IsPedShooting(ped) or IsControlJustPressed(0, 24) then
                    ...
                    if CanShoot then
                        ...
                        if weapon ~= `WEAPON_UNARMED` then
                            TriggerEvent('inventory:client:CheckWeapon', Core.Shared.Weapons[weapon]["name"])
                            ...
                            if CurrentWeaponData and next(CurrentWeaponData) then
                                if not data.RepairingData.Ready then
                                    ...
                                    if IsControlJustPressed(0, 38) then
                                        Core.Functions.TriggerCallback('weapons:server:RepairWeapon', function(HasMoney)
                                            if HasMoney then
                                                CurrentWeaponData = {}
                                            end
