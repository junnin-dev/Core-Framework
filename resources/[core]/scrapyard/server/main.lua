...
RegisterNetEvent('scrapyard:server:LoadVehicleList', function()
    local src = source
    TriggerClientEvent("scapyard:client:setNewVehicles", src, Config.CurrentVehicles)
end)


Core.Functions.CreateCallback('scrapyard:checkOwnerVehicle', function(_, cb, plate)
...
RegisterNetEvent('scrapyard:server:ScrapVehicle', function(listKey)
    local src = source
    ...
    if Config.CurrentVehicles[listKey] ~= nil then
        ...
        if Luck == Odd then
            ...
            TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items["rubber"], 'add')
