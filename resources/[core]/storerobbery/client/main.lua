...
CreateThread(function()
    Wait(1000)
    if Core.Functions.GetPlayerData().job ~= nil and next(Core.Functions.GetPlayerData().job) then
                            ...
                            if IsControlJustPressed(0, 38) then
                                if CurrentCops >= Config.MinimumStoreRobberyPolice then
                                    ...
                                    if Config.Safes[safe].type == "keypad" then
                                        ...
                                        Core.Functions.TriggerCallback('storerobbery:server:getPadlockCombination', function(combination)
                                    ...
                                    Core.Functions.Notify(Lang:t("error.minimum_store_robbery_police", { MinimumStoreRobberyPolice = Config.MinimumStoreRobberyPolice}), "error")
                                end
                            end
                        else
...
function setupRegister()
    Core.Functions.TriggerCallback('storerobbery:server:getRegisterStatus', function(Registers)
...
function setupSafes()
    Core.Functions.TriggerCallback('storerobbery:server:getSafeStatus', function(Safes)
...
RegisterNUICallback('success', function(_, cb)
    if currentRegister ~= 0 then
        ...
        Core.Functions.Progressbar("search_register", Lang:t("text.emptying_the_register"), lockpickTime, false, true, {
...
RegisterNUICallback('fail', function(_ ,cb)
    if usingAdvanced then
        if math.random(1, 100) < 20 then
            TriggerServerEvent("storerobbery:server:removeAdvancedLockpick")
            TriggerEvent('inventory:client:ItemBox', Core.Shared.Items["advancedlockpick"], "remove")
        end
    else
        if math.random(1, 100) < 40 then
            TriggerServerEvent("storerobbery:server:removeLockpick")
    ...
    if (IsWearingHandshoes() and math.random(1, 100) <= 25) then
        local pos = GetEntityCoords(PlayerPedId())
        TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
        Core.Functions.Notify(Lang:t("error.you_broke_the_lock_pick"))
    end
    lockpick(false)
    cb('ok')
end)
...
RegisterNUICallback('TryCombination', function(data, cb)
    Core.Functions.TriggerCallback('storerobbery:server:isCombinationRight', function(combination)
...
RegisterNetEvent('storerobbery:client:setRegisterStatus', function(batch, val)
...
RegisterNetEvent('storerobbery:client:setSafeStatus', function(safe, bool)
    Config.Safes[safe].robbed = bool
end)

RegisterNetEvent('storerobbery:client:robberyCall', function(_, _, _, coords)
