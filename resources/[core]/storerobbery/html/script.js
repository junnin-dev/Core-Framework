...
Padlock.Close = function() {
    ...
    $.post('https://storerobbery/PadLockClose');
}
...
Keypad.Close = function(data) {
    ...
    $.post('https://storerobbery/PadLockClose');
    if (data.error != null) {
        $.post('https://storerobbery/CombinationFail');
    }
}
...
function submitForm(e) {
    $("#keypad").css("display", "none");
    $.post("https://storerobbery/TryCombination", JSON.stringify({
        combination: e.value,
    }));
};
...
findCombo = function(comboArr){
    ...
    for (let i = 0; i < comboArr.length; ++i) {
        ...
        if (angle > (comboArr[i] - numOffset) * tickAngle &&
            ...
            // on unlock
            $.post('https://storerobbery/callcops');
            if (i == comboArr.length - 1) {
                // unlock :)
                $.post('https://storerobbery/PadLockSuccess');
                Padlock.Close();
            }
        }
        break;
...
function unlock() {
    gameOver = true;
    $.post('https://storerobbery/success');
    solveDeg = (Math.random() * 180) - 90
    solvePadding = 4
    maxDistFromSolve = 45
    pinRot = 1
