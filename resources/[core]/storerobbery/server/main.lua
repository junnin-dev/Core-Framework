...
RegisterNetEvent('storerobbery:server:takeMoney', function(register, isDone)
    local src = source
    ...
    if isDone then
        ...
        if math.random(1, 100) <= 10 then
            ...
            TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items["stickynote"], "add")
        end
    end
end)

RegisterNetEvent('storerobbery:server:setRegisterStatus', function(register)
    Config.Registers[register].robbed = true
    Config.Registers[register].time = Config.resetTime
    TriggerClientEvent('storerobbery:client:setRegisterStatus', -1, register, Config.Registers[register])
end)

RegisterNetEvent('storerobbery:server:setSafeStatus', function(safe)
...
RegisterNetEvent('storerobbery:server:SafeReward', function(safe)
    local src = source
	local Player = Core.Functions.GetPlayer(src)
    if not Player then return end
    ...
    if luck <= 10 then
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items["rolex"], "add")
        if luck == odd then
            Wait(500)
            Player.Functions.AddItem("goldbar", 1)
            TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items["goldbar"], "add")
        end
    end
end)

RegisterNetEvent('storerobbery:server:callCops', function(type, safe, streetLabel, coords)
    local cameraId
    if type == "safe" then
...
RegisterNetEvent('storerobbery:server:removeAdvancedLockpick', function()
...
RegisterNetEvent('storerobbery:server:removeLockpick', function()
    local Player = Core.Functions.GetPlayer(source)
...
CreateThread(function()
    while true do
        ...
        if #toSend > 0 then
            ...
            TriggerClientEvent('storerobbery:client:setRegisterStatus', -1, toSend, false)
        end
...
Core.Functions.CreateCallback('storerobbery:server:isCombinationRight', function(_, cb, safe)
    cb(SafeCodes[safe])
end)

Core.Functions.CreateCallback('storerobbery:server:getPadlockCombination', function(_, cb, safe)
    cb(SafeCodes[safe])
end)

Core.Functions.CreateCallback('storerobbery:server:getRegisterStatus', function(_, cb)
    cb(Config.Registers)
end)

Core.Functions.CreateCallback('storerobbery:server:getSafeStatus', function(_, cb)
    cb(Config.Safes)
end)
