...
Core.Functions.CreateCallback('occasions:server:getVehicles', function(_, cb)
...
Core.Functions.CreateCallback("occasions:server:getSellerInformation", function(_, cb, citizenid)
...
Core.Functions.CreateCallback("vehiclesales:server:CheckModelName", function(_, cb, plate)
    if plate then
...
RegisterNetEvent('occasions:server:ReturnVehicle', function(vehicleData)
    local src = source
    ...
    if result[1] then
        if result[1].seller == Player.PlayerData.citizenid then
            ...
            TriggerClientEvent('Core:Notify', src, Lang:t('error.not_your_vehicle'), 'error', 3500)
        end
    else
        TriggerClientEvent('Core:Notify', src, Lang:t('error.vehicle_does_not_exist'), 'error', 3500)
    end
end)

RegisterNetEvent('occasions:server:sellVehicle', function(vehiclePrice, vehicleData)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    ...
    TriggerEvent("log:server:CreateLog", "vehicleshop", "Veículo para venda", "red","**" .. GetPlayerName(src) .. "** tem um " .. vehicleData.model .. " preço de " .. vehiclePrice)
    TriggerClientEvent('occasion:client:refreshVehicles', -1)
end)

RegisterNetEvent('occasions:server:sellVehicleBack', function(vehData)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    local price = 0
    ...
    TriggerClientEvent('Core:Notify', src, Lang:t('success.sold_car_for_price', { value = payout }), 'success', 5500)
    MySQL.query('DELETE FROM player_vehicles WHERE plate = ?', {plate})
end)

RegisterNetEvent('occasions:server:buyVehicle', function(vehicleData)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    local result = MySQL.query.await('SELECT * FROM occasion_vehicles WHERE plate = ? AND occasionid = ?',{vehicleData['plate'], vehicleData["oid"]})
    if result[1] and next(result[1]) then
        if Player.PlayerData.money.bank >= result[1].price then
            ...
            local SellerData = Core.Functions.GetPlayerByCitizenId(SellerCitizenId)
            ...
            TriggerEvent("log:server:CreateLog", "vehicleshop", "bought", "green", "**" .. GetPlayerName(src) .. "** comprou para " .. result[1].price .. " (" .. result[1].plate ..") a partir de **" .. SellerCitizenId .. "**")
            TriggerClientEvent("occasions:client:BuyFinished", src, result[1])
            TriggerClientEvent('occasion:client:refreshVehicles', -1)
            MySQL.query('DELETE FROM occasion_vehicles WHERE plate = ? AND occasionid = ?',{result[1].plate, result[1].occasionid})
            TriggerEvent('phoneserver:sendNewMailToOffline', SellerCitizenId, {
                sender = Lang:t('mail.sender'),
                subject = Lang:t('mail.subject'),
                message = Lang:t('mail.message', { value = NewPrice, value2 = Core.Shared.Vehicles[result[1].model].name})
            })
        else
            TriggerClientEvent('Core:Notify', src, Lang:t('error.not_enough_money'), 'error', 3500)
        end
    end
end)
