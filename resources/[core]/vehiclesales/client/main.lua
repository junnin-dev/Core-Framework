    ...
    if Zone then
        ...
        if vehicles then
            for i = 1, #vehicles, 1 do
                ...
                Core.Functions.SetVehicleProperties(occasionVehicles[Zone][i].car, json.decode(occasionVehicles[Zone][i].mods))
                ...
                if Config.UseTarget then
                    ...
                    EntityZones[i] = exports['target']:AddTargetEntity(occasionVehicles[Zone][i].car, {
                        options = {
                            {
                                type = "client",
                                event = "vehiclesales:client:OpenContract",
                                icon = "fas fa-car",
...
local function despawnOccasionsVehicles()
    ...
    for i = 1, #oSlot, 1 do
        ...
        if oldVehicle then
            Core.Functions.DeleteVehicle(oldVehicle)
        end

        if EntityZones[i] and Config.UseTarget then
            exports['target']:RemoveZone(EntityZones[i])
        end
    end
    EntityZones = {}
end
...
local function openSellContract(bool)
    ...
    SendNUIMessage({
        ...
        plate = Core.Functions.GetPlate(GetVehiclePedIsUsing(PlayerPedId()))
    })
end
...
local function sellVehicleWait(price)
    DoScreenFadeOut(250)
    Wait(250)
    Core.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
    Wait(1500)
    DoScreenFadeIn(250)
    Core.Functions.Notify(Lang:t('success.car_up_for_sale', { value = price }), 'success')
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end

local function SellData(data, model)
    Core.Functions.TriggerCallback("vehiclesales:server:CheckModelName",function(DataReturning)
        local vehicleData = {}
        vehicleData.ent = GetVehiclePedIsUsing(PlayerPedId())
        vehicleData.model = DataReturning
        vehicleData.plate = model
        vehicleData.mods = Core.Functions.GetVehicleProperties(vehicleData.ent)
        vehicleData.desc = data.desc
        TriggerServerEvent('occasions:server:sellVehicle', data.price, vehicleData)
        ...
        while listen do
            if IsControlJustReleased(0, 38) then -- E
                if spot then
                    ...
                    TriggerEvent('vehiclesales:client:OpenContract', data)
                else
                    if IsPedInAnyVehicle(PlayerPedId(), false) then
                        listen = false
                        TriggerEvent('occasions:client:MainMenu')
                        --TriggerEvent('vehiclesales:client:SellVehicle')
                    else
                        Core.Functions.Notify(Lang:t("error.not_in_veh"), "error", 4500)
                    end
                end
            end
            Wait(0)
    ...
    for k, v in pairs(Config.Zones) do
        ...
        SellSpot:onPlayerInOut(function(isPointInside)
            if isPointInside and Zone ~= k then
                Zone = k
                Core.Functions.TriggerCallback('occasions:server:getVehicles', function(vehicles)
...
RegisterNUICallback('sellVehicle', function(data, cb)
    local plate = Core.Functions.GetPlate(GetVehiclePedIsUsing(PlayerPedId())) --Getting the plate and sending to the function
    SellData(data,plate)
    cb('ok')
end)
...
RegisterNUICallback('buyVehicle', function(_, cb)
    TriggerServerEvent('occasions:server:buyVehicle', CurrentVehicle)
    cb('ok')
end)

RegisterNUICallback('takeVehicleBack', function(_, cb)
    TriggerServerEvent('occasions:server:ReturnVehicle', CurrentVehicle)
    cb('ok')
end)

-- Events

RegisterNetEvent('occasions:client:BuyFinished', function(vehdata)
    ...
    Core.Functions.TriggerCallback('Core:Server:SpawnVehicle', function(netId)
        ...
        Core.Functions.Notify(Lang:t('success.vehicle_bought'), "success", 2500)
        TriggerEvent("vehiclekeys:client:SetOwner", vehdata.plate)
        SetVehicleEngineOn(veh, true, true)
        Wait(500)
        Core.Functions.SetVehicleProperties(veh, vehmods)
...
RegisterNetEvent('occasions:client:SellBackCar', function()
    ...
    if IsPedInAnyVehicle(ped, false) then
        ...
        Core.Functions.TriggerCallback('garage:server:checkVehicleOwner', function(owned, balance)
            if owned then
                if balance < 1 then
                    TriggerServerEvent('occasions:server:sellVehicleBack', vehicleData)
                    Core.Functions.DeleteVehicle(vehicle)
                else
                    Core.Functions.Notify(Lang:t('error.finish_payments'), 'error', 3500)
                end
            else
                Core.Functions.Notify(Lang:t('error.not_your_vehicle'), 'error', 3500)
            end
...
RegisterNetEvent('occasions:client:ReturnOwnedVehicle', function(vehdata)
    local vehmods = json.decode(vehdata.mods)
    DoScreenFadeOut(250)
    Wait(500)
    Core.Functions.TriggerCallback('Core:Server:SpawnVehicle', function(netId)
        ...
        TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
        SetVehicleFuelLevel(veh, 100)
        Core.Functions.Notify(Lang:t('info.vehicle_returned'))
        TriggerEvent("vehiclekeys:client:SetOwner", vehdata.plate)
        SetVehicleEngineOn(veh, true, true)
        Wait(500)
        Core.Functions.SetVehicleProperties(veh, vehmods)
...
RegisterNetEvent('occasion:client:refreshVehicles', function()
    if Zone then
        Core.Functions.TriggerCallback('occasions:server:getVehicles', function(vehicles)
            despawnOccasionsVehicles()
            spawnOccasionsVehicles(vehicles)
...
RegisterNetEvent('vehiclesales:client:SellVehicle', function()
    local VehiclePlate = Core.Functions.GetPlate(GetVehiclePedIsIn(PlayerPedId()))
    Core.Functions.TriggerCallback('garage:server:checkVehicleOwner', function(owned, balance)
        if owned then
            if balance < 1 then
                Core.Functions.TriggerCallback('occasions:server:getVehicles', function(vehicles)
                    if vehicles == nil or #vehicles < #Config.Zones[Zone].VehicleSpots then
                        ...
                        Core.Functions.Notify(Lang:t('error.no_space_on_lot'), 'error', 3500)
                    end
                end)
            else
                Core.Functions.Notify(Lang:t('error.finish_payments'), 'error', 3500)
...
RegisterNetEvent('vehiclesales:client:OpenContract', function(data)
    CurrentVehicle = occasionVehicles[Zone][data.Contract]
    if CurrentVehicle then
        Core.Functions.TriggerCallback('occasions:server:getSellerInformation', function(info)
            if info then
        ...
        Core.Functions.Notify(Lang:t("error.not_for_sale"), 'error', 7500)
    end
end)

RegisterNetEvent('occasions:client:MainMenu', function()
    local MainMenu = {
        {
            isMenuHeader = true,
        ...
        },
        {
            ...
            params = {
                event = 'vehiclesales:client:SellVehicle',
            }
        },
        {
            ...
            params = {
                event = 'occasions:client:SellBackCar',
            }
        }
    }

    exports['menu']:openMenu(MainMenu)
end)
    ...
    for k, cars in pairs(Config.Zones) do
        ...
        SpawnZone[k]:onPlayerInOut(function(isPointInside)
            if isPointInside and IsPedInAnyVehicle(PlayerPedId(), false) then
                exports['core']:DrawText(Lang:t("menu.interaction"), 'left')
                TextShown = true
                Listen4Control()
            else
                listen = false
            ...
            for k2, v in pairs(Config.Zones[k].VehicleSpots) do
                ...
                VehicleZones:onPlayerInOut(function(isPointInside)
                    if isPointInside and IsCarSpawned(k2) then
                        exports['core']:DrawText(Lang:t("menu.view_contract_int"), 'left')
                        TextShown = true
                        Listen4Control(k2)
                    else
                        listen = false
