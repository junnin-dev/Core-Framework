...
local function DamageRandomComponent()
	...
	exports['mechanicjob']:SetVehicleStatus(Core.Functions.GetPlate(vehicle), randomComponent, exports['mechanicjob']:GetVehicleStatus(Core.Functions.GetPlate(vehicle), randomComponent) - randomDamage)
end

local function CleanVehicle(veh)
	local ped = PlayerPedId()
	TaskStartScenarioInPlace(ped, "WORLD_HUMAN_MAID_CLEAN", 0, true)
	Core.Functions.Progressbar("cleaning_vehicle", Lang:t("progress.clean_veh"), math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
	}, {}, {}, {}, function() -- Done
		Core.Functions.Notify(Lang:t("success.cleaned_veh"))
		SetVehicleDirtLevel(veh, 0.1)
		SetVehicleUndriveable(veh, false)
		WashDecalsFromVehicle(veh, 1.0)
		TriggerServerEvent('vehiclefailure:server:removewashingkit', veh)
		TriggerEvent('inventory:client:ItemBox', Core.Shared.Items["cleaningkit"], "remove")
		ClearAllPedProps(ped)
		ClearPedTasks(ped)
	end, function() -- Cancel
		Core.Functions.Notify(Lang:t("error.failed_notification"), "error")
		ClearAllPedProps(ped)
		ClearPedTasks(ped)
	end)
end
...
local function RepairVehicleFull(veh)
	...
	Core.Functions.Progressbar("repair_vehicle", Lang:t("progress.repair_veh"), math.random(20000, 30000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		Core.Functions.Notify(Lang:t("success.repaired_veh"))
...
local function RepairVehicle(veh)
	...
	Core.Functions.Progressbar("repair_vehicle", Lang:t("progress.repair_veh"), math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
...
RegisterNetEvent('vehiclefailure:client:RepairVehicle', function()
	local veh = Core.Functions.GetClosestVehicle()
	...
	if veh ~= nil and veh ~= 0 and engineHealth < 500 then
		...
		if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
			...
			if #(pos - vehpos) > 4.9 then
				Core.Functions.Notify(Lang:t("error.out_range_veh"), "error")
			else
				Core.Functions.Notify(Lang:t("error.inside_veh"), "error")
			end
		end
	else
		if veh == nil or veh == 0 then
			Core.Functions.Notify(Lang:t("error.not_near_veh"), "error")
		else
			Core.Functions.Notify(Lang:t("error.healthy_veh"), "error")
		end
	end
end)
...
RegisterNetEvent('iens:repaira', function()
	if isPedDrivingAVehicle() then
		...
		Core.Functions.Notify(Lang:t("error.inside_veh_req"))
	end
end)

RegisterNetEvent('iens:besked', function()
	Core.Functions.Notify(Lang:t("error.roadside_avail"))
end)

RegisterNetEvent('iens:notAllowed', function()
	Core.Functions.Notify(Lang:t("error.no_permission"))
end)
	...
	if isPedDrivingAVehicle() then
		...
		if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				...
				Core.Functions.Notify(Lang:t(('fix_message_%s'):format(fixMessagePos)))
				fixMessagePos = fixMessagePos + 1
				if fixMessagePos > repairCfg.fixMessageCount then fixMessagePos = 1 end
			else
				Core.Functions.Notify(Lang:t("error.veh_damaged"))
			end
		else
			Core.Functions.Notify(Lang:t(('nofix_message_%s'):format(noFixMessagePos)))
					...
					if cfg.sundayDriver and GetVehicleClass(vehicle) ~= 14 then -- Not for boats
						...
						if speed >= 1.0 then
							...
							if brake > 127 then
								...
								--exports['vehicletuning']:SetVehicleStatus(Core.Functions.GetPlate(vehicle), "brakes", exports['vehicletuning']:GetVehicleStatus(Core.Functions.GetPlate(vehicle), "brakes") - 0.01)
							end
