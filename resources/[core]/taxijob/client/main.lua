                ...
                if dist < 20 then
                    ...
                    if dist < 5 then
                        ...
                        if IsControlJustPressed(0, 38) then
                            ...
                            TriggerServerEvent('taxi:server:NpcPay', meterData.currentFare)
                            meterActive = false
                            SendNUIMessage({
                                action = "resetMeter"
                            })
                            Core.Functions.Notify(Lang:t("info.person_was_dropped_off"), 'success')
                            if NpcData.DeliveryBlip ~= nil then
                                RemoveBlip(NpcData.DeliveryBlip)
                            end
...
RegisterNetEvent("taxi:client:TakeVehicle", function(data)
    ...
    if SpawnPoint then
        ...
        if CanSpawn then
            ...
            Core.Functions.Notify(Lang:t("info.no_spawn_point"), "error")
        end
    else
        Core.Functions.Notify(Lang:t("info.no_spawn_point"), 'error')
        return
...
RegisterNetEvent('taxi:client:DoTaxiNpc', function()
    if whitelistedVehicle() then
        if not NpcData.Active then
            ...
            Core.Functions.Notify(Lang:t("info.npc_on_gps"), 'success')
           ...
           if not Config.UseTarget then
                    ...
                    if dist < 20 then
                        ...
                        if dist < 5 then
                            ...
                            if IsControlJustPressed(0, 38) then
                                ...
                                Core.Functions.Notify(Lang:t("info.go_to_location"))
                                if NpcData.NpcBlip ~= nil then
                                    RemoveBlip(NpcData.NpcBlip)
                                end
                                GetDeliveryLocation()
            ...
            Core.Functions.Notify(Lang:t("error.already_mission"))
        end
    else
        Core.Functions.Notify(Lang:t("error.not_in_taxi"))
    end
end)

RegisterNetEvent('taxi:client:toggleMeter', function()
    local ped = PlayerPedId()
    if IsPedInAnyVehicle(ped, false) then
        if whitelistedVehicle() then
            ...
            Core.Functions.Notify(Lang:t("error.missing_meter"), 'error')
        end
    else
        Core.Functions.Notify(Lang:t("error.no_vehicle"), 'error')
    end
end)

RegisterNetEvent('taxi:client:enableMeter', function()
    if meterIsOpen then
        SendNUIMessage({
            action = "toggleMeter"
        })
    else
        Core.Functions.Notify(Lang:t("error.not_active_meter"), 'error')
    end
end)

RegisterNetEvent('taxi:client:toggleMuis', function()
    Wait(400)
    if meterIsOpen then
        ...
        Core.Functions.Notify(Lang:t("error.no_meter_sight"), 'error')
    end
end)
...
RegisterNetEvent('taxijob:client:requestcab', function()
    TaxiGarage()
end)
        ...
        exports['target']:SpawnPed({
            ...
            target = {
                options = {
                    {
                        type = "client",
                        event = "taxijob:client:requestcab",
    ...
    zone:onPlayerInOut(function(isPlayerInside)
        if isPlayerInside then
            if whitelistedVehicle() and not isInsidePickupZone and not NpcData.NpcTaken then
                ...
                exports['core']:DrawText(Lang:t("info.call_npc"), Config.DefaultTextLocation)
                callNpcPoly()
            end
        else
    ...
    delieveryZone:onPlayerInOut(function(isPlayerInside)
        if isPlayerInside then
            if whitelistedVehicle() and not isInsideDropZone and NpcData.NpcTaken then
                ...
                exports['core']:DrawText(Lang:t("info.drop_off_npc"), Config.DefaultTextLocation)
                dropNpcPoly()
            end
        else
...
function setupCabParkingLocation()
    ...
    taxiParking:onPlayerInOut(function(isPlayerInside)
        if isPlayerInside and not Notified and Config.UseTarget then
            if whitelistedVehicle() then
                exports['core']:DrawText(Lang:t("info.vehicle_parking"), Config.DefaultTextLocation)
                Notified = true
        ...
        if isPlayerInsideZone then
            if IsControlJustReleased(0, 38) then
                ...
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    ...
                    if meterIsOpen then
                        TriggerEvent('taxi:client:toggleMeter')
                        meterActive = false
                    end
                    TaskLeaveVehicle(PlayerPedId(), vehicle, 0)
                    Wait(2000) -- 2 second delay just to ensure the player is out of the vehicle
                    DeleteVehicle(vehicle)
                    Core.Functions.Notify(Lang:t("info.taxi_returned"), 'success')
                end
            end
        end
        Wait(1)
...
-- switched boss menu from bossmenu to taxijob
CreateThread(function()
    while true do
        local sleep = 1000
