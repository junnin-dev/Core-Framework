...
CreateThread(function()
    while true do
        Wait(3600000)
        dailyWithdraws = {}
        TriggerClientEvent('Core:Notify', -1, "Redefinição do limite diário de retirada", "success")
    end
end)
...
RegisterCommand('atm', function(source)
    ...
    if visas ~= nil and masters ~= nil then
        for _, v in pairs(visas) do
            ...
            local xCH = Core.Functions.GetPlayerByCitizenId(cardHolder)
            if xCH ~= nil then
    ...
    TriggerClientEvent('atms:client:loadATM', src, cards)
end)
end
-- Event

RegisterNetEvent('atms:server:enteratm',function ()
    local src = source
    local xPlayer = Core.Functions.GetPlayer(src)
    local visas = xPlayer.Functions.GetItemsByName('visa')
    local masters = xPlayer.Functions.GetItemsByName('mastercard')
...
RegisterNetEvent('atms:server:doAccountWithdraw', function(data)
    if data ~= nil then
        local src = source
        local xPlayer = Core.Functions.GetPlayer(src)
...
Core.Functions.CreateCallback('debitcard:server:requestCards', function(source, cb)
    local src = source
    local xPlayer = Core.Functions.GetPlayer(src)
    local visas = xPlayer.Functions.GetItemsByName('visa')
    local masters = xPlayer.Functions.GetItemsByName('mastercard')
...
Core.Functions.CreateCallback('debitcard:server:deleteCard', function(source, cb, data)
...
Core.Functions.CreateCallback('atms:server:loadBankAccount', function(source, cb, cid, cardnumber)
    local src = source
    local xPlayer = Core.Functions.GetPlayer(src)
