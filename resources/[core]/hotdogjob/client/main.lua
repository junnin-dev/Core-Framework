...
local function FinishMinigame(faults)
    ...
    if Config.Stock[Quality].Current + 1 <= Config.Stock[Quality].Max[Config.MyLevel] then
        ...
        if Config.MyLevel == 1 then
            Core.Functions.Notify(Lang:t("success.made_hotdog", {value = Config.Stock[Quality].Label}), "success")
            ...
            if Luck == LuckyNumber then
                Core.Functions.Notify(Lang:t("success.made_luck_hotdog", {value = LuckyAmount, value2 = Config.Stock[Quality].Label}), "success")
        ...
        Core.Functions.Notify(Lang:t("error.no_more", {value = Config.Stock[Quality].Label}), "error")
    end
    PreparingFood = false
end
...
local function StartWorking()
    Core.Functions.TriggerCallback('hotdogjob:server:HasMoney', function(HasMoney)
        if HasMoney then
            if Config.UseTarget then
                ...
                exports['target']:AddTargetEntity(StandObject, {
                    options = {
                        {
                            icon = "fas fa-hand",
                ...
                UpdateUI()
                Core.Functions.Notify(Lang:t("success.deposit", {deposit = Config.StandDeposit}), 'success')
            else
                local SpawnCoords = Config.Locations["spawn"].coords
                IsWorking = true
                LoadModel("prop_hotdogstand_01")
            ...
            Core.Functions.Notify(Lang:t("error.no_money"), 'error')
        end
    end)
end
...
local function StopWorking()
    if DoesEntityExist(StandObject) then
        Core.Functions.TriggerCallback('hotdogjob:server:BringBack', function(DidBail)
            if DidBail then
                DeleteObject(StandObject)
                ClearPedTasksImmediately(PlayerPedId())
                IsWorking = false
                ...
                Core.Functions.Notify(Lang:t("success.deposit_returned", {deposit = Config.StandDeposit}), 'success')
            else
                Core.Functions.Notify(Lang:t("error.deposit_notreturned"), 'error')
            end
        end)
    else
        Core.Functions.Notify(Lang:t("error.no_stand_found"), 'error')
        IsWorking = false
        StandObject = nil
        IsPushing = false
        IsUIActive = false
...
local function SellToPed(ped)
    ...
    while pedDist > OffsetData.Distance do
        ...
        if PlayerDist > 10.0 then
            ...
            Core.Functions.Notify(Lang:t("error.too_far"), 'error')
            break
        end
        Wait(100)
    end
            ...
            if SellingData.Hotdog ~= nil then
                ...
                if Config.UseTarget then
                    if not zoneMade then
                        zoneMade = true
                        exports['target']:AddEntityZone('sellingDogPed', ped, {
                            name = 'sellingDogPed',
                            debugPoly = false,
                        }, {
                            options = {
                                {
                                    ...
                                    action = function(entity)
                                        Core.Functions.Notify(Lang:t("success.sold_hotdogs", {value = HotdogsForSale, value2 = (HotdogsForSale * SellingPrice)}), 'success')
                                        TriggerServerEvent('hotdogjob:server:Sell', pedCoords, HotdogsForSale, SellingPrice)
                                        SellingData.HasTarget = false
                                        LoadAnim('mp_common')
                                        ...
                                        SellingData.Hotdog = nil
                                        exports['target']:RemoveZone('sellingDogPed')
                                        zoneMade = false
                                    end,
                                },
                                {
                                    ...
                                    label = 'Decline offer',
                                    action = function(entity)
                                        Core.Functions.Notify(Lang:t("error.cust_refused"), 'error')
                                        SellingData.HasTarget = false
                                        FreezeEntityPosition(entity, false)
                                        SetPedKeepTask(entity, false)
                                        SetEntityAsNoLongerNeeded(entity)
                ...
                SellingData.HasTarget = false
                SellingData.Hotdog = nil
                Core.Functions.Notify(Lang:t("error.no_dogs"), 'error')
                break
            end
        else
            SellingData.HasTarget = false
...
local function ToggleSell()
    ...
    if StandObject ~= nil then
                ...
                while true do
                    if SellingData.Enabled then
                        ...
                        if not SellingData.HasTarget then
                            ...
                            local closestPed, closestDistance = Core.Functions.GetClosestPed(coords, PlayerPeds)
        ...
        Core.Functions.Notify(Lang:t("error.no_stand"), 'error')
    end
end
...
RegisterNetEvent('hotdogjob:client:UpdateReputation', function(JobRep)
    PlayerData.metadata["jobrep"] = JobRep
    UpdateLevel()
end)

RegisterNetEvent('hotdogjob:client:ToggleSell', function()
...
RegisterNetEvent('hotdogjob:staff:DeletStand', function()
    local ped = PlayerPedId()
    ...
    if Object ~= nil then
        ...
        if ObjectDistance <= 5 then
            ...
            Wait(100)
            DeleteEntity(Object)
            Core.Functions.Notify(Lang:t("info.admin_removed", "primary"))
        end
    end
end)
...
CreateThread(function()
    if Config.UseTarget then
        exports['target']:AddBoxZone('hotdog_start', vector3(Config.Locations["take"].coords.x, Config.Locations["take"].coords.y, Config.Locations["take"].coords.z), 1, 1, {
            name = 'hotdog_start',
            debugPoly = false,
        ...
        hotdogStart:onPlayerInOut(function(isPointInside)
            if isPointInside then
                ...
                if PlayerData.job.name == 'hotdog' then
                    if not IsWorking then
                        exports['core']:DrawText(Lang:t("info.start_working"), 'left')
                        CreateThread(function()
                            while inZone do
                                Wait(0)
                                if IsControlJustPressed(0, 38) then
                        ...
                        exports['core']:DrawText(Lang:t("info.stop_working"), 'left')
                        CreateThread(function()
                            while inZone do
                                Wait(0)
                                if IsControlJustPressed(0, 38) then
