...
local function AddItem(source, item, amount, slot, info)
	...
	if itemInfo['type'] == 'weapon' then
		info.serie = info.serie or tostring(Core.Shared.RandomInt(2) .. Core.Shared.RandomStr(3) .. Core.Shared.RandomInt(1) .. Core.Shared.RandomStr(2) .. Core.Shared.RandomInt(3) .. Core.Shared.RandomStr(4))
	...
	if (totalWeight + (itemInfo['weight'] * amount)) <= Config.MaxInventoryWeight then
		if (slot and Player.PlayerData.items[slot]) and (Player.PlayerData.items[slot].name:lower() == item:lower()) and (itemInfo['type'] == 'item' and not itemInfo['unique']) then
			...
			TriggerEvent('log:server:CreateLog', 'playerinventory', 'AddItem', 'green', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** tenho item: [slot:' .. slot .. '], itemname: ' .. Player.PlayerData.items[slot].name .. ', valor adicionado: ' .. amount .. ', novo valor total: ' .. Player.PlayerData.items[slot].amount)
		...
		elseif itemInfo['unique'] or (not slot or slot == nil) or itemInfo['type'] == 'weapon' then
			for i = 1, Config.MaxInventorySlots, 1 do
				if Player.PlayerData.items[i] == nil then
					...
					TriggerEvent('log:server:CreateLog', 'playerinventory', 'AddItem', 'green', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** got item: [slot:' .. i .. '], itemname: ' .. Player.PlayerData.items[i].name .. ', added amount: ' .. amount .. ', new total amount: ' .. Player.PlayerData.items[i].amount)
	...
	elseif not Player.Offline then
		Core.Functions.Notify(source, "Inventário muito cheio", 'error')
	end
	return false
end
	...
	if slot then
		if Player.PlayerData.items[slot].amount > amount then
			...
			if not Player.Offline then
				TriggerEvent('log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** item perdido: [slot:' .. slot .. '], itemname: ' .. Player.PlayerData.items[slot].name .. ', quantidade removida: ' .. amount .. ', Novo valor total: ' .. Player.PlayerData.items[slot].amount)
			end
		...
		elseif Player.PlayerData.items[slot].amount == amount then
			...
			TriggerEvent('log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** item perdido: [slot:' .. slot .. '], itemname: ' .. item .. ', quantidade removida: ' .. amount .. ', item removido')
		...
		for _, _slot in pairs(slots) do
			if Player.PlayerData.items[_slot].amount > amountToRemove then
				...
				if not Player.Offline then
					TriggerEvent('log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** lost item: [slot:' .. _slot .. '], itemname: ' .. Player.PlayerData.items[_slot].name .. ', quantidade removida: ' .. amount .. ', Novo valor total: ' .. Player.PlayerData.items[_slot].amount)
				end
			...
			elseif Player.PlayerData.items[_slot].amount == amountToRemove then
				...
				TriggerEvent('log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** lost item: [slot:' .. _slot .. '], itemname: ' .. item .. ', quantidade removida: ' .. amount .. ', item removido')
...
local function ClearInventory(source, filterItems)
	...
	TriggerEvent('log:server:CreateLog', 'playerinventory', 'ClearInventory', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** Inventário limpo')
end
...
local function SetInventory(source, items)
	...
	TriggerEvent('log:server:CreateLog', 'playerinventory', 'SetInventory', 'blue', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** itens definidos: ' .. json.encode(items))
end
...
local function CreateUsableItem(itemName, data)
	Core.Functions.CreateUseableItem(itemName, data)
end
...
local function AddToStash(stashId, slot, otherslot, itemName, amount, info)
	...
	if not ItemData.unique then
		if Stashes[stashId].items[slot] and Stashes[stashId].items[slot].name == itemName then
			...
			local itemInfo = Core.Shared.Items[itemName:lower()]
...
local function CreateNewDrop(source, fromSlot, toSlot, itemAmount)
	...
	if RemoveItem(source, itemData.name, itemAmount, itemData.slot) then
		...
		local itemInfo = Core.Shared.Items[itemData.name:lower()]
		...
		TriggerEvent("log:server:CreateLog", "drop", "newItemDrop", "red", "**".. GetPlayerName(source) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..source.."*) lançou um novo item;nome: **"..itemData.name.."**, quantia: **" .. itemAmount .. "**")
		...
		Core.Functions.Notify(source, Lang:t("notify.missitem"), "error")
	end
end
...
AddEventHandler('onResourceStart', function(resourceName)
	...
	local Players = Core.Functions.GetJNPlayers()
...
RegisterNetEvent('inventory:server:combineItem', function(item, fromItem, toItem)
	...
	local recipe = Core.Shared.Items[toItem.name].combinable
	...
	if not ply.state.inv_busy then
		if name and id then
			...
			if name == "stash" then
				if Stashes[id] then
					if Stashes[id].isOpen then
						local Target = Core.Functions.GetPlayer(Stashes[id].isOpen)
						if Target then
			...
			elseif name == "trunk" then
				if Trunks[id] then
					if Trunks[id].isOpen then
						local Target = Core.Functions.GetPlayer(Trunks[id].isOpen)
						if Target then
				...
				if (Trunks[id] and Trunks[id].isOpen) or (Core.Shared.SplitStr(id, "PLZI")[2] and Player.PlayerData.job.name ~= "police") then
					secondInv.name = "none-inv"
			...
			elseif name == "glovebox" then
				if Gloveboxes[id] then
					if Gloveboxes[id].isOpen then
						local Target = Core.Functions.GetPlayer(Gloveboxes[id].isOpen)
						if Target then
			...
			elseif name == "otherplayer" then
				local OtherPlayer = Core.Functions.GetPlayer(tonumber(id))
				if OtherPlayer then
				...
				if Drops[id] then
					if Drops[id].isOpen then
						local Target = Core.Functions.GetPlayer(Drops[id].isOpen)
						if Target then
		...
		Core.Functions.Notify(src, Lang:t("notify.noaccess"), 'error')
	end
end)
...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
	...
	if (fromInventory == "player" or fromInventory == "hotbar") and (Core.Shared.SplitStr(toInventory, "-")[1] == "itemshop" or toInventory == "crafting") then
		return
	end
	...
	if fromInventory == "player" or fromInventory == "hotbar" then
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			elseif Core.Shared.SplitStr(toInventory, "-")[1] == "otherplayer" then
				local playerId = tonumber(Core.Shared.SplitStr(toInventory, "-")[2])
				...
				if toItemData then
					--Player.PlayerData.items[fromSlot] = toItemData
					local itemInfo = Core.Shared.Items[toItemData.name:lower()]
					toAmount = tonumber(toAmount) or toItemData.amount
					if toItemData.name ~= fromItemData.name then
						RemoveItem(playerId, itemInfo["name"], toAmount, fromSlot)
						AddItem(src, toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("log:server:CreateLog", "robbing", "Item trocado", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) item trocado;nome: **"..itemInfo["name"].."**, quantia: **" .. toAmount .. "** com nome: **" .. fromItemData.name .. "**, amount: **" .. fromAmount.. "** com jogador: **".. GetPlayerName(OtherPlayer.PlayerData.source) .. "** (citizenid: *"..OtherPlayer.PlayerData.citizenid.."* | id: *"..OtherPlayer.PlayerData.source.."*)")
					end
				else
					local itemInfo = Core.Shared.Items[fromItemData.name:lower()]
					TriggerEvent("log:server:CreateLog", "robbing", "Item cair", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) lançou um novo item;nome: **"..itemInfo["name"].."**, quantia: **" .. fromAmount .. "** para o jogador: **".. GetPlayerName(OtherPlayer.PlayerData.source) .. "** (citizenid: *"..OtherPlayer.PlayerData.citizenid.."* | id: *"..OtherPlayer.PlayerData.source.."*)")
				end
				local itemInfo = Core.Shared.Items[fromItemData.name:lower()]
				AddItem(playerId, itemInfo["name"], fromAmount, toSlot, fromItemData.info)
			elseif Core.Shared.SplitStr(toInventory, "-")[1] == "trunk" then
				local plate = Core.Shared.SplitStr(toInventory, "-")[2]
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						AddItem(src, toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("log:server:CreateLog", "trunk", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
					end
				else
					local itemInfo = Core.Shared.Items[fromItemData.name:lower()]
					TriggerEvent("log:server:CreateLog", "trunk", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
				end
				local itemInfo = Core.Shared.Items[fromItemData.name:lower()]
				AddToTrunk(plate, toSlot, fromSlot, itemInfo["name"], fromAmount, fromItemData.info)
			elseif Core.Shared.SplitStr(toInventory, "-")[1] == "glovebox" then
				local plate = Core.Shared.SplitStr(toInventory, "-")[2]
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						AddItem(src, toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("log:server:CreateLog", "glovebox", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
					end
				else
					local itemInfo = Core.Shared.Items[fromItemData.name:lower()]
					TriggerEvent("log:server:CreateLog", "glovebox", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
				end
				local itemInfo = Core.Shared.Items[fromItemData.name:lower()]
				AddToGlovebox(plate, toSlot, fromSlot, itemInfo["name"], fromAmount, fromItemData.info)
			elseif Core.Shared.SplitStr(toInventory, "-")[1] == "stash" then
				local stashId = Core.Shared.SplitStr(toInventory, "-")[2]
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						AddItem(src, toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("log:server:CreateLog", "stash", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - stash: *" .. stashId .. "*")
					end
				else
					local itemInfo = Core.Shared.Items[fromItemData.name:lower()]
					TriggerEvent("log:server:CreateLog", "stash", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - stash: *" .. stashId .. "*")
				end
				local itemInfo = Core.Shared.Items[fromItemData.name:lower()]
				AddToStash(stashId, toSlot, fromSlot, itemInfo["name"], fromAmount, fromItemData.info)
			elseif Core.Shared.SplitStr(toInventory, "-")[1] == "traphouse" then
				-- Traphouse
				local traphouseId = Core.Shared.SplitStr(toInventory, "-")[2]
				local toItemData = exports['traphouse']:GetInventoryData(traphouseId, toSlot)
				local IsItemValid = exports['traphouse']:CanItemBeSaled(fromItemData.name:lower())
				if IsItemValid then
					RemoveItem(src, fromItemData.name, fromAmount, fromSlot)
					TriggerClientEvent("inventory:client:CheckWeapon", src, fromItemData.name)
					if toItemData  then
						...
						toAmount = tonumber(toAmount) or toItemData.amount
						if toItemData.name ~= fromItemData.name then
							exports['traphouse']:RemoveHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount)
							AddItem(src, toItemData.name, toAmount, fromSlot, toItemData.info)
							TriggerEvent("log:server:CreateLog", "traphouse", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - traphouse: *" .. traphouseId .. "*")
						end
					else
						local itemInfo = Core.Shared.Items[fromItemData.name:lower()]
						TriggerEvent("log:server:CreateLog", "traphouse", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - traphouse: *" .. traphouseId .. "*")
					end
					local itemInfo = Core.Shared.Items[fromItemData.name:lower()]
					exports['traphouse']:AddHouseItem(traphouseId, toSlot, itemInfo["name"], fromAmount, fromItemData.info, src)
				else
					Core.Functions.Notify(src, Lang:t("notify.nosell"), 'error')
				end
			else
				-- drop
				...
				if toInventory == nil or toInventory == 0 then
					...
					if toItemData then
						...
						if toItemData.name ~= fromItemData.name then
							...
							TriggerEvent("log:server:CreateLog", "drop", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - dropid: *" .. toInventory .. "*")
						end
					else
						local itemInfo = Core.Shared.Items[fromItemData.name:lower()]
						TriggerEvent("log:server:CreateLog", "drop", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - dropid: *" .. toInventory .. "*")
					end
					local itemInfo = Core.Shared.Items[fromItemData.name:lower()]
			...
			Core.Functions.Notify(src, Lang:t("notify.missitem"), "error")
		end
	elseif Core.Shared.SplitStr(fromInventory, "-")[1] == "otherplayer" then
		local playerId = tonumber(Core.Shared.SplitStr(fromInventory, "-")[2])
		local OtherPlayer = Core.Functions.GetPlayer(playerId)
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				itemInfo = Core.Shared.Items[fromItemData.name:lower()]
				AddItem(playerId, itemInfo["name"], fromAmount, toSlot, fromItemData.info)
			end
		else
			Core.Functions.Notify(src, "Item doesn't exist", "error")
		end
	elseif Core.Shared.SplitStr(fromInventory, "-")[1] == "trunk" then
		local plate = Core.Shared.SplitStr(fromInventory, "-")[2]
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("log:server:CreateLog", "trunk", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** plate: *" .. plate .. "*")
					else
						TriggerEvent("log:server:CreateLog", "trunk", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from plate: *" .. plate .. "*")
					end
				else
					TriggerEvent("log:server:CreateLog", "trunk", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** plate: *" .. plate .. "*")
				end
				AddItem(src, fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				local toItemData = Trunks[plate].items[toSlot]
			...
			Core.Functions.Notify(src, Lang:t("notify.itemexist"), "error")
		end
	elseif Core.Shared.SplitStr(fromInventory, "-")[1] == "glovebox" then
		local plate = Core.Shared.SplitStr(fromInventory, "-")[2]
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("log:server:CreateLog", "glovebox", "Swapped", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src..")* swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** plate: *" .. plate .. "*")
					else
						TriggerEvent("log:server:CreateLog", "glovebox", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from plate: *" .. plate .. "*")
					end
				else
					TriggerEvent("log:server:CreateLog", "glovebox", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** plate: *" .. plate .. "*")
				end
				AddItem(src, fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				local toItemData = Gloveboxes[plate].items[toSlot]
	...
	elseif Core.Shared.SplitStr(fromInventory, "-")[1] == "stash" then
		local stashId = Core.Shared.SplitStr(fromInventory, "-")[2]
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("log:server:CreateLog", "stash", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** stash: *" .. stashId .. "*")
					else
						TriggerEvent("log:server:CreateLog", "stash", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from stash: *" .. stashId .. "*")
					end
				else
					TriggerEvent("log:server:CreateLog", "stash", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** stash: *" .. stashId .. "*")
				end
	...
	elseif Core.Shared.SplitStr(fromInventory, "-")[1] == "traphouse" then
		local traphouseId = Core.Shared.SplitStr(fromInventory, "-")[2]
		local fromItemData = exports['traphouse']:GetInventoryData(traphouseId, fromSlot)
		fromAmount = tonumber(fromAmount) or fromItemData.amount
		if fromItemData and fromItemData.amount >= fromAmount then
			local itemInfo = Core.Shared.Items[fromItemData.name:lower()]
			if toInventory == "player" or toInventory == "hotbar" then
				local toItemData = GetItemBySlot(src, toSlot)
				exports['traphouse']:RemoveHouseItem(traphouseId, fromSlot, itemInfo["name"], fromAmount)
				if toItemData then
					itemInfo = Core.Shared.Items[toItemData.name:lower()]
					toAmount = tonumber(toAmount) or toItemData.amount
					if toItemData.name ~= fromItemData.name then
						RemoveItem(src, toItemData.name, toAmount, toSlot)
						exports['traphouse']:AddHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount, toItemData.info, src)
						TriggerEvent("log:server:CreateLog", "stash", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** stash: *" .. traphouseId .. "*")
					else
						TriggerEvent("log:server:CreateLog", "stash", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from stash: *" .. traphouseId .. "*")
					end
				else
					TriggerEvent("log:server:CreateLog", "stash", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** stash: *" .. traphouseId .. "*")
				end
				AddItem(src, fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				local toItemData = exports['traphouse']:GetInventoryData(traphouseId, toSlot)
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						itemInfo = Core.Shared.Items[toItemData.name:lower()]
						exports['traphouse']:RemoveHouseItem(traphouseId, toSlot, itemInfo["name"], toAmount)
						exports['traphouse']:AddHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount, toItemData.info, src)
					end
				end
				itemInfo = Core.Shared.Items[fromItemData.name:lower()]
	...
	elseif Core.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
		local shopType = Core.Shared.SplitStr(fromInventory, "-")[2]
		local itemData = ShopItems[shopType].items[fromSlot]
		local itemInfo = Core.Shared.Items[itemData.name:lower()]
		local bankBalance = Player.PlayerData.money["bank"]
		local price = tonumber((itemData.price*fromAmount))

		if Core.Shared.SplitStr(shopType, "_")[1] == "Dealer" then
			if Core.Shared.SplitStr(itemData.name, "_")[1] == "weapon" then
				price = tonumber(itemData.price)
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					itemData.info.serie = tostring(Core.Shared.RandomInt(2) .. Core.Shared.RandomStr(3) .. Core.Shared.RandomInt(1) .. Core.Shared.RandomStr(2) .. Core.Shared.RandomInt(3) .. Core.Shared.RandomStr(4))
					itemData.info.quality = 100
					AddItem(src, itemData.name, 1, toSlot, itemData.info)
					TriggerClientEvent('drugs:client:updateDealerItems', src, itemData, 1)
					Core.Functions.Notify(src, itemInfo["label"] .. " bought!", "success")
					TriggerEvent("log:server:CreateLog", "dealers", "Dealer item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. " for $"..price)
				else
					Core.Functions.Notify(src, Lang:t("notify.notencash"), "error")
				end
			else
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					AddItem(src, itemData.name, fromAmount, toSlot, itemData.info)
					TriggerClientEvent('drugs:client:updateDealerItems', src, itemData, fromAmount)
					Core.Functions.Notify(src, itemInfo["label"] .. " bought!", "success")
					TriggerEvent("log:server:CreateLog", "dealers", "Dealer item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. "  for $"..price)
				else
					Core.Functions.Notify(src, "You don't have enough cash..", "error")
				end
			end
		elseif Core.Shared.SplitStr(shopType, "_")[1] == "Itemshop" then
			if Player.Functions.RemoveMoney("cash", price, "itemshop-bought-item") then
				...
				AddItem(src, itemData.name, fromAmount, toSlot, itemData.info)
				TriggerClientEvent('shops:client:UpdateShop', src, Core.Shared.SplitStr(shopType, "_")[2], itemData, fromAmount)
				Core.Functions.Notify(src, itemInfo["label"] .. " bought!", "success")
				TriggerEvent("log:server:CreateLog", "shops", "Shop item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. " for $"..price)
	...
	elseif fromInventory == "crafting" then
		...
		if hasCraftItems(src, itemData.costs, fromAmount) then
			...
			Core.Functions.Notify(src, Lang:t("notify.noitem"), "error")
		end
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("log:server:CreateLog", "drop", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** - dropid: *" .. fromInventory .. "*")
					else
						TriggerEvent("log:server:CreateLog", "drop", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** - from dropid: *" .. fromInventory .. "*")
					end
				else
					TriggerEvent("log:server:CreateLog", "drop", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** -  dropid: *" .. fromInventory .. "*")
				end
				AddItem(src, fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				toInventory = tonumber(toInventory)
...
RegisterServerEvent("inventory:server:GiveItem", function(target, name, amount, slot)
    ...
    local OtherPlayer = Core.Functions.GetPlayer(target)
    local dist = #(GetEntityCoords(GetPlayerPed(src))-GetEntityCoords(GetPlayerPed(target)))
	if Player == OtherPlayer then return Core.Functions.Notify(src, Lang:t("notify.gsitem")) end
	if dist > 2 then return Core.Functions.Notify(src, Lang:t("notify.tftgitem")) end
	local item = GetItemBySlot(src, slot)
	if not item then Core.Functions.Notify(src, Lang:t("notify.infound")); return end
	if item.name ~= name then Core.Functions.Notify(src, Lang:t("notify.iifound")); return end
	...
	if amount <= item.amount then
		...
		if RemoveItem(src, item.name, amount, item.slot) then
			if AddItem(target, item.name, amount, false, item.info) then
				TriggerClientEvent('inventory:client:ItemBox',target, Core.Shared.Items[item.name], "add")
				Core.Functions.Notify(target, Lang:t("notify.gitemrec")..amount..' '..item.label..Lang:t("notify.gitemfrom")..Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname)
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, true)
				TriggerClientEvent('inventory:client:ItemBox',src, Core.Shared.Items[item.name], "remove")
				Core.Functions.Notify(src, Lang:t("notify.gitemyg") .. OtherPlayer.PlayerData.charinfo.firstname.." "..OtherPlayer.PlayerData.charinfo.lastname.. " " .. amount .. " " .. item.label .."!")
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, true)
				...
				Core.Functions.Notify(src, Lang:t("notify.gitinvfull"), "error")
				Core.Functions.Notify(target, Lang:t("notify.giymif"), "error")
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, false)
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, false)
			end
		else
			Core.Functions.Notify(src, Lang:t("notify.gitydhei"), "error")
		end
	else
		Core.Functions.Notify(src, Lang:t("notify.gitydhitt"))
	end
end)
...
Core.Functions.CreateCallback('inventory:server:GetStashItems', function(_, cb, stashId)
	cb(GetStashItems(stashId))
end)

Core.Functions.CreateCallback('inventory:server:GetCurrentDrops', function(_, cb)
	cb(Drops)
end)
...
Core.Commands.Add("resetinv", "Redefinir inventário (somente admin)", {{name="type", help="stash/trunk/glovebox"},{name="id/plate", help="ID of stash or license plate"}}, true, function(source, args)
	...
	if invType and invId then
		...
		elseif invType == "stash" then
			...
			Core.Functions.Notify(source,  Lang:t("notify.navt"), "error")
		end
	else
		Core.Functions.Notify(source,  Lang:t("notify.anfoc"), "error")
	end
end, "admin")
	...
	if Player then
			if itemData then
				...
				elseif itemData["name"] == "labkey" then
					info.lab = exports["methlab"]:GenerateRandomLab()
...
Core.Commands.Add("randomitems", "Dê itens aleatórios (somente Deus)", {}, false, function(source, _)
	...
	for _ = 1, 10, 1 do
		...
		if AddItem(source, randitem["name"], amount) then
			TriggerClientEvent('inventory:client:ItemBox', source, Core.Shared.Items[randitem["name"]], 'add')
            Wait(500)
		end
	end
end, "god")
