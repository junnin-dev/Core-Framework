...
local function ShowEntranceHeaderMenu()
    ...
    exports['menu']:showHeader(headerMenu)
end
...
local function EnterApartment(house, apartmentId, new)
    ...
    openHouseAnim()
    Wait(250)
    Core.Functions.TriggerCallback('apartments:GetApartmentOffset', function(offset)
        if offset == nil or offset == 0 then
            Core.Functions.TriggerCallback('apartments:GetApartmentOffsetNewOffset', function(newoffset)
                ...
                local data = exports['interior']:CreateApartmentFurnished(coords)
                Wait(100)
                HouseObj = data[1]
                POIOffsets = data[2]
                InApartment = true
                ...
                Wait(100)
                TriggerServerEvent('apartments:server:SetInsideMeta', house, apartmentId, true, false)
            ...
            TriggerEvent('weathersync:client:DisableSync')
            Wait(100)
            TriggerServerEvent('apartments:server:SetInsideMeta', house, apartmentId, true, true)
            TriggerServerEvent("InteractSound_SV:PlayOnSource", "houses_door_close", 0.1)
            TriggerServerEvent("apartments:server:setCurrentApartment", CurrentApartment)
        end
        ...
        if new ~= nil then
            if new then
                TriggerEvent('interior:client:SetNewState', true)
            else
                TriggerEvent('interior:client:SetNewState', false)
            end
        else
            TriggerEvent('interior:client:SetNewState', false)
        end
...
local function LeaveApartment(house)
    ...
    exports['interior']:DespawnInterior(HouseObj, function()
        ...
        TriggerServerEvent('apartments:server:SetInsideMeta', CurrentApartment, false)
...
local function SetClosestApartment()
    ...
    if current ~= ClosestHouse and LocalPlayer.state.isLoggedIn and not InApartment then
        ...
        Core.Functions.TriggerCallback('apartments:IsOwner', function(result)
            IsOwned = result
            DeleteApartmentsEntranceTargets()
            DeleteInApartmentTargets()
        end, ClosestHouse)
...
function MenuOwners()
    Core.Functions.TriggerCallback('apartments:GetAvailableApartments', function(apartments)
        if next(apartments) == nil then
            Core.Functions.Notify(Lang:t('error.nobody_home'), "error", 3500)
            CloseMenuFull()
        else
            local apartmentMenu = {
                {
            ...
            exports['menu']:openMenu(apartmentMenu)
        end
    end, ClosestHouse)
end
...
RegisterNetEvent('apartments:client:setupSpawnUI', function(cData)
    Core.Functions.TriggerCallback('apartments:GetOwnedApartment', function(result)
        if result then
            TriggerEvent('spawn:client:setupSpawns', cData, false, nil)
            TriggerEvent('spawn:client:openUI', true)
            TriggerEvent("apartments:client:SetHomeBlip", result.type)
        else
            if Apartments.Starting then
                TriggerEvent('spawn:client:setupSpawns', cData, true, Apartments.Locations)
                TriggerEvent('spawn:client:openUI', true)
            else
                TriggerEvent('spawn:client:setupSpawns', cData, false, nil)
                TriggerEvent('spawn:client:openUI', true)
...
RegisterNetEvent('apartments:client:SpawnInApartment', function(apartmentId, apartment)
    ...
    if RangDoorbell ~= nil then
        ...
        if doorbelldist > 5 then
            Core.Functions.Notify(Lang:t('error.to_far_from_door'))
            return
        end
    end
...
RegisterNetEvent('apartments:client:LastLocationHouse', function(apartmentType, apartmentId)
...
RegisterNetEvent('apartments:client:RingDoor', function(player, _)
    ...
    TriggerServerEvent("InteractSound_SV:PlayOnSource", "doorbell", 0.1)
    Core.Functions.Notify(Lang:t('info.at_the_door'))
end)
