...
RegisterNetEvent('apartments:server:SetInsideMeta', function(house, insideId, bool, isVisiting)
    ...
    if bool then
        ...
        Core.Functions.SetPlayerBucket(src, tonumber(routeId))
    else
        ...
        Player.Functions.SetMetaData("inside", insideMeta)
        Core.Functions.SetPlayerBucket(src, 0)
    end
end)
...
RegisterNetEvent('apartments:server:CreateApartment', function(type, label)
    ...
    TriggerClientEvent('Core:Notify', src, Lang:t('success.receive_apart').." ("..label..")")
...
RegisterNetEvent('apartments:server:UpdateApartment', function(type, label)
    ...
    TriggerClientEvent('Core:Notify', src, Lang:t('success.changed_apart'))
    TriggerClientEvent("apartments:client:SetHomeBlip", src, type)
end)
...
Core.Functions.CreateCallback('apartments:GetAvailableApartments', function(_, cb, apartment)
    local apartments = {}
...
Core.Functions.CreateCallback('apartments:GetApartmentOffset', function(_, cb, apartmentId)
    local retval = 0
...
Core.Functions.CreateCallback('apartments:GetApartmentOffsetNewOffset', function(_, cb, apartment)
...
Core.Functions.CreateCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
...
Core.Functions.CreateCallback('apartments:IsOwner', function(source, cb, apartment)
	local src = source
    local Player = Core.Functions.GetPlayer(src)
    if Player ~= nil then
        local result = MySQL.query.await('SELECT * FROM apartments WHERE citizenid = ?', { Player.PlayerData.citizenid })
...
Core.Functions.CreateCallback('apartments:GetOutfits', function(source, cb)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
