...
local function RegisterDutyTarget()
    ...
    if Config.UseTarget then
        exports['target']:AddBoxZone(dutyTargetBoxID, coords, 1.5, 1.5, {
        ...
        zone:onPlayerInOut(function (isPointInside)
            if isPointInside then
                exports['core']:DrawText("[E] " .. label, 'left')
            else
...
local function RegisterStashTarget()
    ...
    if Config.UseTarget then
        exports['target']:AddBoxZone(stashTargetBoxID, coords, 1.5, 1.5, {
            ...
            options = {{
                type = "client",
                event = "mechanicjob:client:target:OpenStash",
        ...
        zone:onPlayerInOut(function (isPointInside)
            if isPointInside then
                exports['core']:DrawText(Lang:t('labels.o_stash'), 'left')
            else
                exports['core']:HideText()
            end
...
local function RegisterGarageZone()
    ...
    vehicleZone:onPlayerInOut(function (isPointInside)
        if isPointInside and onDuty then
            ...
            if inVehicle then
                exports['core']:DrawText(Lang:t('labels.h_vehicle'), 'left')
            else
                exports['core']:DrawText(Lang:t('labels.g_vehicle'), 'left')
            end
        else
            exports['core']:HideText()
        end
    ...
    plateZone:onPlayerInOut(function (isPointInside)
        if isPointInside and onDuty then
            if plate.AttachedVehicle then
                exports['core']:DrawText(Lang:t('labels.o_menu'), 'left')
            else
                if IsPedInAnyVehicle(PlayerPedId()) then
                    exports['core']:DrawText(Lang:t('labels.work_v'), 'left')
                end
            end
        else
            exports['core']:HideText()
...
local function SendStatusMessage(statusList)
    if statusList ~= nil then
        TriggerEvent('chat:addMessage', {
            ...
            args = {Lang:t('labels.veh_status'), round(statusList["engine"]) .. "/" .. Config.MaxStatusValues["engine"] .. " ("..Core.Shared.Items["advancedrepairkit"]["label"]..")", round(statusList["body"]) .. "/" .. Config.MaxStatusValues["body"] .. " ("..Core.Shared.Items[Config.RepairCost["body"]]["label"]..")", round(statusList["radiator"]) .. "/" .. Config.MaxStatusValues["radiator"] .. ".0 ("..Core.Shared.Items[Config.RepairCost["radiator"]]["label"]..")", round(statusList["axle"]) .. "/" .. Config.MaxStatusValues["axle"] .. ".0 ("..Core.Shared.Items[Config.RepairCost["axle"]]["label"]..")", round(statusList["brakes"]) .. "/" .. Config.MaxStatusValues["brakes"] .. ".0 ("..Core.Shared.Items[Config.RepairCost["brakes"]]["label"]..")", round(statusList["clutch"]) .. "/" .. Config.MaxStatusValues["clutch"] .. ".0 ("..Core.Shared.Items[Config.RepairCost["clutch"]]["label"]..")", round(statusList["fuel"]) .. "/" .. Config.MaxStatusValues["fuel"] .. ".0 ("..Core.Shared.Items[Config.RepairCost["fuel"]]["label"]..")"}
        })
    end
end
    ...
    local openMenu = {
        {
            ...
            params = {
                event = "mechanicjob:client:UnattachVehicle",
            }
        }, {
            header = Lang:t('lift_menu.header_stats'),
            txt = Lang:t('lift_menu.desc_stats'),
            params = {
                event = "mechanicjob:client:CheckStatus",
                args = {
                    number = 1,
                }
            }
            ...
            params = {
                event = "mechanicjob:client:PartsMenu",
                args = {
                    number = 1,
                }
            }
            ...
            txt = "",
            params = {
                event = "mechanicjob:client:target:CloseMenu",
            }
        },
    }
    exports['menu']:openMenu(openMenu)
end

local function PartsMenu()
    local plate = Core.Functions.GetPlate(Config.Plates[ClosestPlate].AttachedVehicle)
    if VehicleStatus[plate] ~= nil then
        local vehicleMenu = {
            {
                header = "Status",
        ...
        for k,v in pairs(Config.ValuesLabels) do
            if math.ceil(VehicleStatus[plate][k]) ~= Config.MaxStatusValues[k] then
                ...
                vehicleMenu[#vehicleMenu+1] = {
                    ...
                    params = {
                        event = "mechanicjob:client:PartMenu",
                        args = {
                            name = v,
                            parts = k
                        }
                ...
                vehicleMenu[#vehicleMenu+1] = {
                    ...
                    params = {
                        event = "mechanicjob:client:NoDamage",
                    }
                }
            end
        end
...
local function PartMenu(data)
    ...
    local TestMenu1 = {
        ...
        {
            header = ""..partName.."",
            txt = Lang:t('parts_menu.repair_op')..Core.Shared.Items[Config.RepairCostAmount[part].item]["label"].." "..Config.RepairCostAmount[part].costs.."x",
            params = {
                event = "mechanicjob:client:RepairPart",
                args = {
                    part = part,
                }
            }
    ...
    exports['menu']:openMenu(TestMenu1)
end
...
local function NoDamage()
    ...
    exports['menu']:openMenu(noDamage)
end
...
local function VehicleList()
    ...
    for k,v in pairs(Config.Vehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            ...
            txt = "Vehicle: "..v.."",
            params = {
                event = "mechanicjob:client:SpawnListVehicle",
                args = {
                    headername = v,
                    spawnName = k
                }
...
local function RepairPart(part)
    ...
    local indx = 0
    local countitem = 0
    Core.Functions.TriggerCallback('inventory:server:GetStashItems', function(StashItems)
        ...
        if hasitem and countitem >= PartData.costs then
            ...
            Core.Functions.Progressbar("repair_part", Lang:t('labels.progress_bar') ..Config.ValuesLabels[part], math.random(5000, 10000), false, true, {
                ...
                TriggerServerEvent('inventory:server:SaveStashItems', "mechanicstash", StashItems)
                SetTimeout(250, function()
                    PartsMenu()
                end)
            end, function()
                Core.Functions.Notify(Lang:t('notifications.rep_canceled'), "error")
            end)
        else
            Core.Functions.Notify(Lang:t('notifications.not_materials'), 'error')
        end
    end, "mechanicstash")
end
...
RegisterNetEvent("mechanicjob:client:UnattachVehicle",function()
    UnattachVehicle()
end)

RegisterNetEvent("mechanicjob:client:PartsMenu",function()
    PartsMenu()
end)

RegisterNetEvent("mechanicjob:client:PartMenu",function(data)
    PartMenu(data)
end)

RegisterNetEvent("mechanicjob:client:NoDamage",function()
    NoDamage()
end)

RegisterNetEvent("mechanicjob:client:CheckStatus",function()
    CheckStatus()
end)

RegisterNetEvent("mechanicjob:client:SpawnListVehicle",function(data)
    local vehicleSpawnName = data.spawnName
    SpawnListVehicle(vehicleSpawnName)
end)

RegisterNetEvent("mechanicjob:client:RepairPart",function(data)
...
RegisterNetEvent('Core:Client:OnPlayerLoaded', function()
    Core.Functions.GetPlayerData(function(PlayerData)
        ...
        if PlayerData.job.onduty then
            if PlayerJob.type == 'mechanic' then
                TriggerServerEvent("Core:ToggleDuty")
            end
        end
    end)
    Core.Functions.TriggerCallback('vehicletuning:server:GetAttachedVehicle', function(plates)
        for k, v in pairs(plates) do
            Config.Plates[k].AttachedVehicle = v.AttachedVehicle
        end
    end)

    Core.Functions.TriggerCallback('vehicletuning:server:GetDrivingDistances', function(retval)
...
RegisterNetEvent('Core:Client:SetDuty', function(duty)
    onDuty = duty
...
RegisterNetEvent('vehicletuning:client:SetAttachedVehicle', function(veh, key)
    if veh ~= false then
...
RegisterNetEvent('vehicletuning:client:RepaireeePart', function(part)
    ...
    Core.Functions.Notify(Lang:t('notifications.partrep', {value = Config.ValuesLabels[part]}))
end)
            ...
            if #(pos - vehpos) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil then
                        ...
                        Core.Functions.Notify(Lang:t('notifications.uknown'), "error")
                    end
                else
                    Core.Functions.Notify(Lang:t('notifications.not_valid'), "error")
                end
            else
                Core.Functions.Notify(Lang:t('notifications.not_close'), "error")
            end
        else
            Core.Functions.Notify(Lang:t('notifications.veh_first'), "error")
        end
    else
        Core.Functions.Notify(Lang:t('notifications.outside'), "error")
    end
end)
...
RegisterNetEvent('vehiclemod:client:fixEverything', function()
    if (IsPedInAnyVehicle(PlayerPedId(), false)) then
        ...
        if not IsThisModelABicycle(GetEntityModel(veh)) and GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
            ...
            Core.Functions.Notify(Lang:t('notifications.wrong_seat'), "error")
        end
    else
        Core.Functions.Notify(Lang:t('notifications.not_vehicle'), "error")
    end
end)
            ...
            if #(pos - vehpos) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                        ...
                        Core.Functions.Progressbar("repair_advanced", Lang:t('notifications.progress_bar'), lockpickTime, false, true, {
                            disableMovement = true,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true,
                            ...
                            if part == "body" then
                                ...
                                TriggerServerEvent("mechanicjob:server:removePart", part, needAmount)
                                TriggerEvent("inventory:client:ItemBox", Core.Shared.Items[Config.RepairCost[part]], "remove")
                            elseif part ~= "engine" then
                                TriggerServerEvent("vehiclemod:server:updatePart", plate, part, GetVehicleStatus(plate, part) + level)
                                TriggerServerEvent("mechanicjob:server:removePart", part, level)
                                TriggerEvent("inventory:client:ItemBox", Core.Shared.Items[Config.RepairCost[part]], "remove")
                            end
                        end, function() -- Cancel
                            openingDoor = false
                            ClearPedTasks(PlayerPedId())
                            Core.Functions.Notify(Lang:t('notifications.process_canceled'), "error")
                        end)
                    else
                        Core.Functions.Notify(Lang:t('notifications.not_part'), "error")
                    end
                else
                    Core.Functions.Notify(Lang:t('notifications.not_valid'), "error")
                end
...
RegisterNetEvent('mechanicjob:client:target:OpenStash', function ()
...
RegisterNetEvent('mechanicjob:client:target:CloseMenu', function()
    DestroyVehiclePlateZone(ClosestPlate)
    RegisterVehiclePlateZone(ClosestPlate, Config.Plates[ClosestPlate])
        ...
        if PlayerJob.type == 'mechanic' then
            ...
            if onDuty then
                if isInsideStashZone then
                    wait = 0
                    if IsControlJustPressed(0, 38) then
                        TriggerEvent("mechanicjob:client:target:OpenStash")
                    end
                end
                ...
                if isInsideVehiclePlateZone then
                    ...
                    if attachedVehicle then
                        ...
                        if IsControlJustPressed(0, 38) and IsPedInAnyVehicle(PlayerPedId()) then
                            ...
                            Wait(500)
                            DoScreenFadeIn(150)
                            TriggerServerEvent('vehicletuning:server:SetAttachedVehicle', veh, ClosestPlate)
