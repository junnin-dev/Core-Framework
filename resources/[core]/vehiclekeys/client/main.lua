...
function GetKeys()
    Core.Functions.TriggerCallback('vehiclekeys:server:GetVehicleKeys', function(keysList)
        KeysList = keysList
    end)
end
...
function LockpickFinishCallback(success)
    ...
    if success then
        ...
        if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
            TriggerServerEvent('vehiclekeys:server:AcquireVehicleKeys', Core.Functions.GetPlate(vehicle))
        else
...
function CarjackVehicle(target)
    ...
    Core.Functions.Progressbar("rob_keys", Lang:t("progress.acjack"), Config.CarjackingTime, false, true, {}, {}, {}, {}, function()
