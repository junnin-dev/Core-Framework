        ...
        if itemName == "markedbills" then
            SetTimeout(math.random(1000, 5000), function()
                if Config.TrapHouses[traphouseId].inventory[slot] ~= nil then
                    ...
                    TriggerClientEvent('traphouse:client:SyncData', -1, traphouseId, Config.TrapHouses[traphouseId])
                end
            end)
        else
            for _ = 1, amount, 1 do
...
RegisterServerEvent('traphouse:server:TakeoverHouse', function(Traphouse)
    local src = source
    ...
    if not HasCitizenIdHasKey(CitizenId, Traphouse) then
        if Player.Functions.RemoveMoney('cash', Config.TakeoverPrice) then
            TriggerClientEvent('traphouse:client:TakeoverHouse', src, Traphouse)
        else
...
RegisterServerEvent('traphouse:server:AddHouseKeyHolder', function(CitizenId, TraphouseId, IsOwner)
    local src = source
    ...
    if Config.TrapHouses[TraphouseId] ~= nil then
        ...
        if Config.TrapHouses[TraphouseId].keyholders == nil then
            ...
            if #Config.TrapHouses[TraphouseId].keyholders + 1 <= 6 then
                ...
                TriggerClientEvent('Core:Notify', src, Lang:t("error.no_slots"))
            end
        end
    else
        TriggerClientEvent('Core:Notify', src, Lang:t("error.occured"))
    end
end)

RegisterServerEvent('traphouse:server:TakeMoney', function(TraphouseId)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    if Config.TrapHouses[TraphouseId].money ~= 0 then
        ...
        TriggerClientEvent('Core:Notify', src, Lang:t("error.no_money"), 'error')
    end
end)

RegisterServerEvent('traphouse:server:RobNpc', function(Traphouse)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
...
Core.Commands.Add("multikeys", Lang:t("info.give_keys"), {{name = "id", help = "Player id"}}, true, function(source, args)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
            ...
            if not HasCitizenIdHasKey(TargetData.PlayerData.citizenid, Traphouse) then
                if Config.TrapHouses[Traphouse] ~= nil then
                    ...
                    if Config.TrapHouses[Traphouse].keyholders == nil then
                        ...
                        TriggerClientEvent('traphouse:client:SyncData', -1, Traphouse, Config.TrapHouses[Traphouse])
                    else
                ...
                TriggerClientEvent('Core:Notify', src, Lang:t("error.have_keys"), 'error')
            end
        else
            TriggerClientEvent('Core:Notify', src, Lang:t("error.not_owner"), 'error')
        end
    else
