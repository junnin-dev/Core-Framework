...
local function RegisterTraphouseEntranceTarget(traphouseID, traphouseData)
    ...
    exports['target']:AddBoxZone(boxName, coords, boxData.length, boxData.width, {
        name = boxName,
        ...
        options = {
            {
                type = 'client',
                event = 'traphouse:client:EnterTraphouse',
...
local function RegisterTraphouseEntranceZone(traphouseID, traphouseData)
    ...
    zone:onPlayerInOut(function (isPointInside)
        if isPointInside then
            exports['core']:DrawText('[E] ' .. Lang:t('targetInfo.enter'), 'left')
        else
...
local function RegisterTraphouseInteractionZone(traphouseID, traphouseData)
    ...
    zone:onPlayerInOut(function (isPointInside)
        if isPointInside then
            exports['core']:DrawText('[E] ' .. Lang:t('targetInfo.options'), 'left')
        else
            exports['core']:HideText()
            TriggerEvent('traphouse:client:target:CloseMenu')
        end
...
local function RegisterTraphouseInteractionTarget(traphouseID, traphouseData)
    ...
    local options = {
        {
            type = "client",
            event = "traphouse:client:target:TakeOver",
    ...
    if IsKeyHolder then
        options = {
            {
                type = "client",
                event = "traphouse:client:target:ViewInventory",
                label = Lang:t("targetInfo.inventory"),
                traphouseData = traphouseData
            },
            {
                type = "client",
                event = "traphouse:client:target:TakeMoney",
        ...
        if IsHouseOwner then
            options[#options+1] = {
                type = "client",
                event = "traphouse:client:target:SeePinCode",
...
local function RegisterTraphouseExitZone(coords, traphouseID, traphouseData)
    ...
    zone:onPlayerInOut(function (isPointInside)
        if isPointInside then
            exports['core']:DrawText('[E] ' .. Lang:t("targetInfo.leave"), 'left')
        else
            exports['core']:HideText()
        end
...
local function RegisterTraphouseExitTarget(coords, traphouseID, traphouseData)
    ...
    exports['target']:AddBoxZone(boxName, coords, boxData.length, boxData.width, {
        ...
        options = {
            {
                type = "client",
                event = "traphouse:client:target:ExitTraphouse",
...
local function OpenHeaderMenu(data)
    ...
    headerMenu[#headerMenu+1] = {
        ...
        params = {
            event = "traphouse:client:target:CloseMenu",
        }
    }
...
local function EnterTraphouse(data)
    ...
    data = exports['interior']:CreateTrevorsShell(coords)
    TraphouseObj = data[1]
    POIOffsets = data[2]
...
local function LeaveTraphouse(k, data)
    ...
    DoScreenFadeOut(250)
    Wait(250)
    exports['interior']:DespawnInterior(TraphouseObj, function()
    ...
    if Config.UseTarget then
        exports['target']:RemoveZone('traphouseInteraction' .. k)
        data.polyzoneBoxData['interaction'].created = false

        exports['target']:RemoveZone('traphouseExit' .. k)
...
RegisterNetEvent('traphouse:client:EnterTraphouse', function()
    if ClosestTraphouse ~= nil then
...
RegisterNetEvent('traphouse:client:TakeoverHouse', function(TraphouseId)
    Core.Functions.Progressbar("takeover_traphouse", Lang:t("info.taking_over"), math.random(1000, 3000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent('traphouse:server:AddHouseKeyHolder', PlayerData.citizenid, TraphouseId, true)
    end, function()
        Core.Functions.Notify(Lang:t("error.cancelled"), "error")
    end)
end)

RegisterNetEvent('traphouse:client:target:ViewInventory', function (data)
...
RegisterNetEvent('traphouse:client:target:TakeOver', function ()
    TriggerServerEvent('traphouse:server:TakeoverHouse', CurrentTraphouse)
end)

RegisterNetEvent('traphouse:client:target:TakeMoney', function ()
    TriggerServerEvent("traphouse:server:TakeMoney", CurrentTraphouse)
end)

RegisterNetEvent('traphouse:client:target:SeePinCode', function (data)
    Core.Functions.Notify(Lang:t('info.pin_code', { value = data.traphouseData.pincode }))
end)

RegisterNetEvent('traphouse:client:target:ExitTraphouse', function (data)
    LeaveTraphouse(data.traphouseID, Config.TrapHouses[data.traphouseID])
end)

RegisterNetEvent('traphouse:client:SyncData', function(k, data)
...
RegisterNetEvent('traphouse:client:target:CloseMenu', function ()
...
RegisterNUICallback('ErrorMessage', function(data, cb)
    Core.Functions.Notify(data.message, 'error')
    cb('ok')
end)
...
RegisterNUICallback('EnterPincode', function(d, cb)
    ...
    if tonumber(d.pin) == data.pincode then
        ...
        Core.Functions.Notify(Lang:t("error.incorrect_code"), 'error')
    end
    cb('ok')
end)
                                    ...
                                    if not IsEntityDead(targetPed) then
                                        if CanRob then
                                            if InDistance then
                                                ...
                                                TriggerServerEvent('traphouse:server:RobNpc', ClosestTraphouse)
                                                CanRob = false
                                            end
                                        end
                                    end
...
CreateThread(function ()
    ...
    if Core.Functions.GetPlayerData() ~= nil then
        PlayerData = Core.Functions.GetPlayerData()
    end
            ...
            if not InsideTraphouse then
                if isInsideEntranceTarget then
                    ...
                    if IsControlJustPressed(0, 38) then
                        TriggerEvent("traphouse:client:EnterTraphouse")
                        exports['core']:HideText()
                    end
                end
            else
