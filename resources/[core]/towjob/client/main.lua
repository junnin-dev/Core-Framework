...
local function MenuGarage()
    ...
    for k in pairs(Config.Vehicles) do
        towMenu[#towMenu+1] = {
            ...
            params = {
                event = "tow:client:TakeOutVehicle",
                args = {
                    vehicle = k
                }
            }
...
local function CreateZone(type, number)
    ...
    elseif type == "towspots" then
        event = "tow:client:SpawnNPCVehicle"
    ...
    if Config.UseTarget and type == "main" then
        ...
        zoneCombo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if type == "main" then
                    TriggerEvent('tow:client:PaySlip')
                elseif type == "vehicle" then
                    TriggerEvent('tow:client:Vehicle')
                elseif type == "towspots" then
                    TriggerEvent('tow:client:SpawnNPCVehicle')
                end
            end
        end)
        if type == "vehicle" then
            ...
            zoneComboV:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    TriggerEvent('tow:client:ShowMarker', true)
                else
                    TriggerEvent('tow:client:ShowMarker', false)
                end
            end)
        elseif type == "towspots" then
...
local function deliverVehicle(vehicle)
    ...
    Core.Functions.Notify(Lang:t("mission.delivered_vehicle"), "success")
    Core.Functions.Notify(Lang:t("mission.get_new_vehicle"))
...
RegisterNetEvent('tow:client:SpawnVehicle', function()
...
RegisterNetEvent('jobs:client:ToggleNpc', function()
    if Core.Functions.GetPlayerData().job.name == "tow" then
        if CurrentTow ~= nil then
            Core.Functions.Notify(Lang:t("error.finish_work"), "error")
            return
        end
        NpcOn = not NpcOn
        if NpcOn then
...
RegisterNetEvent('tow:client:TowVehicle', function()
    ...
    if isTowVehicle(vehicle) then
            ...
            if not IsPedInAnyVehicle(PlayerPedId()) then
                if vehicle ~= targetVehicle then
                    ...
                    if #(towPos - targetPos) < 11.0 then
                        Core.Functions.Progressbar("towing_vehicle", Lang:t("mission.towing_vehicle"), 5000, false, true, {
                            ...
                            if NpcOn then
                                RemoveBlip(CurrentBlip)
                                Core.Functions.Notify(Lang:t("mission.goto_depot"), "primary", 5000)
                                ...
                                TriggerServerEvent('tow:server:nano', vehNetID)
                                --remove zone
                                CurrentLocation.zoneCombo:destroy()
                            end
                            Core.Functions.Notify(Lang:t("mission.vehicle_towed"), "success")
                        end, function() -- Cancel
                            StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
                            Core.Functions.Notify(Lang:t("error.failed"), "error")
                        end)
                    end
                end
            end
        else
            Core.Functions.Progressbar("untowing_vehicle", Lang:t("mission.untowing_vehicle"), 5000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
                ...
                CurrentTow = nil
                drawDropOff = false
                Core.Functions.Notify(Lang:t("mission.vehicle_takenoff"), "success")
            end, function() -- Cancel
                StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
                Core.Functions.Notify(Lang:t("error.failed"), "error")
            end)
        ...
        Core.Functions.Notify(Lang:t("error.not_towing_vehicle"), "error")
    end
end)

RegisterNetEvent('tow:client:TakeOutVehicle', function(data)
    local coords = Config.Locations["vehicle"].coords
    ...
    if #(pos - coords) <= 5 then
        ...
        TriggerServerEvent('tow:server:DoBail', true, vehicleInfo)
        selectedVeh = vehicleInfo
    else
        Core.Functions.Notify(Lang:t("error.too_far_away"), 'error')
    end
end)

RegisterNetEvent('tow:client:Vehicle', function()
    local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
    if not CurrentTow then
        if vehicle and isTowVehicle(vehicle) then
            DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
            TriggerServerEvent('tow:server:DoBail', false)
        else
            MenuGarage()
        end
    else
...
RegisterNetEvent('tow:client:PaySlip', function()
    if JobsDone > 0 then
        RemoveBlip(CurrentBlip)
        TriggerServerEvent("tow:server:11101110", JobsDone)
        JobsDone = 0
        NpcOn = false
    else
...
RegisterNetEvent('tow:client:SpawnNPCVehicle', function()
...
RegisterNetEvent('tow:client:ShowMarker', function(active)
    if PlayerJob.name == "tow" then
        showMarker = active
    end
end)
