...
local function RobberyPed()
    if Config.UseTarget then
        exports['target']:AddEntityZone('stealingPed', stealingPed, {
            name = 'stealingPed',
            debugPoly = false,
        }, {
            options = {
                {
                    ...
                    action = function()
                        ...
                        TriggerServerEvent('drugs:server:giveStealItems', stealData.drugType, stealData.amount)
                        TriggerEvent('inventory:client:ItemBox', Core.Shared.Items[stealData.item], "add")
                        stealingPed = nil
                        stealData = {}
                        exports['target']:RemoveZone('stealingPed')
                    end,
                ...
                if IsEntityDead(stealingPed) then
                    ...
                    if not Config.UseTarget and #(pos - pedpos) < 1.5 then
                        if not textDrawn then
                            textDrawn = true
                            exports['core']:DrawText(Lang:t("info.pick_up_button"))
                        end
...
local function SellToPed(ped)
    ...
    if hasTarget then
        while pedDist < 1.5 and not IsPedDeadOrDying(ped) do
            ...
            if getRobbed <= Config.RobberyChance then
                TriggerServerEvent('drugs:server:robCornerDrugs', drugType, bagAmount)
                Core.Functions.Notify(Lang:t("info.has_been_robbed", {bags = bagAmount, drugType = availableDrugs[drugType].label}))
                stealingPed = ped
                stealData = {
                ...
                if pedDist2 < 1.5 and cornerselling then
                    if Config.UseTarget and not zoneMade then
                        zoneMade = true
                        exports['target']:AddEntityZone('sellingPed', ped, {
                            name = 'sellingPed',
                            debugPoly = false,
                        }, {
                            options = {
					    ...
					    Core.Functions.Notify(Lang:t("error.in_vehicle"), 'error')
					    hasTarget = false
                                            SetPedKeepTask(entity, false)
                                            SetEntityAsNoLongerNeeded(entity)
                                            ClearPedTasksImmediately(entity)
                                            lastPed[#lastPed + 1] = entity
                                            exports['target']:RemoveZone('sellingPed')
					    return
					else
                                            Core.Functions.Progressbar("cornerSelling", Lang:t("info.selling_to_ped"), '5000', false, false, {
					    disableMovement = true,
				            disableCarMovement = true,
					    disableMouse = false,
					    disableCombat = false,
					    }, {}, {}, {}, function()
                                                TriggerServerEvent('drugs:server:sellCornerDrugs', drugType, bagAmount, randomPrice)
                                                hasTarget = false
                                ...
                                },
                                {
                                    ...
                                    label = 'Decline offer',
                                    action = function(entity)
                                        Core.Functions.Notify(Lang:t("error.offer_declined"), 'error')
                                        hasTarget = false
                                        SetPedKeepTask(entity, false)
                                        SetEntityAsNoLongerNeeded(entity)
                                        ClearPedTasksImmediately(entity)
                    ...
                    elseif not Config.UseTarget then
                        if not textDrawn then
                            textDrawn = true
                            exports['core']:DrawText(Lang:t("info.drug_offer", {bags = bagAmount, drugLabel = currentOfferDrug.label, randomPrice = randomPrice}))
                        end
...
local function ToggleSelling()
    if not cornerselling then
        ...
        Core.Functions.Notify(Lang:t("info.started_selling_drugs"))
        ...
        cornerselling = false
        LocalPlayer.state:set("inv_busy", false, true)
        Core.Functions.Notify(Lang:t("info.stopped_selling_drugs"))
    end
end

-- Events
RegisterNetEvent('drugs:client:cornerselling', function()
    Core.Functions.TriggerCallback('drugs:server:cornerselling:getAvailableDrugs', function(result)
	    ...
	    if IsPedInAnyVehicle(PlayerPedId(), false) then
	        Core.Functions.Notify(Lang:t("error.in_vehicle"), 'error')
                ...
                if result then
                    ...
                    Core.Functions.Notify(Lang:t("error.has_no_drugs"), 'error')
                    LocalPlayer.state:set("inv_busy", false, true)
                end
	    end
        else
            Core.Functions.Notify(Lang:t("error.not_enough_police", {polices = Config.MinimumDrugSalePolice}), "error")
        end
    end)
end)
...
RegisterNetEvent('drugs:client:refreshAvailableDrugs', function(items)
    availableDrugs = items
    if #availableDrugs <= 0 then
        Core.Functions.Notify(Lang:t("error.no_drugs_left"), 'error')
        cornerselling = false
        LocalPlayer.state:set("inv_busy", false, true)
    end
end)
