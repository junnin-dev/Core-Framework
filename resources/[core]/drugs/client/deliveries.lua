...
AddStateBagChangeHandler('isLoggedIn', nil, function(_, _, value)
    if value then
        Core.Functions.TriggerCallback('drugs:server:RequestConfig', function(DealerConfig)
...
local function OpenDealerShop()
    ...
    for k, _ in pairs(Config.Dealers[currentDealer]["products"]) do
        if Core.Functions.GetPlayerData().metadata["dealerrep"] >= Config.Dealers[currentDealer]["products"][k].minrep then
...
local function KnockDoorAnim(home)
    ...
    local myData = Core.Functions.GetPlayerData()
    if home then
        ...
        exports['core']:DrawText(Lang:t("info.other_dealers_button"), 'left')
        AwaitingInput()
    else
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "knock_door", 0.2)
        Wait(100)
        ...
        TaskPlayAnim(PlayerPed, knockAnimLib, "exit", 3.0, 3.0, -1, 1, 0, false, false, false)
        Wait(1000)
        Core.Functions.Notify(Lang:t("info.no_one_home"), 'error')
    end
end
...
local function RandomDeliveryItemOnRep()
    local myRep = Core.Functions.GetPlayerData().metadata["dealerrep"]
...
local function RequestDelivery()
    if not waitingDelivery then
        ...
        Core.Functions.Notify(Lang:t("info.sending_delivery_email"), 'success')
        TriggerServerEvent('drugs:server:giveDeliveryItems', waitingDelivery)
        SetTimeout(2000, function()
            TriggerServerEvent('phoneserver:sendNewMail', {
                sender = Config.Dealers[currentDealer]["name"],
                subject = "Delivery Location",
                message = Lang:t("info.delivery_info_email", {itemAmount = amount, itemLabel = Core.Shared.Items[waitingDelivery["itemData"]["item"]]["label"]}),
                button = {
                    enabled = true,
        ...
        Core.Functions.Notify(Lang:t("error.pending_delivery"), 'error')
    end
end
...
local function DeliverStuff()
    if deliveryTimeout > 0 then
        ...
        PoliceCall()
        Core.Functions.Progressbar("work_dropbox", Lang:t("info.delivering_products"), 3500, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
        }, {}, {}, {}, function() -- Done
            TriggerServerEvent('drugs:server:successDelivery', activeDelivery, true)
            activeDelivery = nil
            if Config.UseTarget then
                exports['target']:RemoveZone('drugDeliveryZone')
            else
        ...
        TriggerServerEvent('drugs:server:successDelivery', activeDelivery, false)
    end
    deliveryTimeout = 0
end
...
local function SetMapBlip(x, y)
    SetNewWaypoint(x, y)
    Core.Functions.Notify(Lang:t("success.route_has_been_set"), 'success');
end
...
function InitZones()
    if Config.UseTarget then
        for k,v in pairs(Config.Dealers) do
            exports["target"]:AddBoxZone("dealer_"..k, vector3(v.coords.x, v.coords.y, v.coords.z), 1.5, 1.5, {
                name = "dealer_"..k,
                heading = v.heading,
                minZ = v.coords.z - 1,
                maxZ = v.coords.z + 1,
        ...
        dealerCombo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if not dealerIsHome then
                    exports['core']:DrawText(Lang:t("info.knock_button"),'left')
                    AwaitingInput()
                elseif dealerIsHome then
                    exports['core']:DrawText(Lang:t("info.other_dealers_button"), 'left')
                    AwaitingInput()
...
RegisterNetEvent('drugs:client:RefreshDealers', function(DealerData)
    if not Config.UseTarget and dealerCombo then dealerCombo:destroy() end
...
RegisterNetEvent('drugs:client:updateDealerItems', function(itemData, amount)
    TriggerServerEvent('drugs:server:updateDealerItems', itemData, amount, currentDealer)
end)

RegisterNetEvent('drugs:client:setDealerItems', function(itemData, amount, dealer)
    Config.Dealers[dealer]["products"][itemData.slot].amount = Config.Dealers[dealer]["products"][itemData.slot].amount - amount
end)

RegisterNetEvent('drugs:client:setLocation', function(locationData)
    ...
    SetMapBlip(activeDelivery["coords"]["x"], activeDelivery["coords"]["y"])
    if Config.UseTarget then
        exports["target"]:AddBoxZone('drugDeliveryZone', vector3(activeDelivery["coords"].x, activeDelivery["coords"].y, activeDelivery["coords"].z), 1.5, 1.5, {
        ...
        drugDeliveryZone:onPlayerInOut(function(isPointInside)
            if isPointInside then
                ...
                exports['core']:DrawText(Lang:t("info.deliver_items_button", {itemAmount = activeDelivery["amount"], itemLabel = Core.Shared.Items[activeDelivery["itemData"]["item"]]["label"]}),'left')
                CreateThread(function()
...
RegisterNetEvent('drugs:client:sendDeliveryMail', function(type, deliveryData)
