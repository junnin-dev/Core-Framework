...
-- Callbacks
Core.Functions.CreateCallback('drugs:server:RequestConfig', function(_, cb)
    cb(Config.Dealers)
end)

-- Events
RegisterNetEvent('drugs:server:updateDealerItems', function(itemData, amount, dealer)
    local src = source
    ...
    if Config.Dealers[dealer]["products"][itemData.slot].amount - 1 >= 0 then
        ...
        TriggerClientEvent('drugs:client:setDealerItems', -1, itemData, amount, dealer)
    else
        ...
        TriggerClientEvent("Core:Notify", src, Lang:t("error.item_unavailable"), "error")
    end
end)

RegisterNetEvent('drugs:server:giveDeliveryItems', function(deliveryData)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
...
RegisterNetEvent('drugs:server:successDelivery', function(deliveryData, inTime)
    local src = source
    local Player = Core.Functions.GetPlayer(src)
    ...
    local copsOnline = Core.Functions.GetDutyCount('police')
    ...
    if inTime then
        if invItem and invItem.amount >= itemAmount then -- on time correct amount
            ...
            TriggerClientEvent('Core:Notify', src, Lang:t("success.order_delivered"), 'success')
            SetTimeout(math.random(5000, 10000), function()
                TriggerClientEvent('drugs:client:sendDeliveryMail', src, 'perfect', deliveryData)
                Player.Functions.SetMetaData('dealerrep', (curRep + Config.DeliveryRepGain))
            end)
        else
            TriggerClientEvent('Core:Notify', src, Lang:t("error.order_not_right"), 'error')-- on time incorrect amount
            if invItem then
            ...
            SetTimeout(math.random(5000, 10000), function()
                TriggerClientEvent('drugs:client:sendDeliveryMail', src, 'bad', deliveryData)
                if curRep - 1 > 0 then
        ...
        if invItem and invItem.amount >= itemAmount then -- late correct amount
            TriggerClientEvent('Core:Notify', src, Lang:t("error.too_late"), 'error')
            Player.Functions.RemoveItem(item, itemAmount)
            Player.Functions.AddMoney('cash', math.floor(payout / Config.OverdueDeliveryFee), "delivery-drugs-too-late")
            TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items[item], "remove")
            SetTimeout(math.random(5000, 10000), function()
                TriggerClientEvent('drugs:client:sendDeliveryMail', src, 'late', deliveryData)
                if curRep - 1 > 0 then
                    Player.Functions.SetMetaData('dealerrep', (curRep - Config.DeliveryRepLoss))
                else
                    Player.Functions.SetMetaData('dealerrep', 0)
...
-- Commands
Core.Commands.Add("newdealer", Lang:t("info.newdealer_command_desc"), {{
    ...
    if result then return TriggerClientEvent('Core:Notify', source, Lang:t("error.dealer_already_exists"), "error") end
    MySQL.insert('INSERT INTO dealers (name, coords, time, createdby) VALUES (?, ?, ?, ?)', {dealerName, pos, time, Player.PlayerData.citizenid}, function()
        ...
        TriggerClientEvent('drugs:client:RefreshDealers', -1, Config.Dealers)
    end)
end, "admin")

Core.Commands.Add("deletedealer", Lang:t("info.deletedealer_command_desc"), {{
    ...
    if result then
        ...
        TriggerClientEvent('drugs:client:RefreshDealers', -1, Config.Dealers)
        TriggerClientEvent('Core:Notify', source, Lang:t("success.dealer_deleted", {dealerName = dealerName}), "success")
    else
        TriggerClientEvent('Core:Notify', source, Lang:t("error.dealer_not_exists_command", {dealerName = dealerName}), "error")
    end
end, "admin")

Core.Commands.Add("dealers", Lang:t("info.dealers_command_desc"), {}, false, function(source, _)
    local DealersText = ""
    if Config.Dealers ~= nil and next(Config.Dealers) ~= nil then
        ...
        TriggerClientEvent('Core:Notify', source, Lang:t("error.no_dealers"), 'error')
    end
end, "admin")

Core.Commands.Add("dealergoto", Lang:t("info.dealergoto_command_desc"), {{
    ...
    if Config.Dealers[DealerName] then
        ...
        TriggerClientEvent('Core:Notify', source, Lang:t("success.teleported_to_dealer", {dealerName = DealerName}), 'success')
    else
        TriggerClientEvent('Core:Notify', source, Lang:t("error.dealer_not_exists"), 'error')
    end
end, "admin")
