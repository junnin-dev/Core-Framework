...
-- Compatibilidade com as opções de cura do menu txAdmin.
-- Este é um evento do lado do servidor somente do administrador que passará o ID do jogador alvo ou -1.
...
RegisterNetEvent('hospital:server:SendToBed', function(bedId, isRevive)
	...
	Player.Functions.RemoveMoney("bank", Config.BillCost , "respawned-at-hospital")
		exports['management']:AddMoney("ambulance", Config.BillCost)
		...
		for k, v in pairs(Config.Locations["jailbeds"]) do
			if not v.taken then
				...
				if Config.WipeInventoryOnRespawn then
					...
					TriggerClientEvent('Core:Notify', src, Lang:t('error.possessions_taken'), 'error')
				end
				Player.Functions.RemoveMoney("bank", Config.BillCost, "respawned-at-hospital")
					exports['management']:AddMoney("ambulance", Config.BillCost)
				TriggerClientEvent('hospital:client:SendBillEmail', src, Config.BillCost)
...
RegisterNetEvent('hospital:server:TreatWounds', function(playerId)
	local src = source
	local Player = Core.Functions.GetPlayer(src)
	local Patient = Core.Functions.GetPlayer(playerId)
	if Patient then
		if Player.PlayerData.job.name =="ambulance" then
			Player.Functions.RemoveItem('bandage', 1)
			TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items['bandage'], "remove")
	...
	if Patient then
		if oldMan then
			if Player.Functions.RemoveMoney("cash", 5000, "revived-player") then
				...
				TriggerClientEvent('inventory:client:ItemBox', src, Core.Shared.Items['firstaid'], "remove")
...
RegisterNetEvent('hospital:server:SendDoctorAlert', function()
    ...
    if not doctorCalled then
        ...
        for _, v in pairs(players) do
            if v.PlayerData.job.name == 'ambulance' and v.PlayerData.job.onduty then
                TriggerClientEvent('Core:Notify', v.PlayerData.source, Lang:t('info.dr_needed'), 'ambulance')
            end
        end
...
RegisterNetEvent('hospital:server:CanHelp', function(helperId, canHelp)
	...
	if canHelp then
		...
		TriggerClientEvent('Core:Notify', helperId, Lang:t('error.cant_help'), "error")
	end
end)
...
Core.Functions.CreateCallback('hospital:GetDoctors', function(_, cb)
	local amount = 0
    local players = Core.Functions.GetJNPlayers()
    for _, v in pairs(players) do
        if v.PlayerData.job.name == 'ambulance' and v.PlayerData.job.onduty then
...
Core.Functions.CreateCallback('hospital:GetPlayerStatus', function(_, cb, playerId)
...
Core.Functions.CreateCallback('hospital:GetPlayerBleeding', function(source, cb)
	local src = source
...
Core.Commands.Add('911e', Lang:t('info.ems_report'), {{name = 'message', help = Lang:t('info.message_sent')}}, false, function(source, args)
	local src = source
	local message
...
Core.Commands.Add("status", Lang:t('info.check_health'), {}, false, function(source, _)
	local src = source
	local Player = Core.Functions.GetPlayer(src)
	if Player.PlayerData.job.name == "ambulance" then
		TriggerClientEvent("hospital:client:CheckStatus", src)
	else
		TriggerClientEvent('Core:Notify', src, Lang:t('error.not_ems'), "error")
	end
end)

Core.Commands.Add("heal", Lang:t('info.heal_player'), {}, false, function(source, _)
	local src = source
	local Player = Core.Functions.GetPlayer(src)
	if Player.PlayerData.job.name == "ambulance" then
...
Core.Commands.Add("revivep", Lang:t('info.revive_player'), {}, false, function(source, _)
	local src = source
	local Player = Core.Functions.GetPlayer(src)
	if Player.PlayerData.job.name == "ambulance" then
...
Core.Commands.Add("revive", Lang:t('info.revive_player_a'), {{name = "id", help = Lang:t('info.player_id')}}, false, function(source, args)
	local src = source
	if args[1] then
		...
		if Player then
			...
			TriggerClientEvent('Core:Notify', src, Lang:t('error.not_online'), "error")
		end
	else
...
Core.Commands.Add("setpain", Lang:t('info.pain_level'), {{name = "id", help = Lang:t('info.player_id')}}, false, function(source, args)
	local src = source
	if args[1] then
		local Player = Core.Functions.GetPlayer(tonumber(args[1]))
		if Player then
...
Core.Commands.Add("kill", Lang:t('info.kill'), {{name = "id", help = Lang:t('info.player_id')}}, false, function(source, args)
	local src = source
	if args[1] then
		local Player = Core.Functions.GetPlayer(tonumber(args[1]))
		if Player then
...
Core.Commands.Add('aheal', Lang:t('info.heal_player_a'), {{name = 'id', help = Lang:t('info.player_id')}}, false, function(source, args)
	local src = source
	if args[1] then
		local Player = Core.Functions.GetPlayer(tonumber(args[1]))
		if Player then
...
Core.Functions.CreateUseableItem("ifaks", function(source, item)
	local src = source
	local Player = Core.Functions.GetPlayer(src)
...
Core.Functions.CreateUseableItem("bandage", function(source, item)
	local src = source
	local Player = Core.Functions.GetPlayer(src)
	if Player.Functions.GetItemByName(item.name) ~= nil then
...
Core.Functions.CreateUseableItem("painkillers", function(source, item)
	local src = source
	local Player = Core.Functions.GetPlayer(src)
	if Player.Functions.GetItemByName(item.name) ~= nil then
...
Core.Functions.CreateUseableItem("firstaid", function(source, item)
	local src = source
	local Player = Core.Functions.GetPlayer(src)
	if Player.Functions.GetItemByName(item.name) ~= nil then
