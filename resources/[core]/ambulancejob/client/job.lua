...
function TakeOutVehicle(vehicleInfo)
    ...
    Core.Functions.TriggerCallback('Core:Server:SpawnVehicle', function(netId)
        ...
        if Config.VehicleSettings[vehicleInfo] ~= nil then
            Core.Shared.SetDefaultVehicleExtras(veh, Config.VehicleSettings[vehicleInfo].extras)
        end
...
function MenuGarage()
    ...
    local authorizedVehicles = Config.AuthorizedVehicles[Core.Functions.GetPlayerData().job.grade.level]
...
function Status()
    if isStatusChecking then
        ...
        exports['menu']:openMenu(statusMenu)
    end
end
...
RegisterNetEvent('hospital:client:CheckStatus', function()
    ...
    if player ~= -1 and distance < 5.0 then
        ...
        Core.Functions.TriggerCallback('hospital:GetPlayerStatus', function(result)
            if result then
                for k, v in pairs(result) do
                    ...
                    elseif result["WEAPONWOUNDS"] then
                        for _, v2 in pairs(result["WEAPONWOUNDS"]) do
                            TriggerEvent('chat:addMessage', {
                                color = { 255, 0, 0 },
                                multiline = false,
                                args = { Lang:t('info.status'), Core.Shared.Weapons[v2].damagereason }
                            })
                        end
                    elseif result["BLEED"] > 0 then
                        ...
                        Core.Functions.Notify(Lang:t('success.healthy_player'), 'success')
                    end
                end
        ...
        Core.Functions.Notify(Lang:t('error.no_player'), 'error')
    end
end)

RegisterNetEvent('hospital:client:RevivePlayer', function()
    local hasItem = Core.Functions.HasItem('firstaid')
    if hasItem then
        local player, distance = GetClosestPlayer()
        if player ~= -1 and distance < 5.0 then
            local playerId = GetPlayerServerId(player)
            Core.Functions.Progressbar("hospital_revive", Lang:t('progress.revive'), 5000, false, true, {
                ...
                Core.Functions.Notify(Lang:t('success.revived'), 'success')
        ...
        Core.Functions.Notify(Lang:t('error.no_firstaid'), "error")
    end
end)

RegisterNetEvent('hospital:client:TreatWounds', function()
    local hasItem = Core.Functions.HasItem('bandage')
    if hasItem then
        local player, distance = GetClosestPlayer()
        if player ~= -1 and distance < 5.0 then
            local playerId = GetPlayerServerId(player)
            Core.Functions.Progressbar("hospital_healwounds", Lang:t('progress.healing'), 5000, false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
                ...
                StopAnimTask(PlayerPedId(), healAnimDict, "exit", 1.0)
                Core.Functions.Notify(Lang:t('success.helped_player'), 'success')
        ...
        Core.Functions.Notify(Lang:t('error.no_bandage'), "error")
    end
end)
    ...
    CreateThread(function()
        ...
        while check do
            if IsControlJustPressed(0, 38) then
                exports['core']:KeyPressed(38)
                if variable == "sign" then
                    TriggerEvent('EMSToggle:Duty')
                elseif variable == "stash" then
                    TriggerEvent('ambulancejob:stash')
                elseif variable == "armory" then
                    TriggerEvent('ambulancejob:armory')
                elseif variable == "storeheli" then
                    TriggerEvent('ambulancejob:storeheli')
                elseif variable == "takeheli" then
                    TriggerEvent('ambulancejob:pullheli')
                elseif variable == "roof" then
                    TriggerEvent('ambulancejob:elevator_main')
                elseif variable == "main" then
                    TriggerEvent('ambulancejob:elevator_roof')
                end
            end
            Wait(1)
        end
...
RegisterNetEvent('ambulancejob:stash', function()
    if onDuty then
        TriggerServerEvent("inventory:server:OpenInventory", "stash",
            "ambulancestash_" .. Core.Functions.GetPlayerData().citizenid)
        TriggerEvent("inventory:client:SetCurrentStash", "ambulancestash_" .. Core.Functions.GetPlayerData().citizenid)
    end
end)

RegisterNetEvent('ambulancejob:armory', function()
    if onDuty then
        ...
        while CheckVehicle do
            if IsControlJustPressed(0, 38) then
                ...
                if IsPedInAnyVehicle(ped, false) then
                    Core.Functions.DeleteVehicle(GetVehiclePedIsIn(ped))
                else
...
RegisterNetEvent('ambulancejob:elevator_roof', function()
    local ped = PlayerPedId()
...
RegisterNetEvent('ambulancejob:elevator_main', function()
    local ped = PlayerPedId()
...
CreateThread(function()
    for k, v in pairs(Config.Locations["vehicle"]) do
        ...
        boxZone:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" and onDuty then
                exports['core']:DrawText(Lang:t('text.veh_button'), 'left')
                EMSVehicle(k)
            else
                CheckVehicle = false
    ...
    for k, v in pairs(Config.Locations["helicopter"]) do
        ...
        boxZone:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" and onDuty then
                exports['core']:DrawText(Lang:t('text.heli_button'), 'left')
                EMSHelicopter(k)
            else
                CheckHeli = false
                exports['core']:HideText()
...
if Config.UseTarget then
    CreateThread(function()
        for k, v in pairs(Config.Locations["duty"]) do
            exports['target']:AddBoxZone("duty" .. k, vector3(v.x, v.y, v.z), 1.5, 1, {
                name = "duty" .. k,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
        ...
        for k, v in pairs(Config.Locations["stash"]) do
            exports['target']:AddBoxZone("stash" .. k, vector3(v.x, v.y, v.z), 1, 1, {
                name = "stash" .. k,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
        ...
        for k, v in pairs(Config.Locations["armory"]) do
            exports['target']:AddBoxZone("armory" .. k, vector3(v.x, v.y, v.z), 1, 1, {
                name = "armory" .. k,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
        ...
        for k, v in pairs(Config.Locations["roof"]) do
            exports['target']:AddBoxZone("roof" .. k, vector3(v.x, v.y, v.z), 2, 2, {
                name = "roof" .. k,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
        ...
        for k, v in pairs(Config.Locations["main"]) do
            exports['target']:AddBoxZone("main" .. k, vector3(v.x, v.y, v.z), 1.5, 1.5, {
                name = "main" .. k,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
    ...
    CreateThread(function()
        ...
        signCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if not onDuty then
                    exports['core']:DrawText(Lang:t('text.onduty_button'), 'left')
                    EMSControls("sign")
                else
                    exports['core']:DrawText(Lang:t('text.offduty_button'), 'left')
                    EMSControls("sign")
                end
            else
                check = false
        ...
        stashCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if onDuty then
                    exports['core']:DrawText(Lang:t('text.pstash_button'), 'left')
                    EMSControls("stash")
                end
            else
                check = false
        ...
        armoryCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if onDuty then
                    exports['core']:DrawText(Lang:t('text.armory_button'), 'left')
                    EMSControls("armory")
                end
            else
                check = false
        ...
        roofCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if onDuty then
                    exports['core']:DrawText(Lang:t('text.elevator_main'), 'left')
                    EMSControls("main")
                else
                    exports['core']:DrawText(Lang:t('error.not_ems'), 'left')
                end
            else
                check = false
                exports['core']:HideText()
        ...
        mainCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if onDuty then
                    exports['core']:DrawText(Lang:t('text.elevator_roof'), 'left')
                    EMSControls("roof")
                else
                    exports['core']:DrawText(Lang:t('error.not_ems'), 'left')
                end
