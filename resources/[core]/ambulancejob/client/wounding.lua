...
local function DoBleedAlert()
    if not isDead and tonumber(isBleeding) > 0 then
        Core.Functions.Notify(Lang:t('info.bleed_alert', {bleedstate = Config.BleedingStates[tonumber(isBleeding)].label}), "error", 5000)
    end
end
...
RegisterNetEvent('hospital:client:UseIfaks', function()
    ...
    Core.Functions.Progressbar("use_bandage", Lang:t('progress.ifaks'), 3000, false, true, {
        ...
        TriggerEvent("inventory:client:ItemBox", Core.Shared.Items["ifaks"], "remove")
...
RegisterNetEvent('hospital:client:UseBandage', function()
    local ped = PlayerPedId()
    Core.Functions.Progressbar("use_bandage", Lang:t('progress.bandage'), 4000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
        ...
        TriggerEvent("inventory:client:ItemBox", Core.Shared.Items["bandage"], "remove")
        SetEntityHealth(ped, GetEntityHealth(ped) + 10)
        if math.random(1, 100) < 50 then
            RemoveBleed(1)
        end
...
RegisterNetEvent('hospital:client:UsePainkillers', function()
    local ped = PlayerPedId()
    Core.Functions.Progressbar("use_bandage", Lang:t('progress.painkillers'), 3000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
        ...
        TriggerEvent("inventory:client:ItemBox", Core.Shared.Items["painkillers"], "remove")
        onPainKillers = true
        if painkillerAmount < 3 then
            painkillerAmount = painkillerAmount + 1
        end
            ...
            if bleedTickTimer >= Config.BleedTickRate and not isInHospitalBed then
                if not isDead and not InLaststand then
                    if isBleeding > 0 then
                        ...
                        TriggerServerEvent("evidence:server:CreateBloodDrop", Core.Functions.GetPlayerData().citizenid, Core.Functions.GetPlayerData().metadata["bloodtype"], coords)
