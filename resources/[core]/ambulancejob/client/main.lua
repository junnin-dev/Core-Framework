...
local function DoLimbAlert()
    if not isDead and not InLaststand then
        if #injured > 0 then
            ...
            Core.Functions.Notify(limbDamageMsg, "primary")
        end
    end
end
...
local function DoBleedAlert()
    if not isDead and tonumber(isBleeding) > 0 then
        Core.Functions.Notify(Lang:t('info.bleed_alert', {bleedstate = Config.BleedingStates[tonumber(isBleeding)].label}), "error")
    end
end
...
local function CheckWeaponDamage(ped)
    ...
    for k, v in pairs(Core.Shared.Weapons) do
        if HasPedBeenDamagedByWeapon(ped, k, 0) then
            detected = true
...
RegisterNetEvent('hospital:client:ambulanceAlert', function(coords, text)
    ...
    Core.Functions.Notify({text = text, caption = street1name.. ' ' ..street2name}, 'ambulance')
...
RegisterNetEvent('hospital:client:Revive', function()
    ...
    emsNotified = false
    Core.Functions.Notify(Lang:t('info.healthy'))
end)
...
RegisterNetEvent('hospital:client:HealInjuries', function(type)
    ...
    TriggerServerEvent("hospital:server:RestoreWeaponDamage")
    Core.Functions.Notify(Lang:t('success.wounds_healed'), 'success')
end)
...
RegisterNetEvent('hospital:client:SendToBed', function(id, data, isRevive)
    ...
    CreateThread(function ()
        Wait(5)
        if isRevive then
            Core.Functions.Notify(Lang:t('success.being_helped'), 'success')
...
RegisterNetEvent('hospital:client:SendBillEmail', function(amount)
    SetTimeout(math.random(2500, 4000), function()
        ...
        if Core.Functions.GetPlayerData().charinfo.gender == 1 then
            gender = Lang:t('info.mrs')
        end
        local charinfo = Core.Functions.GetPlayerData().charinfo
...
CreateThread(function()
    while true do
        ...
        if isInHospitalBed and canLeaveBed then
            sleep = 0
            exports['core']:DrawText(Lang:t('text.bed_out'))
        ...
        while listen do
            if IsControlJustPressed(0, 38) then
                ...
                if variable == "checkin" then
                   TriggerEvent('ambulancejob:checkin')
                    listen = false
                elseif variable == "beds" then
                    TriggerEvent('ambulancejob:beds')
                    listen = false
                end
            end
            Wait(1)
...
RegisterNetEvent('ambulancejob:checkin', function()
    if doctorCount >= Config.MinimalDoctors then
        TriggerServerEvent("hospital:server:SendDoctorAlert")
    else
        TriggerEvent('animations:client:EmoteCommandStart', {"notepad"})
        Core.Functions.Progressbar("hospital_checkin", Lang:t('progress.checking_in'), 2000, false, true, {
            ...
            if bedId then
                ...
                Core.Functions.Notify(Lang:t('error.beds_taken'), "error")
            end
        end, function() -- Cancel
            TriggerEvent('animations:client:EmoteCommandStart', {"c"})
...
RegisterNetEvent('ambulancejob:beds', function()
...
if Config.UseTarget then
    CreateThread(function()
        for k, v in pairs(Config.Locations["checking"]) do
            exports['target']:AddBoxZone("checking"..k, vector3(v.x, v.y, v.z), 3.5, 2, {
                name = "checking"..k,
                heading = -72,
                debugPoly = false,
                minZ = v.z - 2,
        ...
        for k, v in pairs(Config.Locations["beds"]) do
            exports['target']:AddBoxZone("beds"..k,  v.coords, 2.5, 2.3, {
                name = "beds"..k,
                heading = -20,
                debugPoly = false,
                minZ = v.coords.z - 1,
    ...
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["checking"]) do
            ...
            checkingCombo:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    if doctorCount >= Config.MinimalDoctors then
                        exports['core']:DrawText(Lang:t('text.call_doc'),'left')
                        CheckInControls("checkin")
                    else
                        exports['core']:DrawText(Lang:t('text.check_in'), 'left')
                        CheckInControls("checkin")
                    end
                else
                    listen = false
        ...
        for k, v in pairs(Config.Locations["beds"]) do
            ...
            bedCombo:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    exports['core']:DrawText(Lang:t('text.lie_bed'), 'left')
