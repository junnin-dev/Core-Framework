...
local function callCops()
    ...
    if call == chance then
        TriggerServerEvent('diving:server:CallCops', coords)
    end
end
...
local function takeCoral(coral)
    ...
    Core.Functions.Progressbar("take_coral", Lang:t("info.collecting_coral"), times * 1000, false, true, {
        ...
        TriggerServerEvent('diving:server:TakeCoral', currentDivingLocation.area, coral, true)
        ClearPedTasks(ped)
...
local function setDivingLocation(divingLocation)
    ...
    for k, v in pairs(Config.CoralLocations[currentDivingLocation.area].coords.Coral) do
        if Config.UseTarget then
            exports['target']:AddBoxZone('diving_coral_zone_'..k, v.coords, v.length, v.width, {
            ...
            zones[k]:onPlayerInOut(function(inside)
                if inside then
                    currentArea = k
                    exports['core']:DrawText(Lang:t("info.collect_coral_dt"))
                else
                    currentArea = 0
...
local function sellCoral()
    ...
    Core.Functions.Progressbar("sell_coral_items", Lang:t("info.checking_pockets"), math.random(2000, 4000), false, true, {}, {}, {}, {}, function() -- Done
        ClearPedTasks(playerPed)
        TriggerServerEvent('diving:server:SellCoral')
    ...
    for i = 1, #Config.SellLocations do
        ...
        if Config.UseTarget then
            ...
            zone:onPlayerInOut(function(inside)
                if inside then
                    inSellerZone = true
                    exports['core']:DrawText(Lang:t("info.sell_coral_dt"))
                else
                    inSellerZone = false
                    exports['core']:HideText()
                end
...
RegisterNetEvent('Core:Client:OnPlayerLoaded', function()
    Core.Functions.TriggerCallback('diving:server:GetDivingConfig', function(config, area)
...
RegisterNetEvent('diving:client:NewLocations', function()
    Core.Functions.TriggerCallback('diving:server:GetDivingConfig', function(config, area)
        Config.CoralLocations = config
        setDivingLocation(area)
    end)
...
RegisterNetEvent('diving:client:UpdateCoral', function(area, coral, bool)
    Config.CoralLocations[area].coords.Coral[coral].PickedUp = bool
end)

RegisterNetEvent('diving:client:CallCops', function(coords, msg)
...
RegisterNetEvent("diving:client:setoxygenlevel", function()
    if oxgenlevell == 0 then
       oxgenlevell = Config.oxygenlevel -- oxygenlevel
       Core.Functions.Notify(Lang:t("success.tube_filled"), 'success')
       TriggerServerEvent('diving:server:removeItemAfterFill')
    else
        Core.Functions.Notify(Lang:t("error.oxygenlevel", {oxygenlevel = oxgenlevell}), 'error')
    end
end)
...
RegisterNetEvent('diving:client:UseGear', function()
    local ped = PlayerPedId()
    if iswearingsuit == false then
        if oxgenlevell > 0 then
            ...
            if not IsPedSwimming(ped) and not IsPedInAnyVehicle(ped) then
                gearAnim()
                Core.Functions.Progressbar("equip_gear", Lang:t("info.put_suit"), 5000, false, true, {}, {}, {}, {},
                    function() -- Done
                        deleteGear()
                ...
                Core.Functions.Notify(Lang:t("error.not_standing_up"), 'error')
            end
        else
            Core.Functions.Notify(Lang:t("error.need_otube"), 'error')
        end
    elseif iswearingsuit == true then
        gearAnim()
        Core.Functions.Progressbar("remove_gear", Lang:t("info.pullout_suit"), 5000, false, true, {}, {}, {}, {},
            function() -- Done
                SetEnableScuba(ped, false)
                ...
                ClearPedTasks(ped)
                deleteGear()
                Core.Functions.Notify(Lang:t("success.took_out"))
                TriggerServerEvent("InteractSound_SV:PlayOnSource", nil, 0.25)
                iswearingsuit = false
                oxgenlevell = oxgenlevell
            end)
