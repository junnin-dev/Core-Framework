...
--[[ -- Only used by the following "banking:initiateTransfer"
...
RegisterNetEvent('banking:initiateTransfer', function(_)
    --[[
    local _src = source
    local _startChar = Core.Functions.GetPlayer(_src)
    ...
    if (checkAccount) then
        ...
        if receiptName ~= false or receiptName ~= nil then
            ...
            if userOnline ~= false then
                ...
                if acType == "Current" then
                    ...
                    TriggerClientEvent('banking:openBankScreen', _src)
                    TriggerClientEvent('banking:successAlert', _src, 'You have sent a bank transfer to '..receiptName..' for the amount of $'..data.amount)
                else
        ...
        TriggerClientEvent('banking:transferError', _src, 'The account details entered could not be located.')
    end
]]
end)
...
Core.Functions.CreateCallback('banking:getBankingInformation', function(source, cb)
...
RegisterNetEvent('banking:createBankCard', function(pin)
    local src = source
    ...
    TriggerClientEvent('banking:successAlert', src, Lang:t('success.debit_card'))

    TriggerEvent('log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** successfully ordered a debit card")
end)

RegisterNetEvent('banking:doQuickDeposit', function(amount)
    local src = source
    local xPlayer = Core.Functions.GetPlayer(src)
    ...
    if tonumber(amount) <= currentCash then
        ...
        if bank then
            TriggerClientEvent('banking:openBankScreen', src)
            TriggerClientEvent('banking:successAlert', src, Lang:t('success.cash_deposit', {value = amount}))
            TriggerEvent('log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a cash deposit of $"..amount.." successfully.")
        end
    end
end)

RegisterNetEvent('banking:toggleCard', function(toggle)
    local src = source
    local xPlayer = Core.Functions.GetPlayer(src)
...
RegisterNetEvent('banking:doQuickWithdraw', function(amount, _)
    local src = source
    local xPlayer = Core.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentCash then
        ...
        if cash then
            TriggerClientEvent('banking:openBankScreen', src)
            TriggerClientEvent('banking:successAlert', src, Lang:t('success.cash_withdrawal', {value = amount}))
            TriggerEvent('log:server:CreateLog', 'banking', 'Banking', 'red', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a cash withdrawal of $"..amount.." successfully.")
        end
    end
end)

RegisterNetEvent('banking:updatePin', function(currentBankCard, newPin)
    if newPin ~= nil then
        local src = source
        local xPlayer = Core.Functions.GetPlayer(src)
        while xPlayer == nil do Wait(0) end
...
RegisterNetEvent('banking:savingsDeposit', function(amount)
    local src = source
    local xPlayer = Core.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentBank then
        ...
        TriggerClientEvent('banking:openBankScreen', src)
        TriggerClientEvent('banking:successAlert', src, Lang:t('success.savings_deposit', {value = tostring(amount)}))
        TriggerEvent('log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a savings deposit of $"..tostring(amount).." successfully..")
    end
end)

RegisterNetEvent('banking:savingsWithdraw', function(amount)
    local src = source
    local xPlayer = Core.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentSavings then
        ...
        while savings == nil do Wait(0) end
        TriggerClientEvent('banking:openBankScreen', src)
        TriggerClientEvent('banking:successAlert', src, Lang:t('success.savings_withdrawal', {value = tostring(amount)}))
        TriggerEvent('log:server:CreateLog', 'banking', 'Banking', 'red', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** fez uma retirada de economia de $"..tostring(amount).." successfully.")
    end
end)

RegisterNetEvent('banking:createSavingsAccount', function()
    local src = source
    local xPlayer = Core.Functions.GetPlayer(src)
    local success = createSavingsAccount(xPlayer.PlayerData.citizenid)
    repeat Wait(0) until success ~= nil
    TriggerClientEvent('banking:openBankScreen', src)
    TriggerClientEvent('banking:successAlert', src, Lang:t('success.opened_savings'))
    TriggerEvent('log:server:CreateLog', 'banking', 'Banking', "lightgreen", "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** abriu uma conta poupança")
end)


Core.Commands.Add('givecash', Lang:t('command.givecash'), {{name = 'id', help = 'Player ID'}, {name = 'amount', help = 'Amount'}}, true, function(source, args)
  local src = source
	...
	if id and amount then
		local xPlayer = Core.Functions.GetPlayer(src)
		local xReciv = Core.Functions.GetPlayer(id)
                    ...
                    if amount > 0 then
                        if xPlayer.Functions.RemoveMoney('cash', amount) then
                            if xReciv.Functions.AddMoney('cash', amount) then
                                TriggerClientEvent('Core:Notify', src, Lang:t('success.give_cash',{id = tostring(id), cash = tostring(amount)}), "success")
                                TriggerClientEvent('Core:Notify', id, Lang:t('success.received_cash',{id = tostring(src), cash = tostring(amount)}), "success")
                                TriggerClientEvent("payanimation", src)
                            else
                                -- Return player cash
                                xPlayer.Functions.AddMoney('cash', amount)
                                TriggerClientEvent('Core:Notify', src, Lang:t('error.not_give'), "error")
                            end
                        else
                            TriggerClientEvent('Core:Notify', src, Lang:t('error.not_enough'), "error")
                        end
                    else
                        TriggerClientEvent('Core:Notify', src, Lang:t('error.invalid_amount'), "error")
                    end
				else
					TriggerClientEvent('Core:Notify', src, Lang:t('error.too_far_away'), "error")
				end
			else
				TriggerClientEvent('Core:Notify', src, Lang:t('error.dead'), "error")
			end
		else
			TriggerClientEvent('Core:Notify', src, Lang:t('error.wrong_id'), "error")
		end
	else
		TriggerClientEvent('Core:Notify', src, Lang:t('error.givecash'), "error")
	end
end)
