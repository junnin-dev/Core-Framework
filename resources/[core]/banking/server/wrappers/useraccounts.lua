        ...
        if not self.cardActive then
            ...
            MySQL.update('UPDATE bank_cards SET cardnumber = ?, cardPin = ?, cardDecrypted = ?, cardActive = ?, cardLocked = ?, cardType = ? WHERE citizenid = ? AND record_id = ?', {
                ...
                if self.source ~= -1 then
                    TriggerClientEvent('banking:client:newCardSuccess', self.source, cardNumber, friendlyName)
                    local xPlayer = Core.Functions.GetPlayer(self.source)
...
RegisterNetEvent('banking:server:registerCurrentAccount', function(cid)
...
RegisterNetEvent('banking:server:registerSavingsAccount', function(cid)
