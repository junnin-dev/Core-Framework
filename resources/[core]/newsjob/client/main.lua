...
function MenuGarage()
    ...
    local Vehicles = Config.Vehicles[Core.Functions.GetPlayerData().job.grade.level]
    for veh, label in pairs(Vehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            header = label,
            txt = "",
            params = {
                event = "newsjob:client:TakeOutVehicle",
                args = {
                    vehicle = veh
                }
            }
...
function MenuHeliGarage()
    ...
    local Helicopters = Config.Helicopters[Core.Functions.GetPlayerData().job.grade.level]
    for veh, label in pairs(Helicopters) do
        vehicleMenu[#vehicleMenu+1] = {
            header = label,
            txt = "",
            params = {
                event = "newsjob:client:TakeOutHelicopters",
                args = {
                    vehicle = veh
                }
            }
...
RegisterNetEvent('newsjob:client:TakeOutVehicle', function(data)
    local vehicle = data.vehicle
    TakeOutVehicle(vehicle)
end)

RegisterNetEvent('newsjob:client:TakeOutHelicopters', function(data)
    local vehicle = data.vehicle
