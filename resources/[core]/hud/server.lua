...
RegisterNetEvent('hud:server:GainStress', function(amount)
    ...
    TriggerClientEvent('Core:Notify', src, Lang:t("notify.stress_gain"), 'error', 1500)
end)
...
RegisterNetEvent('hud:server:RelieveStress', function(amount)
    ...
    Player.Functions.SetMetaData('stress', newStress)
    TriggerClientEvent('hud:client:UpdateStress', src, newStress)
    TriggerClientEvent('Core:Notify', src, Lang:t("notify.stress_removed"))
end)

RegisterNetEvent('hud:server:saveUIData', function(data)
    local src = source
	-- Check Permissions
    if not Core.Functions.HasPermission(src, 'admin') and not IsPlayerAceAllowed(src, 'command') then
		return
	end
...
Core.Functions.CreateCallback('hud:server:getMenu', function(source, cb)
    cb(Config.Menu)
end) 

Core.Functions.CreateCallback('hud:server:getRank', function(source, cb)
    local src = source
