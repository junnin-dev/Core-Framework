...
local function hasHarness()
    ...
    local hasHarness = exports['smallresources']:HasHarness()
    if hasHarness then
        _harness = true
    else
        _harness = false
...
local function loadSettings()
    Core.Functions.Notify(Lang:t("notify.hud_settings_loaded"), "success")
    Wait(1000)
...
local function HandleSetupResource()
    Core.Functions.TriggerCallback('hud:server:getRank', function(isAdminOrGreater)
...
local function restartHud()
    ...
    Core.Functions.Notify(Lang:t("notify.hud_restart"), "error")
    Wait(1500)
    ...
    Wait(500)
    Core.Functions.Notify(Lang:t("notify.hud_start"), "success")
    SendNUIMessage({
        action = 'menu',
        topic = 'restart',
    })
...
RegisterNetEvent("hud:client:resetStorage", function()
    ...
    Core.Functions.TriggerCallback('hud:server:getMenu', function(menu) loadSettings(menu); SetResourceKvp('hudSettings', json.encode(menu)) end)
end)
...
RegisterNetEvent("hud:client:LoadMap", function()
    ...
    if Menu.isToggleMapShapeChecked == "square" then
        ...
        if Menu.isMapNotifChecked then
            Core.Functions.Notify(Lang:t("notify.load_square_map"))
        end
        ...
        Wait(1200)
        if Menu.isMapNotifChecked then
            Core.Functions.Notify(Lang:t("notify.loaded_square_map"))
        end
    elseif Menu.isToggleMapShapeChecked == "circle" then
        ...
        if Menu.isMapNotifChecked then
            Core.Functions.Notify(Lang:t("notify.load_circle_map"))
        end
        ...
        Wait(1200)
        if Menu.isMapNotifChecked then
            Core.Functions.Notify(Lang:t("notify.loaded_circle_map"))
        end
    end
end)
...
RegisterNUICallback('cinematicMode', function(data, cb)
    ...
    if data.checked then
        ...
        if Menu.isCinematicNotifChecked then
            Core.Functions.Notify(Lang:t("notify.cinematic_on"))
        end
    else
        CinematicShow(false)
        if Menu.isCinematicNotifChecked then
            Core.Functions.Notify(Lang:t("notify.cinematic_off"), 'error')
        end
...
RegisterNetEvent('hud:client:UpdateNeeds', function(newHunger, newThirst) -- Triggered in core
    hunger = newHunger
    thirst = newThirst
end)
...
RegisterNetEvent("admin:client:ToggleDevmode", function()
    dev = not dev
end)
...
RegisterCommand('+engine', function()
    ...
    if GetIsVehicleEngineRunning(vehicle) then
        Core.Functions.Notify(Lang:t("notify.engine_off"))
    else
        Core.Functions.Notify(Lang:t("notify.engine_on"))
    end
            ...
            if IsPedInAnyVehicle(ped, false) and not IsThisModelABicycle(GetEntityModel(GetVehiclePedIsIn(ped, false))) and not isElectric(GetVehiclePedIsIn(ped, false)) then
                if exports[Config.FuelScript]:GetFuel(GetVehiclePedIsIn(ped, false)) <= 20 then -- At 20% Fuel Left
                    if Menu.isLowFuelChecked then
                        ...
                        Core.Functions.Notify(Lang:t("notify.low_fuel"), "error")
