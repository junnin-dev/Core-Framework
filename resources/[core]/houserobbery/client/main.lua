...
local function enterRobberyHouse(house)
    ...
    if Config.Houses[house]["tier"] == 1 then
        data = exports['interior']:CreateHouseRobbery(coords)
    end
    Wait(100)
    houseObj = data[1]
    POIOffsets = data[2]
...
local function lockpickFinish(success)
    if success then
        TriggerServerEvent('houserobbery:server:enterHouse', closestHouse)
        Core.Functions.Notify(Lang:t("success.worked"), "success", 2500)
    else
        if usingAdvanced then
            if math.random(1, 100) < 20 then
                TriggerServerEvent("houserobbery:server:removeAdvancedLockpick")
                TriggerEvent('inventory:client:ItemBox', Core.Shared.Items["advancedlockpick"], "remove")
            end
        else
            if math.random(1, 100) < 40 then
                TriggerServerEvent("houserobbery:server:removeLockpick")
                TriggerEvent('inventory:client:ItemBox', Core.Shared.Items["lockpick"], "remove")
            end
        end

        Core.Functions.Notify(Lang:t("error.didnt_work"), "error", 2500)
    end
end
...
local function searchCabin(cabin)
    ...
    LockpickDoorAnim()
    TriggerServerEvent('houserobbery:server:SetBusyState', cabin, currentHouse, true)
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            openingDoor = false
            ClearPedTasks(PlayerPedId())
            TriggerServerEvent('houserobbery:server:searchCabin', cabin, currentHouse)
            Config.Houses[currentHouse]["furniture"][cabin]["searched"] = true
            TriggerServerEvent('houserobbery:server:SetBusyState', cabin, currentHouse, false)
            SucceededAttempts = 0
        ...
        ClearPedTasks(PlayerPedId())
        TriggerServerEvent('houserobbery:server:SetBusyState', cabin, currentHouse, false)
        Core.Functions.Notify(Lang:t("error.process_cancelled"), "error", 3500)
        SucceededAttempts = 0
        FreezeEntityPosition(ped, false)
        SetTimeout(500, function()
            IsLockpicking = false
...
RegisterNetEvent('Core:Client:OnPlayerLoaded', function()
    Core.Functions.TriggerCallback('houserobbery:server:GetHouseConfig', function(HouseConfig)
        Config.Houses = HouseConfig
    end)
end)

RegisterNetEvent('houserobbery:client:ResetHouseState', function(house)
...
RegisterNetEvent('houserobbery:client:enterHouse', function(house)
    enterRobberyHouse(house)
end)

RegisterNetEvent('houserobbery:client:setHouseState', function(house, state)
    Config.Houses[house]["opened"] = state
end)

RegisterNetEvent('houserobbery:client:setCabinState', function(house, cabin, state)
    Config.Houses[house]["furniture"][cabin]["searched"] = state
end)

RegisterNetEvent('houserobbery:client:SetBusyState', function(cabin, house, bool)
            ...
            if closestHouse ~= nil then
                if CurrentCops >= Config.MinimumHouseRobberyPolice then
                    if not Config.Houses[closestHouse]["opened"] then
                        ...
                        Core.Functions.Notify(Lang:t("error.door_open"), "error", 3500)
                    end
                else
                    Core.Functions.Notify(Lang:t("error.not_enough_police"), "error", 3500)
                end
            end
        else
            Core.Functions.TriggerCallback('Core:HasItem', function(result)
                if closestHouse ~= nil then
                    if result then
                        if CurrentCops >= Config.MinimumHouseRobberyPolice then
                            if not Config.Houses[closestHouse]["opened"] then
                        ...
                        Core.Functions.Notify(Lang:t("error.missing_something"), "error", 3500)
                    end
                end
            end, "screwdriverset")
        end
...
CreateThread(function()
    Wait(500)
    requiredItems = {
        [1] = {name = Core.Shared.Items["advancedlockpick"]["name"], image = Core.Shared.Items["advancedlockpick"]["image"]},
        [2] = {name = Core.Shared.Items["screwdriverset"]["name"], image = Core.Shared.Items["screwdriverset"]["image"]},
    }
    while true do
        inRange = false
