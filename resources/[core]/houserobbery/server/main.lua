...
local function ResetHouseStateTimer(house)
    ...
    SetTimeout(time, function()
        ...
        TriggerClientEvent('houserobbery:client:ResetHouseState', -1, house)
    end)
end

-- Callbacks

Core.Functions.CreateCallback('houserobbery:server:GetHouseConfig', function(_, cb)
    cb(Config.Houses)
end)

-- Events

RegisterNetEvent('houserobbery:server:SetBusyState', function(cabin, house, bool)
    Config.Houses[house]["furniture"][cabin]["isBusy"] = bool
    TriggerClientEvent('houserobbery:client:SetBusyState', -1, cabin, house, bool)
end)

RegisterNetEvent('houserobbery:server:enterHouse', function(house)
    local src = source
    if not Config.Houses[house]["opened"] then
        ResetHouseStateTimer(house)
        TriggerClientEvent('houserobbery:client:setHouseState', -1, house, true)
    end
    TriggerClientEvent('houserobbery:client:enterHouse', src, house)
    Config.Houses[house]["opened"] = true
end)

RegisterNetEvent('houserobbery:server:searchCabin', function(cabin, house)
    local src = source
    ...
    if itemFound < 4 then
        ...
        for _ = 1, itemCount, 1 do
            ...
            local itemInfo = Core.Shared.Items[randomItem]
            if math.random(1, 100) == 69 then
                randomItem = "painkillers"
                itemInfo = Core.Shared.Items[randomItem]
        ...
        TriggerClientEvent('Core:Notify', src, Lang:t("error.emty_box"), 'error')
    end

    Config.Houses[house]["furniture"][cabin]["searched"] = true
    TriggerClientEvent('houserobbery:client:setCabinState', -1, house, cabin, true)
end)

RegisterNetEvent('houserobbery:server:removeAdvancedLockpick', function()
...
RegisterNetEvent('houserobbery:server:removeLockpick', function()
    local Player = Core.Functions.GetPlayer(source)
