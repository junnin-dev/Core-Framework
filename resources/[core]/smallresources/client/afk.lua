...
local secondsUntilKick = 1800 -- Limite de tempo de chute do AFK (em segundos)
...
local function updatePermissionLevel()
    Core.Functions.TriggerCallback('afkkick:server:GetPermissions', function(userGroups)
...
RegisterNetEvent('Core:Client:OnPermissionUpdate', function()
    updatePermissionLevel()
end)
                        ...
                        if time then
                            if time > 0 then
                                ...
                                if _type == 'minutes' then
                                    Core.Functions.Notify('Você é AFK e será chutado ' .. math.ceil(time / 60) .. ' minute(s)!', 'error', 10000)
                                elseif _type == 'seconds' then
                                    Core.Functions.Notify('Você é AFK e será chutado ' .. time .. ' seconds!', 'error', 10000)
                                end
                                time -= 1
                            else
