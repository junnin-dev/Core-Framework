...
RegisterCommand('hu', function()
    ...
    handsup = not handsup
    if exports['policejob']:IsHandcuffed() then return end
    if handsup then
        TaskPlayAnim(ped, animDict, anim, 8.0, 8.0, -1, 50, 0, false, false, false)
        exports['smallresources']:addDisableControls(disableHandsupControls)
    else
        ClearPedTasks(ped)
        exports['smallresources']:removeDisableControls(disableHandsupControls)
    end
end, false)
