...
JNClothing.ResetItemTexture = function(obj, category) {
    ...
    if (canChange) {
        if (hasTracker && clothingCategory == "accessory") {
            ...
            if (clothingCategory == "model") {
                ...
                if (buttonType == "item") {
                    JNClothing.ResetItemTexture(this, clothingCategory);
                }
            } else {
                if (buttonType == "item") {
...
JNClothing.ToggleChange = function(bool) {
    canChange = bool;
}
...
$(document).ready(function() {
    window.addEventListener('message', function(event) {
        switch (event.data.action) {
            case "open":
                JNClothing.Open(event.data);
                break;
            case "close":
                JNClothing.Close();
                break;
            case "updateMax":
                JNClothing.SetMaxValues(event.data.maxValues);
                break;
            case "reloadMyOutfits":
                JNClothing.ReloadOutfits(event.data.outfits);
                break;
            case "toggleChange":
                JNClothing.ToggleChange(event.data.allow);
                break;
            case "ResetValues":
                JNClothing.ResetValues();
                break;
...
JNClothing.ReloadOutfits = function(outfits) {
...
JNClothing.SetCurrentValues = function(clothingValues) {
...
JNClothing.Open = function(data) {
    ...
    JNClothing.SetMaxValues(data.maxValues);
    $(".clothing-menu-header").html("");
    JNClothing.SetCurrentValues(data.currentClothing);
...
JNClothing.SetMaxValues = function(maxValues) {
...
JNClothing.ResetValues = function() {
