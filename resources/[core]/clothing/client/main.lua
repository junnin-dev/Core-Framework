...
RegisterNetEvent('clothing:client:getOutfits', function(requiredJob, gradeLevel)
    local gender = "male"
    if Core.Functions.GetPlayerData().charinfo.gender == 1 then gender = "female" end
    Core.Functions.TriggerCallback('clothing:server:getOutfits', function(result)
        openMenu({
...
if Config.UseTarget then
    ...
    CreateThread(function()
        for k, v in pairs(Config.Stores) do
            ...
            exports['target']:AddBoxZone(v.shopType .. k, v.coords, v.length, v.width, {
                name = v.shopType .. k,
                debugPoly = false,
                minZ = v.coords.z-1,
                maxZ = v.coords.z+1,
        ...
        for k, v in pairs(Config.ClothingRooms) do
            ...
            if v.isGang then
                action = function()
                    ...
                    TriggerEvent('clothing:client:getOutfits', v.requiredJob, gradeLevel)
                end
            else
                action = function()
                    customCamLocation = v.cameraLocation
            ...
            exports['target']:AddBoxZone('clothing_' .. v.requiredJob .. k, v.coords, v.length, v.width, {
    ...
    CreateThread(function()
        ...
        clothingCombo:onPlayerInOut(function(isPointInside, _, zone)
            if isPointInside then
                ...
                if zoneName == 'surgeon' then
                    exports['core']:DrawText('[E] - Plastic Surgery', 'left')
                elseif zoneName == 'clothing' then
                    exports['core']:DrawText('[E] - Clothing Shop', 'left')
                elseif zoneName == 'barber' then
                    exports['core']:DrawText('[E] - Barber', 'left')
                end
            else
                inZone = false
        ...
        clothingRoomsCombo:onPlayerInOut(function(isPointInside, _, zone)
            if isPointInside then
                local zoneID = tonumber(Core.Shared.SplitStr(zone.name, "_")[2])
            ...
            if inZone then
                ...
                elseif zoneName == 'barber' then
                    ...
                    if IsControlJustReleased(0, 38) then
                        ...
                        TriggerEvent('clothing:client:getOutfits', clothingRoom.requiredJob, gradeLevel)
                    end
                end
            end
            Wait(sleep)
...
RegisterNetEvent('clothing:client:openOutfitMenu', function()
    Core.Functions.TriggerCallback('clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNUICallback('selectOutfit', function(data, cb)
    TriggerEvent('clothing:client:loadOutfit', data)
    cb('ok')
end)
...
RegisterNetEvent('clothing:client:openMenu')
AddEventHandler('clothing:client:openMenu', function()
    customCamLocation = nil
    openMenu({
...
RegisterNUICallback('saveOutfit', function(data, cb)
    ...
    TriggerServerEvent('clothes:saveOutfit', data.outfitName, model, skinData)
    cb('ok')
end)

RegisterNetEvent('clothing:client:reloadOutfits')
AddEventHandler('clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
...
RegisterNUICallback('close', function(_, cb)
    ...
    disableCam()
    FreezeEntityPosition(PlayerPedId(), false)
    TriggerEvent('clothing:client:onMenuClose')
    cb('ok')
end)
...
function SaveSkin()
    ...
    TriggerServerEvent("clothing:saveSkin", model, clothing)
end

RegisterNetEvent('clothes:client:CreateFirstCharacter')
AddEventHandler('clothes:client:CreateFirstCharacter', function()
...
AddEventHandler("clothes:loadSkin", function(_, model, data)
...
RegisterNetEvent('clothing:client:loadPlayerClothing')
AddEventHandler('clothing:client:loadPlayerClothing', function(data, ped)
...
RegisterNetEvent('clothing:client:loadOutfit')
AddEventHandler('clothing:client:loadOutfit', function(oData)
    local ped = PlayerPedId()
...
AddEventHandler("clothing:client:adjustfacewear",function(type)
    if Core.Functions.GetPlayerData().metadata["ishandcuffed"] then return end
...
function reloadSkin(health)
    ...
    TriggerServerEvent("clothes:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES
    TriggerServerEvent("clothing:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES - Event 2
