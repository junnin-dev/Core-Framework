...
Citizen.CreateThread(function()
    while true do
        ...
        if dist <= 1.0 then
            Core.Functions.DrawText3D(MissionMarker.x, MissionMarker.y, MissionMarker.z,
...
RegisterNetEvent('armoredtruckheist:client:911alert', function()
    if PoliceAlert == 0 then
        ...
        TriggerServerEvent("armoredtruckheist:server:callCops", streetLabel, transCoords)
...
RegisterNetEvent('armoredtruckheist:client:robberyCall', function(streetLabel, coords)
    if PlayerJob.name == "police" then
        ...
        TriggerEvent('policealerts:client:AddPoliceAlert', {
            ...
            callSign = Core.Functions.GetPlayerData().metadata["callsign"]
        })
        ...
        if MissionStart == 1 then
            ...
            if dist <= 7 and BlownUp == 0 and PlayerJob.name ~= 'police' then
                ...
                if IsControlPressed(0, 47) and GuardsDead == 1 then
                    ...
                    TriggerEvent("armoredtruckheist:client:911alert")
                    Citizen.Wait(500)
                end
            end
    ...
    if IsVehicleStopped(transport) then
        if IsVehicleSeatFree(transport, -1) and IsVehicleSeatFree(transport, 0) and IsVehicleSeatFree(transport, 1) and
            ...
            if not IsEntityInWater(PlayerPedId()) then
                ...
                Core.Functions.Notify('A carga será detonada em ' .. TimeToBlow / 1000 .. ' seconds.', "error")
                ...
                BlownUp = 1
                lootable = 1
                Core.Functions.Notify('Você pode começar a coletar dinheiro.', "success")
                RemoveBlip(TruckBlip)
            else
                Core.Functions.Notify('Saia da água', "error")
            end
        else
            Core.Functions.Notify('O veículo deve estar vazio para colocar a carga', "error")
        end
    else
        Core.Functions.Notify('Você não pode roubar um veículo que está se movendo.', "error")
    end
end
        ...
        if lootable == 1 then
            ...
            if dist <= 4.5 then
                if PickupMoney == 0 then
                    hintToDisplay('Press [E] Para pegar o dinheiro')
                    PickupMoney = 1
                end
                if IsControlJustPressed(0, 38) then
                    lootable = 0
...
function TakingMoney()
    ...
    TaskPlayAnim(PlayerPedId(), "anim@heists@ornate_bank@grab_cash_heels", "grab", 8.0, -8.0, -1, 1, 0, false, false,
        false)
    FreezeEntityPosition(PlayerPedId(), true)
    Core.Functions.Notify('Você está arrumando dinheiro em uma bolsa', "success")
