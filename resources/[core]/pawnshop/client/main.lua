...
CreateThread(function()
    if Config.UseTarget then
        for key, value in pairs(Config.PawnLocation) do
            exports['target']:AddBoxZone('PawnShop'..key, value.coords, value.length, value.width, {
                name = 'PawnShop'..key,
        ...
        if GetClockHours() >= Config.TimeOpen and GetClockHours() <= Config.TimeClosed then
            local pawnShop = {
                ...
                },
                {
                    ...
                    params = {
                        event = 'pawnshop:client:openPawn',
                        args = {
            ...
            if not isMelting then
                pawnShop[#pawnShop + 1] = {
                    ...
                    params = {
                        event = 'pawnshop:client:openMelt',
                        args = {
            ...
            if canTake then
                pawnShop[#pawnShop + 1] = {
                    ...
                    params = {
                        isServer = true,
                        event = 'pawnshop:server:pickupMelted',
                        args = {
                            items = meltedItem
                        }
                    }
            ...
            exports['menu']:openMenu(pawnShop)
        else
            Core.Functions.Notify(Lang:t('info.pawn_closed', { value = Config.TimeOpen, value2 = Config.TimeClosed }))
        end
    else
        local pawnShop = {
            {
...
RegisterNetEvent('pawnshop:client:openPawn', function(data)
    Core.Functions.TriggerCallback('pawnshop:server:getInv', function(inventory)
        ...
        for _, v in pairs(PlyInv) do
            for i = 1, #data.items do
                if v.name == data.items[i].item then
                    pawnMenu[#pawnMenu + 1] = {
                        header = Core.Shared.Items[v.name].label,
                        txt = Lang:t('info.sell_items', { value = data.items[i].price }),
                        params = {
                            event = 'pawnshop:client:pawnitems',
                            args = {
                                label = Core.Shared.Items[v.name].label,
...
RegisterNetEvent('pawnshop:client:openMelt', function(data)
    Core.Functions.TriggerCallback('pawnshop:server:getInv', function(inventory)
        ...
        for _, v in pairs(PlyInv) do
            for i = 1, #data.items do
                if v.name == data.items[i].item then
                    meltMenu[#meltMenu + 1] = {
                        header = Core.Shared.Items[v.name].label,
                        txt = Lang:t('info.melt_item', { value = Core.Shared.Items[v.name].label }),
                        params = {
                            event = 'pawnshop:client:meltItems',
                            args = {
                                label = Core.Shared.Items[v.name].label,
        ...
        exports['menu']:openMenu(meltMenu)
    end)
end)

RegisterNetEvent('pawnshop:client:pawnitems', function(item)
    local sellingItem = exports['input']:ShowInput({
        header = Lang:t('info.title'),
    ...
    if sellingItem then
        ...
        if tonumber(sellingItem.amount) > 0 then
            TriggerServerEvent('pawnshop:server:sellPawnItems', item.name, sellingItem.amount, item.price)
        else
            Core.Functions.Notify(Lang:t('error.negative'), 'error')
        end
    end
end)

RegisterNetEvent('pawnshop:client:meltItems', function(item)
    local meltingItem = exports['input']:ShowInput({
        header = Lang:t('info.melt'),
    ...
    if meltingItem then
        ...
        if meltingItem.amount ~= nil then
            if tonumber(meltingItem.amount) > 0 then
                TriggerServerEvent('pawnshop:server:meltItemRemove', item.name, meltingItem.amount, item)

            else
                Core.Functions.Notify(Lang:t('error.no_melt'), 'error')
            end
        else
            Core.Functions.Notify(Lang:t('error.no_melt'), 'error')
        end
...
RegisterNetEvent('pawnshop:client:startMelting', function(item, meltingAmount, meltTimees)
    if not isMelting then
        isMelting = true
        meltTime = meltTimees
        meltedItem = {}
                ...
                if LocalPlayer.state.isLoggedIn then
                    ...
                    if meltTime <= 0 then
                        ...
                        if Config.SendMeltingEmail then
                            ...
                            Core.Functions.Notify(Lang:t('info.message'), 'success')
                        end
                    end
                else
                    break
...
RegisterNetEvent('pawnshop:client:resetPickup', function()
    canTake = false
end)
